...
## 模式 A · 描述 → 实装（交互·有用户描述）
1. **解析描述 → schema 字段**：tier（realistic/uncanny/cosmic）、hp/armor/evasion/speed、threat、hostility、initialStance、aiPattern、attacks[]（含 damageType/damage/~~可选 sanityDamage~~〔已删·« 2026-07-10 理智系统移除 »：无 sanityDamage / DamageType 'sanity' / role 'sanity'〕/injuryOnHit）、weakness/immunity、loot、victoryConditions，以及库元数据 `bands`/`biomes`/`role`/`codex`。
