...
扫描澄清（非 bug）：① lore 条目**内联定义在各 event 文件里**——「~90 条悬空 lore」是误报；② 47 条「未接好选项」全是对话/区域 schema——事件文件里真·未接好选项 = 0；③ DC：体力 12–50、氧 30——满状态都 ≥35% 可过。
...
- 决策器＝「理性谨慎玩家」：捞料、预估回程氧、躲必死/战斗、氧≤reserve+margin 上浮；`fightForLoot` 切接战。改判定改 `player.ts`。（注：sim `player.ts` 仍有旧 sanity≤12 上浮判据待去除·#284）
