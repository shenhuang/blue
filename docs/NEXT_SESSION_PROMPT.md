下个 session 的 prompt — 内容 pass（reef 浅段 uncanny/cosmic + reef 第二只敌人）/ 或 Phase D（invasion/defense）
直接把下面（含本行以下全部）粘进新 session 即可。

你在接手「深海回响」(Deep Echo) —— 潜水题材文字冒险 Roguelike（Vite + React + TS），仓库在 `~/Desktop/Blue`。

**重要里程碑**：基建+地图 revamp 的三根支柱（材料经济 / 多灯塔基地 / 海图）**已全部闭环**——Phase A（材料经济）+ Phase B（灯塔数据模型）+ Phase C（海图集成 + 修复循环）均已实装并提交。所以这个 session **回到内容缺口**（推荐），或开 Phase D（灯塔防御战，revamp 之后的新系统）。
...
## 选项 B：Phase D — 灯塔防御战（invasion/defense）

revamp 给 Phase D 铺好了地基：`Lighthouse.integrity?` / `region?` 字段已预留（inert），SPEC §3.1/§4 留了 `threat` + stub 事件类型的位。现在灯塔真的上线了（home + 可修前哨 + 海图渲染 + 建造），防御战有了载体。这是**大新系统**，动手前务必先写设计（在 SPEC 里开 Phase D 小节或单独 doc）并用 AskUserQuestion 跟作者敲定核心循环（威胁怎么涨、防御怎么打、灯塔被破的后果、与现有战斗系统的关系、是否要 `Lighthouse.integrity` 真消费）。比内容 pass 风险大、面更宽。
