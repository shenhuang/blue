...
> **状态：基础+解耦已实装（2026-06-14 交互 session·「按顺序实装」→「全做」）· 少量余项待续。** 已落地：支柱三 目录自动加载 + 支柱一 元数据回填 9 只 + §4 `pickEnemy`/`matchEnemies` + §6 `check-enemy-refs` 四门 + combat-runner 过滤 +（续）支柱二 `enemyRef`（**route B 加法·非破坏**·`resolveEncounterMember`/`canResolveMember`·startCombat 接入）+ §7.4 `--new-enemy` 脚手架 + §5 入库 SKILL 草案（`docs/skills/深海回响_敌人入库_SKILL.md`）。**regress 31/31 全绿**。待续：route A（把 `combatEncounters` 迁出敌人文件·纯重构·非必须）、模式 B 的 schedule（作者立·SKILL 草案已备）、codex 充实 + 图鉴 UI。详见 CHANGELOG #121（含同日续）。原始设计 2026-06-13。
