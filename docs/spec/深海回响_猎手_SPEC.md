...
- **灯（光）**：只知道「有东西在接近」——存在 + 大致逼近度（远/中/近），**读不出精确位置/距离**。＝现有 `clarity.ts::threatContact` 的模糊琥珀接触（S3 廉价版 #80），方位按 turn 漂移、定不住。**« ⚠ 2026-07-19 #316 已删除：琥珀接触整个移除（不扫也每回合动·与「信息只在扫描时更新」相悖·作者拍板）——「灯＝接近感」通道现空缺，猎手情报只剩声呐扫描快照 + 遭遇本身。»**
...
- 有些猎手避难后等得更久（守在出口）→ 你不想打就得耗更多资源（氧/电）等它走或另寻出路。
