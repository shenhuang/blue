...
| node.1.0 | 21 | `wreck_approach → wreck`（节点内链·鲨鱼战在此·#222 续：原 grouper 石斑鱼改无生物氛围铺垫·避免战斗前再放生物误导新手） | rest |
...
## tutorial.json 改动
- `descent` 删去 `triggerEventId:tutorial.wreck_approach`（→ 变节点移动到 node.1.0）。
- `wreck`（stealth_grab 成功路径）删去 `triggerEventId:tutorial.deeper`（→ 节点移动到 node.2.0）。
- 保留节点内链：`prologue→descent`、`wreck_approach→wreck`、`deeper→captain_quarters`（triggerEventId 不动）。
...
- `§0`：从「单节点 scriptedStart」改断言「3-node·node.0.0=prologue·node.1.0=wreck_approach·node.2.0=deeper」。
