...
| **每潜刷新**（run 级·出洞即弃） | `'run'` 采尽（`run.harvestedNodes`）+ `lastScanTurn`（旧 scanMemory·声呐无升级化 2026-07-19）+ `stalker` + `visitedNodeIds` | `RunState`（不入 profile） | run 结束丢弃（`'run'` 档「下次重进刷新」） |
