...
> **进度（2026-06-06）：S0（#71）+ S1（#74）+ S2（#78）+ S3 廉价版（#80）已落地；§6.5 宏观（揭示动画 + 海况遮蔽）已落地（#80）；声呐范围升级轴（§8.1 主升级轴）已接（#80）。§6.5 即时新 POI 浮现已落地（#83）；S3 定位 stalker（§8.7）已升级为「猎手」子系统、Phase 1 spine 已落地（#84·见 `docs/深海回响_猎手_SPEC.md`）。**定向 ping（§5·#86·暴露按方向计）+ 房间 feature 数升级（§6/§8.3 续·#87·收尾 #74 尾巴）已落地**——声呐与房间 SPEC 的核心 + 收尾全部完成。仍留（均小/可选）：猎手 Phase 2+（升级规避·decoy·大型生物·执着等待·见猎手 SPEC §7）/ 定向 ping『各方向 reach 各自升级』细分（现 `sonarScanRange` 是统一范围轴）/ 声呐图聚焦扇区可视化打磨（现仅文字标注）/ §5 later（接触带大小·开放水域扫描）。**
