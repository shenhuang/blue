...
> **⚠ « 2026-07-19 声呐无升级化（CHANGELOG #315）»**：§5「`scanMemory` 会过时的记忆（逐节点 stamp）」与 §8「射程/升级轴（`sonarScanRange`·1 scan/turn 之外的部分）」已删——扫描态收敛为 `run.lastScanTurn` 单标量（一记 ping 全图揭示·迷雾全图三态 黑/亮/灰·「会过时」语义升格为整图过期）。S0/S1 的房间模型、洞穴 SDF 渲染、「1 scan/停留」、付电+自曝 均不受影响。
> **⚠ « 2026-07-19 标记层收窄（CHANGELOG #316）»**：**S3 威胁定位（琥珀 `threatContact`）整个删除**（作者拍板——alert 驱动·方位按 turn 漂移＝不扫描也每回合动，与「信息只在扫描时更新」相悖）；「残图小地图」与「非相邻定位标记」同删（只画能抵达的相邻节点 + 敌）。敌显现在只剩：追猎红点（扫描快照）+ The Warren 女王（扫过后实时常显·boss 特权）。
