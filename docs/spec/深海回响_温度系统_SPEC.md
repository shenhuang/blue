...
超过 `WARN` 阈值后按应力连续扣**体力**（热极=过热脱力 / 冷极=失温麻木·叙事分极性·数学同款）。沿用旧氮醉 `narcosisSanityDrain` 的**连续·确定性·低应力≈0**形状（注：`narcosisSanityDrain` 本身已随理智系统移除删除·« 2026-07-10 »——本函数落点是**体力**、不是理智，机制不受影响）：
