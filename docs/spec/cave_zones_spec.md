...
5. **事件池 Batch 1–4**（tide/grotto/deep_cave/chasm）照 cave_batch_prompts.md。**区数按删后更新**：cave 4→3 · crack 4→3 · tide 4→3 · grotto 4→3 · deep_cave 7→6（事件数不变·按 tag 共享）。
...
| tag | 区数 | 新事件 | 对应区 |
|-----|------|--------|--------|
| cave | 4 | 0 | reef_pocket, kelp_hollow, coral_grotto, wreck_hold（10–42m·blue_caves 池上限 55m 覆盖充分） |
| crack | 4 | 0 | anchor_hollow, limestone_slot, sunken_chimney, drowned_well |
| tide | 4 | **7** | tidal_seam, tidal_bore_cave, silt_alcove, sea_arch_cave |
| chamber | 3 | 0 | sandfall_cave, blue_throat, lava_branch |
| vent | 1 | 0 | thermal_pocket（热液渗入·复用现有 vent 池） |
| grotto | 4 | **7** | moonpool_cavern, ossuary_passage, echo_cavern, submerged_arch |
| deep_cave | 7 | **8** | trench_hall, black_basin, serpentine_deep, collapsed_caldera, mirror_maze + blind_alley + murk_gallery（deep_cave 池覆盖 35–124m·深度足够） |
...
```json
{
  "id": "zone.coral_grotto",
  "name": "珊瑚洞",
  "description": "礁石里一道珊瑚长得特别密的穴，颜色比外面艳，温度比外面低，声音比外面小。",
  "depthRange": [14, 32],
  "layerCount": 3,
  "nodesPerLayer": [1, 2],
  "zoneTagsByDepth": [{ "minDepth": 0, "tags": ["cave"] }],
  ...
  "generation": "random",
  "mapShape": "maze",
  "depthCurveRange": [1.5, 2.5],
  "canFreeAscend": false,
  "ambushEncounters": ["combat.reef_grouper_solo"]
}
```
...
```json
{
  "id": "zone.sunken_chimney",
  "name": "沉烟囱",
  "description": "竖直的管状裂缝，从上面看下去全是黑的，缝壁摸上去是湿的冷的，不知道下面多深。",
  "depthRange": [40, 72],
  "layerCount": 4,
  "nodesPerLayer": [1, 2],
  "zoneTagsByDepth": [{ "minDepth": 0, "tags": ["crack"] }],
...
```json
{
  "id": "zone.sea_arch_cave",
  "name": "海拱穴",
  "description": "海蚀拱的内侧，水面以下还有一段往里延伸，光条有时候能从拱顶的缝里漏下来。",
  "depthRange": [12, 30],
  "layerCount": 3,
  "nodesPerLayer": [1, 2],
  "zoneTagsByDepth": [{ "minDepth": 0, "tags": ["tide"] }],
...
```json
{
  "id": "zone.echo_cavern",
  "name": "回声穴",
  "description": "某些形状的水道会让声音从三个方向回来。气泡声落在里面是一种不对的节奏。",
  "depthRange": [44, 80],
  "layerCount": 6,
  "nodesPerLayer": [2, 3],
  "zoneTagsByDepth": [{ "minDepth": 0, "tags": ["grotto"] }],
...
```json
{
  "id": "zone.trench_hall",
  "name": "沟堂",
  "description": "海沟侧壁里蚀出来的大型腔室，高度超过视野范围。顶部是黑的，底部也是黑的，自己在哪一段很难说。",
  "depthRange": [72, 114],
  "layerCount": 9,
  "nodesPerLayer": [2, 3],
  "zoneTagsByDepth": [{ "minDepth": 0, "tags": ["deep_cave"] }],
