...
> **⚠⚠ 二次 supersede · 2026-07-10 理智系统移除**：上条把欺骗换成的那根「低理智」轴**现也整体删除**——连续理智值 `run.stats.sanity` + 事件理智检定（`check.stat:'sanity'` 全塌成 success 分支）+ `sanityRange` 门 + 幻觉系统（`HALLUCINATION_*` / `maybeHallucinationEncounter`）全删；感知**全诚实、零欺骗轴**；「发疯 / 头脑不正常」改为二态节点门（地点缝 seam·`types/dive.ts` `DiveNode.seam.bypassCapability:'steady_mind'`〔定心坠〕·无它入缝即 `executeDeath`·当前 DORMANT〔0 seam 节点〕）。SAVE_VERSION 13→14·守门 `scripts/check-no-sanity.mjs`（叙事中文「理智 / 幻觉」不禁）。**本档一切 `sanity` / 低 san / tell↔sanity / §3.7「另一个世界」均作废**；诚实感知侧（灯 / 声呐 / 电 / signature / 射程 / 探图）仍保留。**§3.7 留下的设计缺口见其块内 « TODO(作者) »。**
...
- « 2026-07-10 理智移除：`sanity`「你轴」删除·现只剩诚实世界；「拒绝裁决 / d_reveal 暧昧」的**叙事口径保留**，但不再靠理智值做成机制。 »
- « 2026-07-10 理智系统移除·**整条作废**：连续理智值删除·再无「可主动压低的理智」这把钥匙·「另一个世界」§3.7 因此失去驱动＝设计缺口（见 §3.7 TODO(作者)）。 »（详 §3.7）
...
1. **clarity 模型 = 双传感器 + 探测双刃**（不是单纯「灯塔光−深度」涌现）：
   ...
   —— 主动感知是**双向的**：照亮/ping 让你看见世界，也把你暴露给世界。**细化（2026-06-02 续，见 §3.2）：声呐独立于灯塔网；灯＝近距地面真相、声呐＝远距但返回不可信（可被生物躲/骗）；耗电声呐多灯少、暴露灯高声呐较低；灯/声呐 效果·耗能·电量都可升级。**
2. **最深层 = 分层 + 递归纵深**：
...
4. « 2026-07-10 理智移除·**作废**：无理智值·tell 不再随理智失真或加难；tell 作为诚实信息（灯 / 声呐 / 盯看）保留·读不读得懂靠玩家。 »
...
- **声呐 = 不可信传感器（核心欺骗面）**：你看到的是声呐的**返回**，不是真东西，返回与实际有**差**；而这个差可来自两个你**分辨不了**的源头——① 生物**躲开**声呐（没回波）/ ② 生物**喂假回波**（把自己显示成地形/信标/空水——**mimic 就是这类**）。同一个可疑回波，游戏**拒绝告诉你**是真、是骗。
- **要确认真相只能靠近用灯看**（近距＝地面真相）——但点灯凑近又是最高暴露。**读真相永远要自曝，张力是结构性的、不靠脚本。**
...
- « 2026-07-10 理智移除·作废：无 sanity 耦合；黑暗里信息少仍成立（灯门硬约束·感知重做 §2.1），但不再叠加理智失真。 »
...
- **入潜兑现**：抵达后假信标解析成 mimic（mimic 事件本身保留·感知重做 §4 冻结）。tell（点灯/ping/盯看） « 2026-07-10 理智移除：tell 现为**诚实信息**·读不读得懂靠玩家·无理智失真 »。**读出 tell + 够强 = 活**（付代价、走开）；读错/太弱 = 被吃（但原则上仍可逃，代价惨重——无脚本死）。
...
### 3.7 另一个世界（~~低 san 的另一面~~，亦真亦假；多在 Phase 3+ 兑现）  « 2026-07-10 理智系统移除·**整节作废** »

> « **TODO(作者)：「另一个世界」失去驱动＝设计缺口** »——本节整套「主动压低理智 → 解锁只在低 san 出现的节点 / POI / 事件 / 路径」建在连续理智值 + `sanityRange` 低段门控上。理智值删除后，**没有可压低的理智、也没有低-san 状态来 gate 这层可探索内容**。这与剧情 SPEC §4.4「基因路降理智入幻＝解锁动词」、§1「驾驭·刻意入幻」是**同一根机制**（压低理智 → 过到另一层），一并塌掉。缺口待作者定向：给「另一个世界」新造一个不依赖理智值的进入门（候选纯占位·未拍：地点缝 seam 的扩展？特定装备 / 仪式触发的「过界」态？），还是弃掉「主动压低进另一层」这一取舍、把「只在特殊态出现的内容」改挂别的条件？**不臆造替代设计·留作者拍。** 下列原机制描述保留作历史理据。

- ~~低 san 不止是惩罚（声呐失真、灯也幻觉、tell 难读）——它**同时解锁另一层可探索内容**：只在低 san 才「出现」的节点 / POI / 事件 / 路径（机制＝`sanityRange` 低段门控，quirk #21 的泛化）。~~
- **亦真亦假**：那个世界是真的、还是被压垮的脑子看出来的，**游戏拒绝裁决**（同 §1·叙事口径保留）。~~mimic / corpse-wearer / d_reveal 多半住这一侧。~~ « 2026-07-10：mimic / corpse-wearer / d_reveal 本就靠海图 + 事件（感知重做 §4 冻结·不依赖理智）·继续存在·只是不再挂「低 san 侧」。 »
- ~~**取舍（理智的双向经济）**：高 san 在真实世界看得清、活得稳；**主动压低 san 才进得去另一个世界**（代价＝再信不过自己的感知）。理智从「单向要守的血条」变成**双向的钥匙**。~~ « 2026-07-10 理智移除·作废 »
- ~~**顺带救活死内容**：低-san 门控内容此前几乎触发不到（quirk #21：`silent_chamber`「只在被压垮的潜水员眼里出现」，但没机制把 san 压下去）——给低 san 一个去处后，这些内容活了。~~ « 2026-07-10 理智移除：`sanityRange` 门控删除·`silent_chamber` 这类「只在低 san 出现」的内容现无触发条件·随理智系统一并作废（要保留需改挂非理智条件·见节首 TODO）。 »
...
海图假 POI（无灯之光，§3.5）→ 横渡 → 入潜遭遇；corpse-wearer 姊妹 apex；接 `flag.d_reveal`。回收浅/中/深 + trench/abyssal 伏笔。
...
- **Phase 3**：`ChartPoi` 加 `mimic?: true`（假 POI，不被任何灯塔解释却 lit）；mimic 敌人 def（apex）；d_reveal 触发 outcome。

---

## 7. 待作者复核的子决策（带提案）

1. ~~灯塔 ↔ 声呐网是否同一套？~~ **已定（2026-06-02）：独立。** 声呐是潜水员**自带装备**，灯塔网只管宏观海图、不下到水里。声呐的代价（耗电 + 诱敌）与不可信（返回≠真、可被躲/骗）由装备本身承担，不靠网络耦合。见 §3.2。
...
- **电（Phase 0）**：电池总量 / 声呐 ping 单次耗电 / 灯每回合耗电（声呐 >> 灯）/ 灯·声呐·电量三条升级轨的档位与账单 / 声呐被躲·被骗的触发与强度。
