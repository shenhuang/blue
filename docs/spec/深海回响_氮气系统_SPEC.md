...
> **⚠ « 2026-07-10 理智系统移除 »**：`run.stats.sanity` 已删。**本档 §3「氮醉→理智」整节作废**——`narcosisSanityDrain` / `NARCOSIS_K` / `NARCOSIS_P` / `ambientPressure` 全删。**氮气现在只喂减压债（上浮 bends·§4）**，不再有任何"氮醉扣理智"通路。累积模型（§2）、上浮 gating（§4）、战斗氮气 ×1.5（`combatNitrogenGain`）**均保留不受影响**。SAVE_VERSION 13→14（Stats 重塑·quirk #99 不迁移）。
> 关系：本档 **supersedes 主 SPEC §3/§5 与 §390/§401–402 的氮气数值**（旧 `depth/30` 线性式 + 硬阶梯氮醉）。
> 主 SPEC 的氮气「角色定位」（氮气=债务）不变；~~理智=边界~~〔理智轴已删·« 2026-07-10 »〕。~~剧情 SPEC 的氮醉宇宙观（深海还你你带下去的东西·永不交底）是本档要**接线兑现**的对象~~——« 2026-07-10 理智系统移除 »：氮醉→幻觉的接线目标已随理智删除撤销；剧情侧氮醉叙事若保留需另找机制载体（不再走理智）。« TODO(作者)：氮醉宇宙观是否改挂别的表现（纯文案 / 体力 / 其它）或整体下架 »
...
3. ~~**氮醉→理智**：连续函数（随 氮气 × 深度 平滑增长），非 SPEC 旧硬阶梯。~~ « 2026-07-10 理智系统移除 »：整条决定作废（理智轴已删·见 §3）。
...
- ~~**氮醉→理智**：**引擎里完全没有**。`tickTurns` 扣理智只有「深度≥30」与「能见度」两项，与氮气零关联。但主 SPEC §3/§401 与整个剧情 SPEC 一章宇宙观全压在氮醉上 → **spec↔code 承重墙缺口**。~~ « 2026-07-10 理智系统移除 »：理智轴整体删除后此缺口消解——氮醉不再需要接线到理智（本条审计作废）。
...
> **✝ « 2026-07-10 理智系统移除 »：本节整体作废。** `narcosisSanityDrain` / `narcosisDrain` / `NARCOSIS_K` / `NARCOSIS_P` / `ambientPressure` 已删，氮气**不再扣理智**（只喂减压债·§4）。以下模型 / 常量 / 锚点值仅存为历史设计，**不再是任何当前机制**；§2 累积与 §4 上浮 gating 不受影响。

```text
narcosis_per_turn = k · (N/100)^p · (P(d) − 1)        // k = 0.4, p = 2
                  = 0.4 · (N/100)^2 · (d/50)
```

- `(N/100)^p`（p=2）→ 低氮几乎不扣、高氮才咬（连续地复刻「阈值感」）。
- `(P(d)−1)=d/50` → 深度加权（同氮量，越深越醉），兑现 SPEC「高浓度 **+ 深度**」。

**每回合扣理智（首版 k=0.4,p=2）**：60m/N45 → 0.10；108m/N66 → 0.38；180m/N84 → 1.0；290m/N94 → 2.1。

**与现有理智压力叠加**：`tickTurns` 现有的「深度≥30 基础衰减」(0.2/0.5/1.0) 与「能见度」仍在 → 氮醉是**叠加**项。深+高氮处总压力会很重（合意：深渊本就是理智死亡陷阱），但首版要 playtest 一遍总量；可选项：把粗糙的「深度基础衰减」部分让位给氮醉（更同源）——首版**先保留两者**，落地后再评。

**这条解锁的不只是数值**：理智阶梯（红字选项门 / 假节点 / UI 故障）按 `sanity` 门控 → 氮醉是把一章幻觉系统真正点亮的那根线（剧情 SPEC §3.4 早已指定「对接现有系统」，本档即那次对接）。
...
| 文件 | 改动 |
|---|---|
| `engine/nitrogen.ts` **（新）** | 模型单点：`ambientP(d)`、`ceiling(d)`、`stepNitrogen(N,d,Δt)`、~~`narcosisDrain(N,d)`~~〔已删·« 2026-07-10 »〕、`N2` 阈值。纯函数、便于回归。 |
| `engine/events.ts` | `tickTurns`：氮气改调 `stepNitrogen`；~~新增 `narcosisDrain` 扣理智~~〔作废·« 2026-07-10 理智系统移除 »〕。 |
...
2. ~~**氮醉**：`narcosisDrain` 对 N、对深度单调；低氮 ≈ 0；几个锚点值钉死。~~ « 2026-07-10 理智系统移除 »：门作废（`narcosisDrain` 已删）。
...
- ~~**P2 氮醉**：接 `narcosisDrain` 扣理智（可与 P1 同批）。门：#2。~~ « 2026-07-10 理智系统移除 »：整期作废（理智轴已删）。
- **P3 上浮 gating**：isAscentBlocked 统管 + CombatView/AscentView 改 + 删凿穿洞顶。门：#3、#4、#5。
- **P4 内容（可单独 session）**：主线中期（约 45–60m）安排「氮气演示」一拍——让玩家停留看氮气可见上涨 + 首尝一次减压停留~~/轻氮醉~~〔氮醉不再有机械后果·« 2026-07-10 »〕。属叙事内容，不阻塞 P1–P3。

---

## 8. 待作者确认的旋钮

- `D0=100`、`τ=15`~~、氮醉 `k=0.4/p=2`~~〔氮醉常量作废·« 2026-07-10 »〕：首版数值，是否同意作为 playtest 起点。
- 上浮 `N2` 阈值（40/60/80）与 bends 数值：随新曲线是否要顺手重调一遍。
- ~~理智双压力（深度基础衰减 + 氮醉）首版保留 vs 让位合并。~~ « 2026-07-10 理智系统移除 »：无理智轴·本旋钮作废。
