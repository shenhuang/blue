...
> 主 SPEC 的氮气「角色定位」（氮气=债务、理智=边界）不变；剧情 SPEC 的氮醉宇宙观（深海还你你带下去的东西·永不交底）是本档要**接线兑现**的对象，不动其叙事。
...
3. **氮醉→理智**：连续函数（随 氮气 × 深度 平滑增长），非 SPEC 旧硬阶梯。
...
- **氮醉→理智**：**引擎里完全没有**。`tickTurns` 扣理智只有「深度≥30」与「能见度」两项，与氮气零关联。但主 SPEC §3/§401 与整个剧情 SPEC 一章宇宙观全压在氮醉上 → **spec↔code 承重墙缺口**。
...
| 文件 | 改动 |
|---|---|
| `engine/nitrogen.ts` **（新）** | 模型单点：`ambientP(d)`、`ceiling(d)`、`stepNitrogen(N,d,Δt)`、`narcosisDrain(N,d)`、`N2` 阈值。纯函数、便于回归。 |
| `engine/events.ts` | `tickTurns`：氮气改调 `stepNitrogen`；新增 `narcosisDrain` 扣理智。 |
...
2. **氮醉**：`narcosisDrain` 对 N、对深度单调；低氮 ≈ 0；几个锚点值钉死。
...
- **P2 氮醉**：接 `narcosisDrain` 扣理智（可与 P1 同批）。门：#2。
- **P3 上浮 gating**：isAscentBlocked 统管 + CombatView/AscentView 改 + 删凿穿洞顶。门：#3、#4、#5。
- **P4 内容（可单独 session）**：主线中期（约 45–60m）安排「氮气演示」一拍——让玩家停留看氮气可见上涨 + 首尝一次减压停留/轻氮醉。属叙事内容，不阻塞 P1–P3。

---

## 8. 待作者确认的旋钮

- `D0=100`、`τ=15`、氮醉 `k=0.4/p=2`：首版数值，是否同意作为 playtest 起点。
- 上浮 `N2` 阈值（40/60/80）与 bends 数值：随新曲线是否要顺手重调一遍。
- 理智双压力（深度基础衰减 + 氮醉）首版保留 vs 让位合并。
