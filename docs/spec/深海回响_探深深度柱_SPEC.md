...
> 状态：**已实现（2026-06-14·#130/#131，§10 定案版落地）**——机制+级数/深度/电梯 capstone 均已 land（regress 34/34 + build 绿）；数值仍为占位起手值，待作者统一调（见 defer-number-tuning）。
>
...
### 实现注记（2026-06-14·已实现并 commit·CHANGELOG #131·commit `37bd41a` 等·regress 34/34 + build 绿）
...
- **电梯 capstone**：`DepthColumnTier` 加 `capstone?`+`setsFlag?`（types/columns.ts）；`engine/columns.ts::columnTrack` 把 `t.setsFlag` 透传进派生 `LighthouseUpgradeDef`（capstone 用不同文案·名义 360m）。新 item `item.station_module`（items.json·material·sellPrice 0·decay eternal·**获取途径未实装**·占位由额外剧情得·测试/dev 直接给）= 海沟 lv4 料门（canBuildAt 走料）。建 lv4 → `setsFlag=story.ch1.station_found`（`STATION_FOUND_FLAG`·engine/story.ts·已登记 `allStoryFlags()`）（hook 已接·**暂不可见**）+ 电梯入口潜点（`poi.dive.trench.t4`）lit·摆 `{0.86,0.44}`（海沟前哨揭示圈内·脱离 t1-t3 簇·`mapX/mapY` 显式坐标）。海沟竖井 t1-t3 设 `noPoi`（band/probe 照常派生·不出海图 POI ＝ 海沟只露电梯一个下潜点）。**科考站 flag-gated 区（圈）暂不加**（作者 2026-06-14「以后再说·先别加圈」）——将来加回 `chart_regions` revealFlag 指向该 flag 的 region 即接上。
