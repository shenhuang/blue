...
**强度档（Low / High / Max ＝ 推理 + 校对投入·与模型选择正交）**：
- **Low** — 机械/确定性数据改·面窄·`regress:quick` 即可兜（找→删→调→grep）。
- **High** — 有分量但范围清楚：浅水文案创作 / 单文件深水叙事 / 海图接线（reach 门紧）/ 边界清楚的新机制。
- **Max** — 架构 epic（存档形状 / 持久图 / 跨系统类型）+ 最难的欺骗轴叙事·要深推理 + 全局校对·**别降级**。
...
## E1–E4 · 四个事件池（tide/grotto = Sonnet · High·deep_cave/chasm = Opus · High）
...
## T2 · B 持久大图 多口洞架构（Opus · Max · 架构 epic）
...
## T2b · 深门 循环再生洞「会呼吸的洞」（Opus · Max · 依赖 T2 · ⏳ 最后期 · 初期不做）
...
## T3a · chart-POI 基础接线（Sonnet · High · 依赖 T1）
...
## T3b · 跨 beacon 多口 POI（Opus · High · 依赖 T2 + T3a）
