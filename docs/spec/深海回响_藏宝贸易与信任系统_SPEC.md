...
- **诡异不可辨（核心·作者拍）**：你终于在最深处见到失联的同伴，但游戏**永不确认那到底还是不是本人**——**(a) 死了、被有一定智慧的水鬼穿上；(b) 生物改造过度、活着但变得和水鬼一样**，没人能证实。**Vera 不说话**（锁死模糊·也省整棵对话树·靠摆货/手势交易）；玩家一直在她身上找"是她"的证据（按断片说脸永不可见），game 一次都不给。
...
- **唯一特例**：能开店、有智慧的水鬼比"顶级捕食者"高一档 → 定成**全局唯一的特例**（更古老/唯一一个），**别让普通水鬼沾这智慧**（守住怪物平时的威胁·接 `boss_enemy_design` 水鬼定位）。
...
6. **Vera 唯一门（延后·随 Vera 建）**：深区特例水鬼商人全局单一。
...
**§12.11 消化**：队名/币/徽定（本节）；Corin/Nerea 名 draft 保留；**仍待作者**：四件收藏品具体物件（Corin 现用「半枚红喉鹈币」·Nerea 待）、Vera 不可逆价 + 最深区落位、Sela 独立信任面板（§3.8·现只商店头显数字）、全数值（defer）。**下一步＝Slice 2**（Nerea 线埋 Vera 伏笔 + 渐深遭遇「Sela 深一柱再遇」+ 藏宝图 Mira便宜/Sela好图分层·§12.10·Opus·medium）。**〔#247 更新〕Slice 2 的 Ch1 部分已落·Nerea 线移二章·见 §12.13。**

### 12.13 实装状态（2026-07-02 · #247 · Cowork 交互 · Opus · Slice 2 的 Ch1 部分落地 · Nerea 移二章）

**作者拍（在场）**：① 收藏品统一为**赤喉海盗团徽章**（非币）——Corin 遗物从「半枚红喉鹈币」**retcon** 成 `item.keepsake.corin_badge`；**徽章两段揭示**：交还 Corin 徽章（Ch1·stage 1）Sela 收口只说「说来话长·不是今天」，交还 Nerea 徽章（Ch2·stage 2）才交底赤喉海盗团旧事——徽章＝「他们从前不只是下海捞宝」的逐件累积实证。② **Nerea 整条线移到第二章**（＝生物派主区·贴她生物押边）；二章零代码，故 Nerea 藏宝线延后二章建。

**已落**（sandbox `npm run regress` 全绿·含 tsx 行为测·build 留 Mac/nightly·未提交）：
- **Corin 徽章 retcon**：`item.keepsake.corin_coin`→`item.keepsake.corin_badge`（连 items.json/sela.json/corin.cache 事件/`playthrough-trust`·全仓 grep 无残留 corin_coin）；`sela.corin_return` 文案改 stage-1 收口。
- **渐深遭遇**（§12.4）：热液区 3 个 `roam.sela_meet_deep_{new,waxing,full}`（owner `lighthouse.ch1_vent_outpost`·depth 90-110·gated `story.ch1.anchor.vent`+`flag.sela.met`+`intel.mira.sela`+相位窗·`lunarOffWindow hidden`）+ 3 个 `sela.meet_deep_*` 事件（置 `flag.sela.met_deep`·`flag.*` 非 story.* 不进 `allStoryFlags`·供二章 Nerea offer 门控消费）。`check-lunar-reach` 绿（roaming 带窗合法）。
- **藏宝图来源分层「低层」**（§12.4）：Mira 便宜卖 `item.treasure_map.salvage_generic`（20 金·新 `SHOP_STOCK_CHARTS` 表·**价与 sellPrice 解耦**〔story 物 sellPrice=0·miraOfferFor 会算成 0〕·`oneTimeFlag flag.salvage_chart_bought` 守一次性〔买过不上架 + buyFromMira no-op·免回港补货重复扣金币〕）→ story.setsFlag+marksPois 揭示浅礁固定点 `poi.anchor.salvage_generic`（owner home·depth 30-45）→ openEvent `salvage.generic_cache` 给 modest 常见打捞料（gold 源·对照 crew 好图更深+带收藏品）。Mira 新对话节点 `mira.sell_chart`（gated `story.ch1.anchor.reef`+`notHasFlag salvage_chart_bought`）。
- **门/测**：`playthrough-trust §6`（Mira 图买→揭示→一次性 no-op）+`§7`（深层遇见 gated `story.ch1.anchor.vent`）；经济 DAG 因新 loot 重排·已 `emit-economy-graph --write` 再生。

**仍待作者**：Nerea 二章线（图/藏点/stage-2 揭示·待二章落地）、各人徽章记号区分、Vera 不可逆价 + 最深区落位、Sela 独立信任面板（§3.8）、全数值（`defer-number-tuning`：`SHOP_STOCK_CHARTS` 价 20 / gainTrust 量 / loot 数都是占位）。

---

### 12.14 实装状态（2026-07-03 · Cowork 交互 · Opus · Sela 引荐门：与深度脱钩 → Mira 打捞委托单点门控）

**作者拍（在场）**：① **Mira ＝销赃中间人 / receiver**（跟海盗经济做买卖、知道谁是谁、偶尔进红喉鹈币），**非**赤喉海盗团前成员——保住「浅处开明店的她 vs 被通缉藏在深处的 Sela」对比 + 不稀释 Sela 唯一性；**留缝**：她「怎么这么熟赤喉海盗团」当伏笔，日后 Mira 高信任期再抖（现只做「引荐门」这一处二元用途）。② **Sela 引荐门与深度脱钩**：`sela_tip` 不再直接门 `story.ch1.anchor.midwater`，改由 Mira 一次性打捞委托门控。③ **门在「任务完成 flag」而非 `npcTrustTier`**——「引荐了没」是二元 story 事实、Mira 暂无分档内容；flag 门让「做完委托 ⟹ 见得到 Sela」是**结构性保证**，免被 `defer-number-tuning` 的调参把 grant 压到门槛下、复现「白干」bug（数值门的隐患）。Mira 分级信任轨等她真有分档内容再 **additive** 接（§4 retrofit）。

**设计要点（单一旋钮＝任务位置）**：深度只写在 `poi.anchor.mira_salvage` 的 offer 门上（`requiresFlags` 含 `story.ch1.mira_salvage_offered`，而该 flag 由委托对话在 `story.ch1.anchor.midwater` 之后才给）。**挪任务＝只改这一处 offer 门**，Sela 可达点自动跟着深/浅，不用回头改 `sela_tip`——避免「两处都写深度、把任务挪早了 `sela_tip` 还卡深度＝矛盾 / 白干」。§8 红线仍守：全为支线、不进任何主线 reach/reveal gate。

**已落**（sandbox `npm run regress --skip build` **86/86 全绿**·含 tsx 行为测·build 留 rollup-linux nightly·未提交）：
- **flag 单源**：`engine/story.ts` 加 `MIRA_SALVAGE_OFFERED_FLAG` / `MIRA_SALVAGE_DONE_FLAG`（`story.ch1.mira_salvage_{offered,done}`）+ 进 `allStoryFlags()`（受 playthrough-story §4「data story.* ⊆ allStoryFlags」门守）。
- **Mira 对话**：`dialogRoot` 加 `mira_salvage_offer` 选项（gated `story.ch1.anchor.midwater` + `notHasFlag …offered`）→ 新节点 `mira.salvage_brief`（`onEnter setFlag …offered`·文案＝Mira 第一处 fence tell：有标记沉船的旧图、知道侧舱有铅封匣、以「工钱」让你留下匣里的东西）；**regate** `sela_tip`：`hasFlag` 从 `anchor.midwater` 改 `mira_salvage_done`。
- **POI + 事件**：`chart_pois` `poi.anchor.mira_salvage`（owner 中层哨站·`requiresFlags` tutorial+offered·`openEventId mira.salvage`·`openEventFlag mira_salvage_done`）+ `events/midwater.json` 事件 `mira.salvage`（撬铅封匣·loot `item.deep_token`〔红喉鹈币工钱＝tell + bootstrap 玩家进 Sela 币本位经济〕·`setProfileFlags mira_salvage_done`·留诚实退路）。军舰鸟徽 + 受潮名册＝赤喉海盗团 tell（不点破·withhold 纪律）。
- **门/测**：`playthrough-trust §8`（引荐门耦合：载真 `sela_tip.visibleIf`·`evalCondition` 三态·`applyDialogEffects` 兑现 `intel.mira.sela`·断言「门是 `mira_salvage_done` 非 `anchor.midwater`」）+ `§9`（POI 接委托前 hidden / 后 lit）；经济 DAG 因新 `deep_token` 源重排·已 `emit-economy-graph --write` 再生。

**仍待作者**：Mira 分级信任轨（additive·§4）、委托是否加「回港交差」独立节拍（现无强制 return·做完直接回港 `sela_tip` 解锁）、全数值（`defer-number-tuning`：loot `[2,3]` / offer 点坐标 / 委托文案打磨期过稿）。
