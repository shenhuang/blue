...
2. 跟坐标**打四个 boss**（＝ ch1-boss-branches 两派四分支 · The Warren 已成 · #2 在做）——每打完一个**触发额外剧情**、在一个**封存的箱子**里拿到**剩余笔记的一块**（非裸 drop）。
3. **集齐全部笔记 → 直接解锁海沟区（z5 · ~300 m）的观测站**（＝ §4 C 地理落点）。**解谜机关＝future**（作者现只做「集齐即进」· 真解谜以后加）。
...
- 旧 `ending_blank` + `recording_1`（持破损饰品重访空站 + 录音）→ **删除**（作者拍）：揭穿全交给结局二（Théo 水鬼）· 连带删 `ending_blank` 事件 + `recording_1` lore + `story.ch1.recording.1` flag（`story.ts::ch1RecordingFlag`）+ `poi.dive.vent.story` 的 revisit 门。⚠ 这撤掉 [[remove-sanity-system]]/[[story_canon]] 已实装的「录音第 1 段·录音分段·2+段留二章」那条线。
- **定心坠来源改**：定心坠＝**稳幻觉**的独立饰品（≠ 回影坠〔穿越〕· 两枚不同）· 由**碎片**制作；碎片来源 **旧＝Ch1 观测站 → 新＝Aldo（结局一后给）**· 在**禁岛完成/制作**（宝石材料 · 合 [[forbidden_island]]「破损→群宝修复」链）。
- **观测站可达门扩容**：从「建海沟前哨」扩成「四 boss + 集齐笔记 + 解谜」（§10 前哨料经济仍是到海沟的物理门 · 与剧情门叠加）。

**开放问题（fresh session 定 · 需作者）**
- ~~① Ch1/Ch2 边界~~ **结局二算 Ch1**（作者拍）：整条弧到结局二＝Ch1；禁岛/极地火山在 **Ch1 尾被引入并部分用**（禁岛做定心坠），**完整区域内容＝Ch2**（[[forbidden_island]]）。
- ~~② 定心坠 vs 回影坠~~ **两枚不同饰品**（作者拍）：定心坠＝稳幻觉（碎片制作 · 碎片旧观测站→新 Aldo · 禁岛完成）；回影坠＝穿越（禁岛 · 驱动基因路 boss 链 · future）。
- ~~③ 解谜形式~~ **目前＝集齐笔记即可进观测站 · 无机关**（作者拍 · 真解谜 future）。
- ~~④ 四 boss 掉笔记~~ **每 boss 打完触发额外剧情 · 在封存箱子里拿一块笔记**（作者拍）。
- ~~⑤ recording_1 存废~~ **删**（作者拍 · 见上「该改改该删删」）。

**（§6 开放问题至此全部拍定 · Ch1 弧定稿。）**
