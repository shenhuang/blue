...
**前哨新建造料**（作者给 · **保留金币**〔金额沿用旧档占位·待调〕· 摊进旧 3 阶段）：

- **残骸前哨**（z2）：① 清船体当塔基 `铜质结核×1` · 金 40 → ② 搭落脚架/浮筒 `铜质结核×1` · 金 70 → ③ 通电点亮 `冷光腺×1` · 金 110
- **中层浮标**（z3）：① 抛锚定位基座 `铜质结核×2` · 金 50 → ② 撑浮标架/系留 `铁质结核×2` · 金 90 → ③ 通电点亮 `冷光腺×1` · 金 130
- **热液井台**（z4）：① 凿锚位 `铁质结核×2` · 金 70 → ② 架抗流井台 `铝质结核×2` · 金 120 → ③ 通电点亮 `冷光腺×1` · 金 170
- **海沟前哨**（z5）：① 凿锚位 `铝质结核×3` · 金 80 → ② 架落脚台/系绳 `银质结核×1` · 金 120 → ③ 通电点亮 `冷光腺×1` ＋ `金质结核×1` · 金 180

**分段逻辑**：①② 结构档吃金属结核（越深金属越贵 铜→铁→铝→银→金）；③ 通电点亮恒吃 `冷光腺×1`；海沟前哨把银/金留到 ②③ 当深水精贵件/导体（银＝系绳精件·金＝300 m 导体点灯·呼应「越深越贵」）。每座合计与前述金属梯一致（残骸 铜2 / 中层 铜2铁2 / 热液 铁2铝2 / 海沟 铝3银1金1 · 各 +冷光腺1）。金额占位待作者调（[[defer-number-tuning]]）。
...
**待确认（fresh session 开工前）**：~~① 金币~~ **保留**（作者拍·上表已含·金额占位待调）；~~② 建造分段~~ **3 阶段**（作者拍·已摊入上表）；③ 蛛蟹甲壳去留（退出前哨料·制氧机仍用）；④ 海沟区(z5) 自身产不产料。**实现属经济重做（#294 同摊·deep_token 无 sink / lootFactor 删 等一并看）·非本 obs-station 结构改·建议 fresh session 一起做。**
