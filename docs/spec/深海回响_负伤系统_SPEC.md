...
> 定调（作者拍·2026-06-12）：**死亡螺旋是意图**——潜水本来就该一步错步步错；耗氧增加~~、理智易流失~~〔理智轴已删·« 2026-07-10 »〕都是特定伤害的副作用。伤有轻重：轻时效果小甚至无效（免费警告），重时极易导向死亡、或「瘫痪后等死」的绝望。
> 两条设计底线：① **身体面板默认诚实**~~，理智是它的保险丝~~（2026-06-12 修订：欺骗轴留给海——但第二章「精神错乱」可以干扰你对自身伤势的感知，见 §6.3/§9；骗你的从来不是 UI，是你的脑子，引擎与 baseline 永远持有真值）—— « 2026-07-10 理智系统移除 »：理智保险丝作废，**面板现无例外全诚实**（欺骗轴 TODO 见顶部 banner）；② **绝望来自看清算式不成立，而不是来自没有输入**——重伤收缩行动集为一组烂选项，绝不清空。
...
5. **信息默认全透明**：伤势档位、生效效果、治疗路径三件套全程可见。螺旋越狠，面板越诚实，死亡才是「我早该上浮」而不是「这什么玩意儿」。~~唯一例外是理智保险丝（§6.3/§9）：失真必须可归因——低理智 tells 在场时玩家学到的是「别信」，而不是「被坑」。~~ « 2026-07-10 理智系统移除 »：理智保险丝作废→**面板现无例外全诚实**。
...
export interface InjuryTierEffects {
  ...
  // sanityTakenMult?: number; // « 2026-07-10 理智系统移除 »：已删（理智承伤 × 无载体）
...
默认派生表（`cause` 字段）：physical→流血 · ~~sanity→惊惧~~〔作废·« 2026-07-10 理智系统移除 »〕 · electricity→麻痹 · fire→灼伤。
...
export interface DerivedModifiers {
  ...
  // sanityTakenMult: number; // « 2026-07-10 理智系统移除 »：已删
...
| ~~敌攻理智结算~~（作废） | ~~`engine/combat.ts`（sanityDamage 处）~~ | ~~`sd × sanityTakenMult`~~ —— « 2026-07-10 理智系统移除 »：消费点删除 |
...
| ~~惊惧 `injury.dread`~~（作废） | ~~sanity~~ | ~~无效果~~ | ~~**sanityTakenMult ×1.5**（越深事件越密，自然复利）~~ | ~~none~~ —— « 2026-07-10 理智系统移除 »：`cause: sanity` 与 sanityTakenMult 双删·惊惧整伤种作废 |
...
| 第二章 | **恶心的环境和精神损伤**（核心底色=恶心人）：精神错乱·不良状态·更强更久的毒·水蛭 | physical 威胁缩水，状态机制接棒：毒（新 DamageType `toxin`·长 tick·medkit 难解）、~~惊惧/错乱高发、**理智失真伤势感知**（§9 保险丝）~~〔« 2026-07-10 理智系统移除 »：惊惧/理智失真伤势感知作废·徽章诚实契约不再被烧穿·此拍需另找二章"恶心"主轴〕、水蛭附着（§6.2）。**敌人/环境同一底色**：陷阱、瘴气、毒气、剧毒生物、水蛭式「拔也不是不拔也不是」的两难生物、寄生、产卵、疾病——现有鱼类只是范例，补内容批次再加符合背景的鱼类（喂周末引擎）。**真结局引入大深渊的情报** |
...
- ~~**错乱干扰伤势感知的实装边界**：失真**只住 UI 渲染层**（engine ↛ ui 边界正好兜住，quirk #95）——引擎、存档、baseline 永远是真值；低理智时徽章可错档/漏报/幻报，但失真出现必须与既有低理智 tells 同场（可学习规则：「理智低，别信自己的身体」）。medkit/死亡结算一律按真值走——你以为的轻伤按重伤收你的命，这就是错乱的价格。~~ « 2026-07-10 理智系统移除 »：整条边界作废（低理智 tells 不存在·徽章无失真态）。« TODO(作者)：欺骗轴若保留，换非理智触发 »
...
- ~~**精神类药**：压 `sanityTakenMult`，缓解错乱类问题；能否暂时让失真面板恢复可信 [待拍·§12]。~~ « 2026-07-10 理智系统移除 »：压 sanityTakenMult / 恢复面板可信皆失去载体·本药作废（余止痛剂/肾上腺素两件仍成立）。
...
- ~~**理智保险丝（第二章·§6.3 唯一例外）**：理智低于阈值（或错乱伤在身）时，徽章允许错档/漏报/幻报。失真在 UI 渲染层实现、引擎真值不动，且必须与低理智 tells 同场出现——玩家能学会「什么时候不能信」，这是规则不是坑。~~ « 2026-07-10 理智系统移除 »：理智保险丝作废·徽章无例外全诚实。
...
- [x] `engine/modifiers.ts` + 消费点接线（combat 行动消耗〔availability 同口径〕/~~敌击理智~~〔删·« 2026-07-10 »〕/回合 tick + dive-move〔tickTurns opts + 洋流〕+ 体力上限 `effectiveStaminaMax`）
...
- [ ] 第二章（恶心人批）：`toxin` 伤种（长 tick·medkit 难解）+ ~~错乱伤势感知失真（纯 UI 渲染层 + tells 绑定·文案 [待过稿]）~~〔作废·« 2026-07-10 理智系统移除 »·TODO 见顶部 banner〕+ 水蛭 hazard + ~~精神类药~~〔作废〕+ 敌人/环境 palette（陷阱/瘴气/毒气/剧毒生物/寄生/产卵/疾病·配套鱼类内容批喂周末引擎）
...
3. ~~惊惧×panicked~~ **已拍：暂不叠**——~~惊惧·重已有 ×1.5 自然复利；panicked 联动留二章「精神损伤主轴」与错乱/精神药同场设计。~~ « 2026-07-10 理智系统移除 »：惊惧整伤种作废·本问题无载体。
...
6. ~~错乱失真配比~~ **已拍：留二章错乱批专门设计**——~~现在只锁原则（失真只住 UI 渲染层·引擎真值·与既有低理智 tells 同场·幻报最稀）。~~ « 2026-07-10 理智系统移除 »：错乱失真轴失去理智触发·作废（欺骗轴 TODO 见顶部 banner）。
...
9. ~~精神类药能否暂时恢复面板可信（=用药买回诚实）？若能，它就是二章的「灯」——值得做贵做稀。~~ « 2026-07-10 理智系统移除 »：无失真面板可恢复·本问题作废。
