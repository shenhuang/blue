...
| 第二章 | **恶心的环境和精神损伤**（核心底色=恶心人）：精神错乱·不良状态·更强更久的毒·水蛭 | physical 威胁缩水，状态机制接棒：毒（新 DamageType `toxin`·长 tick·medkit 难解）、惊惧/错乱高发、**理智失真伤势感知**（§9 保险丝）、水蛭附着（§6.2）——徽章的诚实契约在这章被理智烧穿。**敌人/环境同一底色**：陷阱、瘴气、毒气、剧毒生物、水蛭式「拔也不是不拔也不是」的两难生物、寄生、产卵、疾病——现有鱼类只是范例，补内容批次再加符合背景的鱼类（喂周末引擎）。**真结局引入大深渊的情报** |
...
- [ ] 第二章（恶心人批）：`toxin` 伤种（长 tick·medkit 难解）+ 错乱伤势感知失真（纯 UI 渲染层 + tells 绑定·文案 [待过稿]）+ 水蛭 hazard + 精神类药 + 敌人/环境 palette（陷阱/瘴气/毒气/剧毒生物/寄生/产卵/疾病·配套鱼类内容批喂周末引擎）
...
3. ~~惊惧×panicked~~ **已拍：暂不叠**——惊惧·重已有 ×1.5 自然复利；panicked 联动留二章「精神损伤主轴」与错乱/精神药同场设计。
...
6. ~~错乱失真配比~~ **已拍：留二章错乱批专门设计**——现在只锁原则（失真只住 UI 渲染层·引擎真值·与既有低理智 tells 同场·幻报最稀）。
