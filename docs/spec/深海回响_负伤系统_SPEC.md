...
> 定调（作者拍·2026-06-12）：**死亡螺旋是意图**——潜水本来就该一步错步步错；耗氧增加、理智易流失都是特定伤害的副作用。伤有轻重：轻时效果小甚至无效（免费警告），重时极易导向死亡、或「瘫痪后等死」的绝望。
> 两条设计底线：① **身体面板默认诚实，理智是它的保险丝**（2026-06-12 修订：欺骗轴留给海——但第二章「精神错乱」可以干扰你对自身伤势的感知，见 §6.3/§9；骗你的从来不是 UI，是你的脑子，引擎与 baseline 永远持有真值）；② **绝望来自看清算式不成立，而不是来自没有输入**——重伤收缩行动集为一组烂选项，绝不清空。
> 状态：草案待过稿（开放问题 §12 五条留作者）。数值全部是起手值，以 baseline 调。
> 实装进度（2026-06-12·#116）：**Wave 1 机制脊柱 + Wave 2 的 scent 第三通道（§6.1）已实装**——见 §11 勾选；全部叙事/UI 文案为 [待过稿] 草稿（quirk #117 流程），数值（injuryOnHit 0.35/0.4、scent 半线/×1.5、§6 表）为起手值待 baseline 调。
...
5. **信息默认全透明**：伤势档位、生效效果、治疗路径三件套全程可见。螺旋越狠，面板越诚实，死亡才是「我早该上浮」而不是「这什么玩意儿」。唯一例外是理智保险丝（§6.3/§9）：失真必须可归因——低理智 tells 在场时玩家学到的是「别信」，而不是「被坑」。
...
**档位状态机**：无 → 轻 → 重。同伤种再中招升一档；已是重档再中招**不再升级、无额外惩罚**（重档本身已是极值；要不要「重+重=即死/瘫痪」见 §12 开放问题）。不同伤种各自独立计档。超过 3 处上限时新伤顶替最轻的一处？——**不**：超限时新伤直接对「已有最轻伤种」升档（伤上加伤），列表永不超 3 行。
...
默认派生表（`cause` 字段）：physical→流血 · sanity→惊惧 · electricity→麻痹 · fire→灼伤。
...
| 敌攻理智结算 | `engine/combat.ts`（sanityDamage 处） | `sd × sanityTakenMult` |
...
| 惊惧 `injury.dread` | sanity | 无效果 | **sanityTakenMult ×1.5**（越深事件越密，自然复利） | none |
...
| 第二章 | **恶心的环境和精神损伤**（核心底色=恶心人）：精神错乱·不良状态·更强更久的毒·水蛭 | physical 威胁缩水，状态机制接棒：毒（新 DamageType `toxin`·长 tick·medkit 难解）、惊惧/错乱高发、**理智失真伤势感知**（§9 保险丝）、水蛭附着（§6.2）——徽章的诚实契约在这章被理智烧穿。**真结局引入大深渊的情报** |
...
- **错乱干扰伤势感知的实装边界**：失真**只住 UI 渲染层**（engine ↛ ui 边界正好兜住，quirk #95）——引擎、存档、baseline 永远是真值；低理智时徽章可错档/漏报/幻报，但失真出现必须与既有低理智 tells 同场（可学习规则：「理智低，别信自己的身体」）。medkit/死亡结算一律按真值走——你以为的轻伤按重伤收你的命，这就是错乱的价格。
...
- `item.med_kit` 至今故意不上架（见交接备忘）——**负伤系统 Wave 1 上线即其上架契机**，定价/进货随治疗价值重估 [待作者拍]。
...
- **精神类药**：压 `sanityTakenMult`，缓解错乱类问题；能否暂时让失真面板恢复可信 [待拍·§12]。
...
- **理智保险丝（第二章·§6.3 唯一例外）**：理智低于阈值（或错乱伤在身）时，徽章允许错档/漏报/幻报。失真在 UI 渲染层实现、引擎真值不动，且必须与低理智 tells 同场出现——玩家能学会「什么时候不能信」，这是规则不是坑。
...
- [x] `engine/modifiers.ts` + 消费点接线（combat 行动消耗〔availability 同口径〕/敌击理智/回合 tick + dive-move〔tickTurns opts + 洋流〕+ 体力上限 `effectiveStaminaMax`）
...
- [x] scent 第三通道（`enemy.scent` 旁路判定 + spawn ×2〔阈值制翻译＝现身线砍半 `STALKER_SCENT_SPAWN_ALERT_MULT`〕+ patience +50%〔读点折算·止血回常〕·§6.1）——✅ 已实装（#116·鲨/梭鱼起手 scent:true 待作者过 §12.2；战斗侧 unaware→alerted 已接、现役敌种皆 alerted 起步＝暂潜伏；playthrough-stalker §20 含「摸黑/吸声失效但 decoy 仍 guaranteed」断言）
...
- [ ] 第二章（恶心人批）：`toxin` 伤种（长 tick·medkit 难解）+ 错乱伤势感知失真（纯 UI 渲染层 + tells 绑定·文案 [待过稿]）+ 水蛭 hazard + 精神类药
...
1. 重档再中招：维持「无额外惩罚」，还是引入「重+重=瘫痪/即死」？（当前倾向：不引入，瘫痪只属麻痹·重，保持每伤种惩罚形状可预期。）
2. scent 数值（spawn ×2 / patience +50%）是起手拍脑袋值——baseline 调；哪些敌种 `scent: true`（鲨/梭鱼天然候选，逐敌过一遍）。
2b. 轻档流血是否漏味？当前拍：**不漏**（伤口按得住·免费警告设计不破），「彻底止血」=重档被 medkit/回港清除；§6.2 聚怪区域是否连轻档也闻得到，留 baseline 试。
3. 惊惧·重要不要叠一个「panicked 更易触发」的战斗内联动？
4. 瘫痪 + 哨站：爬到哨站算获救吗？（与前哨脊柱/深水区 Phase 2 的交点，本 SPEC 不展开。）
5. 全部新文案（伤势叙事/瘫痪/let_go/错乱失真）走教学关同款过稿流程（quirk #117 红线自查）。
6. 错乱失真的具体形态配比（错档/漏报/幻报各占多少·理智阈值定多少·与哪些既有低理智 tells 绑定）——需作者拍；幻报（显示不存在的伤）最烈，建议最稀。
...
9. 精神类药能否暂时恢复面板可信（=用药买回诚实）？若能，它就是二章的「灯」——值得做贵做稀。
