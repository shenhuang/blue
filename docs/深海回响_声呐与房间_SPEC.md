...
- **低 san 可视化＝墙体闪烁/位移 + 大型 instance 忽隐忽现（作者 2026-06-05，优于「加抖动」）**：低理智不是给图加噪——是**洞壁形状本身开始闪、移、偶尔整片消失**（地图几何本身不可信）＋**大型生物接触忽隐忽现**（一团比你还大的东西闪进闪出、可能根本不在那儿）。这是 §3.2「声呐返回不可信」+ §3.5 mimic + §3.7 另一个世界落到屏上的样子。
