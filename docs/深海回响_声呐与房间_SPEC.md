...
1. **灯塔点亮 → 大扫描揭示动画（回报演出）**：`SeaChartView` 播一记从灯塔铺开的扫描波（暖色 beacon 光，对应微观冷色声呐），**新进入该灯塔范围的 POI 随波到达逐个浮现**（reveal flourish + 名字淡入）。playtest 里资源难攒、点亮那一刻要有**满足感**——这是「终于点亮一座灯」的演出。触发点＝`buildAtLighthouse` / `restoreLighthouse` / `advanceOutpost` 点亮时。**纯 UI 演出**：engine 的 reveal 集合不变，只是把「突然多出来的可见 POI」演成被扫描揭示（原型已演示 sweep + 逐个浮现 + 出范围/潮汐/事件三态）。
