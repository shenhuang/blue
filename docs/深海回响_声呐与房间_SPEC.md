...
> ✅ **两愿望已实装（2026-06-06，STATUS quirk #80）**：① **测绘扫描揭示动画**——`SeaChartView` 灯塔 `.chart-survey-sweep`（慢暖 sweep·3.2s·**key 绑 `builtUpgrades.size`** → 建造/点亮时重挂＝重播＝点亮回报演出·≠ 旋转探照灯·engine reveal 集合不变）+ POI `.chart-poi-arrive` 错峰淡入（按序号 delay＝随波逐个浮现/「尚未扫到」·新 run 的新机会点重挂重播·锚点不重播）。② **POI 不总全揭、随回合变**——新 `chart.ts::chartConditions(profile)`→`SeaChart.conditions{tide,weather}`（确定性 FNV on runsCompleted·**不入存档**·SeaChartView 表头显示「活的海图」）；**浓雾遮蔽一处 roaming 机会点**（确定性挑·随 runsCompleted 变=下次回港又现·**锚点/mimic 永不遮**＝守进度安全 + 海面相对可信）。**未做（留后续）：即时新 POI 浮现**（建灯后新进范围的 POI 现在要下个 run 才进海图——避免在 session 内重洗 roaming 选取；reveal 动画已就位、只待让 chart 在 build 时也重算并 diff 新增）。〔交付：`chartConditions`+`ChartConditions`+`SeaChart.conditions`·`playthrough-chart` §7·smoke L。〕
...
> **进度（2026-06-06）：S0（#71）+ S1（#74）+ S2（#78）+ S3 廉价版（#80）已落地；§6.5 宏观（揭示动画 + 海况遮蔽）已落地（#80）；声呐范围升级轴（§8.1 主升级轴）已接（#80）。仍留：S3 定位 stalker（§8.7）/ §6.5 即时新 POI 浮现 / 定向 ping（§5）/ 房间 feature 数升级。**
