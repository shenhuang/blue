...
>
> **🚧 Phase 2b 留（下个 Phase 2 session）**：① 能源经济（base 层资源，跑设施、决定同时在线数；水上不衰减/水下衰减·水流更快但可水力发电；充电/充氧/中转设施沿 `lighthouse_upgrades` 轨 + 尸体 `aging` 衰减）；② 衰减后果（§3.6：变暗/进度回退/寄存材料丢失）；③ SeaChartView 前哨建造 + 蛙跳出潜点 UI surfacing（2a 建造走 dive 事件、蛙跳收益自动透明）；④ 多前哨链（trench→abyssal 出潜点，按 2a 模板加 OutpostDef + 建造事件）。若 2b 动 profile 持久形状（如能源/衰减计时），再问作者是否破例迁移（quirk #39 流程留备用）。

### Phase 3 — mimic capstone（最深层）
海图假 POI（无灯之光，§3.5）→ 横渡 → 入潜遭遇；tell↔sanity 双耦合（模糊 + 难度，§2.4）；corpse-wearer 姊妹 apex；接 `flag.d_reveal`。**唯一允许的第三只敌人例外**，住终端 zone/海沟。回收浅/中/深伏笔。**必须 Phase 0–2 就位后做；含「另一个世界」（§3.7）的兑现。逐拍演出（怎么引 / 怎么揭 / tell 怎么读 / 那边有什么 / 回报）＝与作者一起一个个敲定（作者 2026-06-02「这个得一起一个个敲定」），不预先写死。**
