...
> **2026-07-23 #332 docs/spec 孤儿扫描收口——删 4 个死/孤儿 spec（#331 同日·Cowork 交互·Opus·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·无新 quirk·commit `4aef797`+docs·push 留 Mac/nightly）**：承 #331 可选残项。37 个 spec 逐个数仓内+记忆引用 + 核已死系统 ⇒ **只 4 个真死/孤儿**：①负伤系统_SPEC（#290 已删系统）②感知门激活_提案草案（非 canon·#264/#315 取代）③move_ui_mock_v2.html（孤儿 mock）④氮气与上浮_SPEC（memory 已标陈旧·以 memory 为唯一源）。保留 氮气系统_SPEC（`src/engine/nitrogen.ts` 代码引用）。删前确认零 markdown-link 引用·`git rm --cached`+mv 出树·删后 check-doc-links + 全量 93/93 绿。余 33 spec 健康。详见 CHANGELOG #332。
>
> **2026-07-23 #331 工作流优化——`regress:sandbox` / `push` 两脚本 + Cowork 记忆整合 53→48 + package.json vite 修正（Cowork 交互·Opus + 记忆整合 subagent·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·新 quirk #274·code commit `6ef1019`+docs·push 留 Mac/nightly）**：起自「分析过去 session 找可优化流程」元请求（采样读 handoff/收尾/功能/nightly transcript + 核脚本/记忆/git）。①`npm run regress:sandbox`（新脚本）把手设 `ESBUILD_BINARY_PATH` 的沙箱回归落成一命令（自动读 esbuild 版本 + `npm pack` 取 linux 二进制缓存进 tmp·不碰作者 node_modules·build 仍跳 #272）·实测全量 93/93 绿。②`npm run push`（Mac 端新脚本）收掉每次收尾固定尾巴＝清 `.git/.sandbox-junk`+strand 锁 → `.deploy-token` 直连 push（token 不打印）→ `git fetch` 刷 ref（#103）·`--dry-run`/`-y`。③`package.json` vite `^5.4.0`→`^8.1.0`（实装 8.1.0 越界·`npm ci` 对 lock 会炸）。④off-repo：Cowork 记忆 53→48（退休 5 个 superseded stub·#284/#300 ⚠ hedge 落成现在时·待 Mac `rm`）。新 quirk #274。**同日续**：修 `npm run push` 落后 origin 时自动 rebase（原版 behind 直接 push 被 non-fast-forward 拒·code `4e75a1e`）。详见 CHANGELOG #331。
>
> **2026-07-23 #330 开阔水域「侧壁/峡谷」档实装——SPEC §6 stub→完整档 + 两轴正交 + 引擎墙几何 + 三门（Cowork 交互·Opus 编排 + Explore + 2 实装 subagent〔render Opus / gates Sonnet·零重叠〕+ 对抗复审 + 加固门 subagent·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·新 quirk #273·code commit `a95c341`+docs·push 留 Mac/nightly）**：承 #328 item2「唯一实质下一步」。三设计点 AskUserQuestion 拍板（两轴正交 / 单侧墙两态 / 墙起伏独立旋钮）→ 实装到底。**两轴**＝底面轴 zoneTag × 侧壁轴 `openWaterWall{side,otherSide,material}`（正交·侧壁非 zoneTag·可组合岩壁+沙/珊瑚/无底断崖）。墙＝`wallInnerX` 单值内面 union 进 `openWaterSdf`·防埋点由 clamp 到节点外包络**构造保证**·floorless slot 峡谷 bake 补渲墙。门＝`check-openwater-wall`(schema+8单测) + `playthrough-openwater-wall`(埋点/不夹断/**taper 形状锁**/**floorless bake 锁**/负控·169 图)。**对抗复审逮到 taper+ripple 失活**（clamp 反向吃掉·墙成竖直板·埋点门查不出）→ 翻符号成 wide-top V/U（channel 250→374·quirk #273）+ 加形状门锁。**手感数值全 defer**（`?dev&scene=openwater_{canyon,cliff,shelf,slot}`）。详见 CHANGELOG #330。
## 1. 一句话状态
