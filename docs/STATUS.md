...
> **2026-06-14 探深「深度柱」§10 定案实现 + 海图/UI 收尾（交互 session〔含作者并发〕·#131·接 #130·已 commit）**：把 #130 占位换成作者 §10 定案——级数/深度（中层↔海沟 4/6→**6/4**·家2/残骸3/中层6/热液4/海沟4）、海沟 t4=**科考站电梯 capstone**（新 item `item.station_module` 料门·`setsFlag=story.ch1.station_found`·band[270,310]·名义360m）、**删** abyssal/hadal/subhadal/nameless 4 band+4 事件文件+111 scenarios（`depth_bands.json` 现空表·全 band 柱派生）。海图收尾（线上 playtest）：海沟竖井 t1-t3 `noPoi`（只露电梯一个潜点·删密集簇）·标记/文字 **counter-scale** 恒定大小不随 zoom 放大（`--chart-cscale`）·`MIN_VIEW_SPAN`0.55→0.18·电梯 POI`{0.86,0.44}`在前哨圈内·**科考站圈暂不加**（flag hook 已接·作者「以后再说」）。改名「探深」→「低频声呐」+ UI 文案 in-world 化。**SAVE 5→6**。`npm run regress` **34/34 全绿+build**。同期作者 **#132 港口**（右栏对话自动关闭 + 左右栏宽度不变量·quirk #132）。SPEC §10 标「已实现」。**遗留**：占位数值（tier-1 顶深/账单/POI 坐标）+ 科考站区/电梯剧情 + St1 四锚点 + 鲸落 step②-⑤ + 周末 schedule 恢复待作者。详见 CHANGELOG #131/#132。
