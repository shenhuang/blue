...
> **2026-07-09 Voss「头足目观察记录」收集录音系列——档案 147/156 实装 + 全系列设计存稿（Cowork 交互·Opus·#278·内容/数据+设计文档·零代码·不 bump SAVE·`regress --skip build` 95/95·未提交待 nightly）**：全仓第一个 `kind:'journal'` 收集分册（group 头足目观察记录）。载体＝录音（防水录音笔·章鱼窝里播放·非纸/手记）·标题 `档案 <编号>`（大数非连号）。实装 2 条：档案 147（逃脱·战术欺骗·`bluecaves.soaked_notebook`「窝里的录音笔」+baseline）+ 档案 156（伪装未逃·活鲸分心·`wreck_graveyard.jarred_page`「铁肋间的录音笔」+baseline·取代旧「认得观察者」150·旧概念存 doc 备用）。全系列设计落 `docs/spec/深海回响_观察手记_档案系列.md`：163 夺卵起家（鲸落 Ch1 隐藏区）/ 168 离渊变笨·装钝·留站=钥匙（Ch2）/ 甲乙丙宠物阶段 / Aldo-Otto NPC 问询链（Ch1 可上）/ 寂寞彩蛋（感情比聪明活得久）。暗脊：章鱼比人聪明·留下是选择·世界低估玩家知真；守不交底（生物学层·与主线导师磁带录音区隔）。纯 node 门 10/10 + `regress --skip build` 95/95 绿（build 留 nightly·#147）。#277 曾误当并发 WIP 隔离·其实即本条。树：main·未提交待 nightly。
> **2026-07-09 Théo 研究课题 SPEC 回填（§9 决议落地）+ 主角语调 §2 对齐新框架（Cowork 交互·Opus·#277·纯 canon 文档·零代码·不 bump SAVE·沙箱 regress --skip build 95/95·未提交待 nightly）**：接 #267 结转「§9 决议已拍·只落地」。① 科考队 SPEC §Théo 课题定**主线 B（人类深度极限的物理边界）＋ A/C 降卫星**（「一句核心＋旁及」笔法·§待对账加第 8 条）+ 死亡段补 1:1 反讽 +「课题残稿」块（受控深层缝载体）；② 剧情 SPEC §4.1 加「受控深层缝」块（Ch1 结尾对大深渊透半档·载体＝Théo 死亡日志 × 导师磁带 `recording_1` 接地/反调对位·三守不点破水鬼/不坐实队友→水鬼通则/不露古文明＝头足类）+ §5 追加规则 11（受控深层缝＝规约 8「永不交底」唯一在册例外）；③ 古文明 SPEC §7 加 Théo＝派系前中立第一位水鬼·「看不出来向」在人类侧最早最纯实例；④ 顺带把 §2 主角语调从旧「冷静寡言」对齐到 `protagonist-voice` memory「玩家投影·三条硬禁（不写情绪/不定义性格/不提性别）」现行框架（保留 `check-protagonist-voice` 门 quirk #184）。决议源＝`深海回响_Théo研究课题_分析讨论稿.md` §9（2026-07-06 拍）。沙箱 `regress --skip build` 95/95 绿。⚠ 树内有非本 session 未提交 WIP（`blue_caves.json`/`lore.json` 05:15 活跃·scenario·fuse 垃圾）→ 显式只 add 自己 5 文档、pathspec 隔离。树：main·未提交待 nightly。
> **2026-07-08 The Warren 接线收尾：到达路由 + 封口墙 + 每间存卵 + SPEC canon 改写（Cowork 交互·Opus·#276·同日续·全 additive·不 bump SAVE·沙箱 regress 95/95·未提交待 nightly）**：接 #275 三卵室骨架把消费侧全接上——`dive-move` 到达路由（`warrenArrivalEncounterId` 纯决策 + `buildWarrenArrival` 组装：封口墙/空卵室/女王阶段三选一）+ 封口墙 party（`warren_wall_spawn`/`warren_wall_guards`·`CombatState.warrenWall`·打穿 `finalizeVictory→applyWarrenVictory` 置 `wallDown`）+ 每间卵室存卵 `warrenHunt.eggs`（提前凿卵削 §15.1 库存·撤退随机⇒预清一间 50% 赌注·清空/撤离置 0）+ `ensureQueenPlaced` 接 `dive-start`（POI 经 `startDiveFromPoi` 汇流）+ 密度接遭遇构造 + 占位 `zone.warren`/POI anchor。`startCombat` 加成员级 `metamorphosisStage` 覆写（注卵 cocoon）+ `warrenWall` 传播；warren 追猎态/路由决策纯逻辑归 `warren-hunt.ts`（`WARREN_LAST_STAND_ROOMS`/`isWarrenLastStand` 移来·单一真相），getEncounter 组装/胜利回写归 `combat-warren.ts`（守 file-budget·combat.ts 1160）。**SPEC canon 改写**：顶部三卵室 canon 覆盖 banner + §4/§5/§8 死角=状态 + §7「越近核心水越稠」失载体降氛围 + §13④/§14③ deferred 更新 + 新 §17 + boss_enemy_design memory 同步。新 baseline：`playthrough-warren-mapgen` E 段 + 3 combat scenario·既有 47 combat baseline 零重录。沙箱 `regress --skip build` 95/95 绿（device npm 403→云容器 npm pack esbuild ship 回 device·见 [[blue_regress_sandbox]]；build 缺 rollup 留 nightly·#147）。树：main·未提交待 nightly。
