...
> **2026-06-10 猎手 Phase 2 收束 + ①根治版徽章 + dev 测试货架（交互 session·#109）**：作者拍板「猎手剩余全做完、和 decoy 一起完整验收」——**§5 钻狭缝**（「容得下多大」与声呐图房间大小**同源**：`roomScale01/nodeIsNarrow` 迁居 `engine/sonar.ts`·面板反向 import·渲染零变化·玩家看图即读窄缝；大型寻路绕窄 + 现身点占位过滤 + 你在窄缝里贴邻不接触、它守「口外」）+ **§6 执着等待者**（`patience` 只管有信号围守·`guardedTurns` 超了 → `gaveUp` 放弃离开·丢信号仍走既有 wait/seek·章鱼=10 执着）+ **§2.2 per-encounter 档案 + active 探测**（`CombatEncounterDef.stalker` 打标签不加敌：盲鳗 sound+active/章鱼 both/梭鱼 light/石斑 sound 慢速；active＝searching 每 3 回合自主探测 3 跳内·T2 迷彩可规避 `playerEvadesProbe` 守地板·摸黑不再万灵）+ **Q3 浅水弱变体**（`zones.json weakHunts` opt-in〔旧灯塔礁+蓝洞群〕·浅水线下哈希 1/10 现身·**信号直读灯/声呐开关**＝§7.5 alert 免压不破·硬性「小且弱」0.55 慢速 wait 不 large/active）。**①根治版**：157 处 label 标注剥离（纯 fiction）+ EventView `.check-badge` 从 `check.{stat,dc}` 渲染（SSR 实测「体力 13」）+ `hideCheck` 保 8 处隐藏判定设计权 + lint 翻面禁回潮（负路径红）。**Mira dev 测试货架**：`?dev` 门后 0 元白拿全部道具（`devGrantItem` 不动金币/备货·`DEV_TOOLS` 迁居 `ui/devMode.ts`）。Phase 3 §3.7 作者拍「留专门 session」。全 run 级可选字段不 bump SAVE_VERSION。`playthrough-stalker` 新 §11-§14·§1-§10 零翻修·**28/28** 全绿 + prod build。新 quirk #108/#109。详见 CHANGELOG #109 + 猎手 SPEC v0.2（机制层完工）。
