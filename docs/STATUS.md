...
> **2026-06-06 声呐与房间 SPEC 收尾 · 三 residual 清零 · SPEC 关闭（方向「释放·closeout」·作者『做完能释放掉文件/流程的先做、直到全部完成』→ 全部 3 项·#90）**：把声呐与房间 SPEC 仅剩三项 residual 全做完、**SPEC 彻底完成关闭**（唯一相关后续＝猎手 Phase 2+ 在 `docs/spec/深海回响_猎手_SPEC.md` §7·与本 SPEC 无关）。① **聚焦扇区可视化**——此前定向 ping 只有文字标注，现 `SonarScanPanel` 据 `run.sensors.sonarDir` 画聚焦扇区楔形 `sonar-focus-wedge`（`focusWedgePath` 纯几何·deeper 朝右/back 朝左/lateral 上下两瓣·略超量程环＝直观「那一向探更远」·暖色 warn 与冷色真图一眼可分）·纯渲染·smoke E4b。② **各方向 reach 各自升级（§5）**——新 `UpgradeEffect{kind:'sonarDirReachBonus',dir,value}`（带 `dir` 判别·**首个非标量旋钮**·聚合进 `Record<SonarDir,number>`）沿同款传感器升级桥（`UpgradeEffect→getUpgradeBonuses→UpgradeBonuses→getRunBonuses→createNewRun→deriveSensorTuning`·逐向夹 `[0,SONAR_DIR_REACH_MAX]`2）落 `SensorTuning.sonarDirReach`；`revealSonarScanDirectional` 加第 5 参 `dirReach`（聚焦焦距 `min(SONAR_DIR_RANGE_MAX+reach, base+FOCUS+reach)`·**近场/别向不延长＝守北极星「再聚焦也扫不穿整洞」**）·`sonarDirReach(run,dir)` 读·`pingSonar` 透传；data `sonar_rig` lv6/7/8 三向各 +1（**定向仍免费·reach 才是 power-up**）。**缺省全 0 → 定向揭示逐字节不变·不 bump SAVE_VERSION**（record 纯对象 JSON round-trip）。③ **§5 later 接触带大小 + 开放水域扫描**——`Stalker.large?`（spawn `depth≥STALKER_LARGE_DEPTH`108m·缺省小 blip）→ `stalkerSonarBlip` 透传 → 画弥散质量 `sonar-stalker-mass` 一大团；开放水域（层状 zone·`!zoneAllowsBacktrack`）声呐图不画洞壁通道边（`sonar-edge`）、只显接触读数（标 `is-open-water`）·迷路洞穴仍画通道（纯渲染·引擎早不门控 zone）。新 quirk #91（`.sonar-stalker circle` 通用规则压过嵌套 mass 的 CSS 特异性陷阱·用 `.sonar-stalker circle.sonar-stalker-mass` 压回）。`playthrough-sonar` §13(i/j)·`-upgrades` §11·`-stalker` §2b·`-save` round-trip·smoke E4b/E4c/E8/J9·全绿 26/26 + prod build·提交 `e55187d`（feat）+ 本 docs commit。详见 CHANGELOG #90 + 声呐与房间 SPEC §7（关闭块）/§11。
