...
> **2026-07-12 #298 敌人词条（affix）系统试点——同种敌人随机单词条·5 词条·resolveDodge 单点（Cowork 交互·Opus 设计+Sonnet subagent 实装·代码+数据+UI·`npm run regress` 90/90 全绿〔沙箱 esbuild·prod build 留 Mac/nightly〕·新 quirk #248·未提交待作者/nightly）**：作者要「同种敌人可带不同词条」。5 词条＝狂暴（每回合两击）/灵巧〔概率闪避·**必中的显式例外**·仅词条内局部 roll〕/硬壳（防御×倍率）/自愈（回合初回血%）/剧毒（挂 poisoned）·彩色标签显于敌名后·词条**初始随机单条**（作者定·未来按 tier 提 count）。单一真相＝`data/affixes.json`（id/中文名/色）+ `engine/affixes.ts`（效果占位常量+`rollAffixes` 无放回抽样〔天然去重〕+`resolveDodge` 单点·对称·状态/装备 OR 来源留口）；`EnemyDef.affixes?/randomAffixes?{count,pool?}`+`EnemyPartyMemberDef.affixesOverride?`〔encounter/scenario 钉死·确定性测试〕+`EnemyInstance.affixes?`〔运行时副本·二期夺取在此 mutate〕（全加法可选·#99·**非词条敌零 RNG 逐字节不变**）。`startCombat` 按 override>random>fixed 注入·6 钩子读 `inst.affixes`（狂暴二击/灵巧闪避归零+短路 status/硬壳防御×/自愈回合初回血/剧毒挂 poisoned·berserk/regen/venom 抽进新 `combat-affixes.ts`·resolveDodge 入 affixes.ts）。check-boundaries **规则九** 守 affixes/randomAffixes.pool/affixesOverride 的 id 合法+去重+count∈[1,池大小]（防灵巧自叠永不命中）。试点敌 `猩红弑亲者`（鱿鱼/头足类·`randomAffixes:{count:1}`·幼体无色→吞噬得色·头足类≠古文明由作者拍）+ `combat.scarlet_kinslayer_solo` encounter〔member 钉 berserk〕+ baseline scenario（victory/turns4/HP61）。文件预算超（combat.ts 1201>1164）→照 check-file-budget 指引**抽取而非 bump baseline**→回落 1159。遗留：数值全占位待调（defer-number-tuning）·二期「攻击一击可秒的友方→夺其词条」需先有 ally 战斗单位（现队友是叙事非战斗单位·机制=corpseEating 推广）·tier→count 映射待作。树：main·commit 待本次收尾·未 push 待 nightly。
> **2026-07-12 #297 观测站重构方向定案 + 地理错位审计（Cowork 交互·Opus·纯设计+只读审计·零代码·spec 落 `docs/spec/深海回响_观测站重构_SPEC.md`）**：审计发现 Ch1 结局（观测站·找到导师）叙事本在海沟（`zone.vent_trench`＝「海沟热液场」）却挂在海图「热液区」标签下，而「海沟区(z5·最右最深)」标签空占蓝洞群（横向浅岸洞穴·非深海沟）；`story.ch1.trench_found` 无 setter ⇒ 海沟前哨不可达（P1-8）；`check-mainline-reachable` 不校验 chart POI 的 requiresFlags 有无 setter（cave_chart_page/warren/trench_found orphan·regress 绿不报）。作者拍：观测站锚深海沟·经海沟前哨建造可达·无深度升级；四鱼锚点→四 boss（作者在做·Warren 已成·#2 在做）；整条观测站剧情重构含地理正名。深度升级压缩整块归 Ch2（消耗式专属图纸料门·零引擎改动）。零代码改动 ⇒ regress 绿态延续 #296 的 91/91。下个 fresh session 专做重构·开工前先拍地理 A/C + 给叙事骨架（spec §4/§6）。
> **2026-07-12 #296 战斗状态系统实装——玩家↔敌对称·独立实例堆叠·布尔免疫（Cowork 交互·Sonnet·代码·`npm run regress` 91/91 全绿〔含 prod build〕·新 quirk #246/#247·commit `909b013`）**：源真 `docs/spec/深海回响_战斗状态系统_SPEC.md`（#295 撞见的那份并发中的 SPEC）。按 SPEC §7 落地：新 `types/status.ts`/`engine/status.ts` 单一源（`StatusInstance`/`StatusKind`/`settleStatusesAtTurnStart` 纯函数+单测·off-by-one 顺序写死）；`EnemyDef.statusImmunity`/`EnemyAttack.applyStatusOnHit` 新增；敌人侧 DoT/decay 从回合末挪到回合开头结算、施加改纯堆叠（去 filter 刷新）；玩家侧 `CombatState.playerStatuses` + `applyPlayerAction` 顶部结算（stunned＝挣扎无效）；三处施加点接布尔免疫。recon 发现 SPEC 建议的 psm 三车道并行会撞同一批 `combat.ts` 函数，改一次性顺序实现。**提交前 6 角度对抗性代码审查**抓到并修复 5 个真 bug：玩家侧 DoT 致死未做门控（4/6 agent 独立命中）、`CombatState.playerStatuses` 必填但存档 hydrate 从不碰 `phase`（同版本旧档 mid-combat resume 会崩·`?? []` 兜底修）、`runEnemyTurn` 循环内结算钩子改写状态后决策分支仍读过期快照、两处施加点缺 `hp>0` 门、三处重复免疫判断收成单点。`bless:combat` 43/43 无变化（现有内容未接入新机制，纯单测覆盖，待内容接入端到端验证）。树：main·commit `909b013`（feat）+ 本条 docs·ahead·未 push 待 nightly。
