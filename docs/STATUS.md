...
> **2026-06-14 worktree 多 feature 并行工作流并入流程（交互 session·#129·机制+约定·非游戏改动）**：讨论「并行开发两 feature 该不该开分支」→ 结论：单人 + 强 regress 门 + 沙箱 mount 不能 unlink（#104）下默认 trunk-based + flag 门控；真并行才用 `git worktree`（绕开「单树切含新增文件的分支切不回 main」）。作者拍「实装」→落成机制：①`scripts/handoff.mjs` 加 **1c** 块 `git worktree list`（`--no-optional-locks`·>1 棵树才打印·单树零噪音·git 失败安全跳过）＝并行线由 handoff 从 git 自报（守 #96 定位轴）；②新 **quirk #130**（worktree 工作流 + 两红线：append-only 文档〔CHANGELOG/QUIRKS〕只在 main 整合时写·收尾次序 feature→merge→main 上 closeout；每树独立 node_modules；worktree 只消树互污染/挑文件、真防同文件打架靠解耦 #95/#116）；③`CLAUDE.md` 并发隔离段加 worktree 指针（gitignore·本机参考）。**regress 全绿**：非-build 32 任务 + prod build＝**33/33**（沙箱 esbuild 双版本 tsx0.28/vite0.21 + linux rollup native·分两次·见 [[blue-regress-sandbox]]）·无 flake。零 TS/源改（只动 handoff.mjs〔不在 build/test 路径〕+ 文档）。**树**：main 本地 [ahead 2 待推]（#128+本条·沙箱无凭证·留作者 push 后 fetch·#103）。详见 CHANGELOG #129。
