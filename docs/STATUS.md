...
- [x] **敌人库（Enemy Library）· 基础已实装（2026-06-14）·余项待续** —— 把敌人从"散文件 + `combat.ts` 手动 import + 无元数据"升级成**可查询 / 可复用 / 目录自动加载 / 可被独立 session 安全产出**的库。两原则：**数据驱动默认**（纯数据敌人＝零引擎改动·只有新机制原语才碰引擎·边界表 SPEC §3）+ **可维护/可扩展优先**（元数据全可选 additive·不入存档）。**已落地**：支柱三 目录自动加载（`scripts/gen-enemy-registry.mjs` → `src/data/enemies/registry.generated.ts`·双运行时安全·改 JSON 后 `npm run gen:enemies`；combat.ts 删 7 手动 import）+ 支柱一 元数据 `bands/biomes/role/threatTier/codex` 回填 9 只 + `engine/enemyLibrary.ts::pickEnemy`/`matchEnemies`/`enemyThreatTier` + `scripts/check-enemy-refs.mjs` **四门入 regress**（registry 不过期 / 引用完整 / 无孤儿 / 有 baseline）+ `combat-runner --list-enemies --band/--biome/--role/--threat-tier` 过滤 + 回归 `playthrough-enemy-library`。**regress 29→31 全绿**。约定全做成 check＝**无新 quirk**。**待续**：① 入库契约脚手架 `combat-runner --new-enemy`；② 支柱二 def·encounter 解耦 `enemyRef`（route A 迁移 vs B 双轨·SPEC §8③·需 caller）；③ §5 两条工作流（描述→实装 / 定时无人值守生成）的 SKILL（B 的 schedule 作者设立时撰）；④ codex 现简短占位·图鉴 UI 留后。设计/进度源 `docs/spec/深海回响_敌人库_SPEC.md`（§3 边界 / §5 工作流 / §7 清单 / §8 开放问题）+ CHANGELOG #121。（设计 2026-06-13·实装 2026-06-14·「按顺序实装」）
