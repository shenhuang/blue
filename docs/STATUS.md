...
> **2026-06-10 猎手 §4 Decoy + 出发前选带（交互 session·#108）**：作者拍的主方向落地——**声诱标/光诱棒**两种 decoy 道具（`ItemDef.decoy.kind` data-driven）：**潜内投放**（`deployDecoy`·烧一枚落当前节点·`DECOY_TURNS`6 回合·不耗回合）把感官匹配的猎手引向诱饵点（`advanceStalker` decoy 分支**优先于真信号**·声↔声/光↔光/双感任一都上钩·lastSignal 刷成诱饵点＝失效后它在那附近搜/等、你借机反向拉开；感官不合/过期＝逐字节不变·确定性零 RNG）；**战斗内必成脱战**（`FleeEffect.guaranteed` 跳掷骰·北极星「decoy 永远是出路」·物品消耗收口 applyPlayerAction 1b·没货的道具行动不再上清单）；**Mira 消耗品货架**（×2/回港·同一套加价限量·med_kit 故意不上架）；**出发前选带**（作者拍板「不全带·死了就没」）＝`applyCarryItems`（只认 consumable·夹库存·容量截断）+ SeaChartView「行前装包」面板·死亡进尸体快照/生还自动归库全走既有闭环。`run.decoy` 真条件字段不 bump SAVE_VERSION。回归：`playthrough-stalker` §10 + `-save` round-trip + `-economy` Phase 6 + smoke K1b/P。`npm run regress` **28/28** 全绿（12.2s）+ prod build。新 quirk #107（消耗品入 run 只此一条路 + flee 子串判定）。详见 CHANGELOG #108。猎手 SPEC 剩余 deferred：§5 狭缝避难/§6 执着等待者/per-encounter sensesBy/浅水弱变体。
