...
> **2026-07-04 修 dev「解锁大区」漏置链尾区自身 beat（热液 `anchor.vent` 永不置→深层 Sela 点/beat-gated 内容不现）·`devUnlockChapterRegion` 从深度柱 `beatFlag` 单源代置本区自身 beat（Cowork 交互·Opus·#256）**：作者报「dev·解锁大区·全部」给热液区置了前哨阶段 flag 却漏 `story.ch1.anchor.vent`，去测热液区深层 Sela 点不现、得手动补 flag。根因：四主线区前哨 `requiresFlag` 是**上一区** beat（链式 build-gate·#198），`devUnlockChapterRegion` 只置它、从不置本区自身 beat；reef/wreck/midwater 靠「本区 beat＝下游区 requiresFlag」被链间接补齐，唯 **vent 是链尾**（`col.vent.storyTier.chainTail`·`anchor.vent` 不是任何前哨的 requiresFlag）谁都不置（单区解锁按钮同根因 off-by-one·被「全部」掩盖）。修法＝加纯查询 `columnBeatFlagForLighthouse`（深度柱 `beatFlag` 单源）·devUnlock 额外置本区自身 beat·单/全部同源；`playthrough-outpost §7d` 翻转旧断言 + 新 §7d′ 逐座核（显式覆盖 vent 链尾）把漏置焊成会红门。全量 `npm run regress` **90/90 全绿**（沙箱分块·build 留 Mac/nightly·#147）·不 bump SAVE·新 quirk #217。**树**：main·3 文件 + 三件套待单次提交·ahead 8 待推。
