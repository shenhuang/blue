...
> **2026-06-11 洞型谱 depthCurve + 平廊 POI「横岩廊」+ 洞型情报上海图（交互 session·#113）**：作者拍「洞穴要更多样：深的/广的/广中藏深的·以后可发散」——迷路图深度赋值升级 `depth = d0 + span·frac^k`（`generateMazeMap` **唯一曲线求值点**·k<1 井+廊 / k=1 旧线性 / k>1 廊+坑「广中藏深」）：`ZoneDef.depthCurveRange`（蓝洞群 [0.45,2.6]）+ `GenOpts.depthCurve` 显式覆盖，实潜按 FNV(zone::seedKey::curve) 派生＝**每个海图洞口固定洞型性格**（零 rng·**缺省/无 seedKey → k=1 逐字节复现旧图**·既有 baseline 全不动）。`PoiModifier` 扩成 **GenOpts 薄投影**（depthRange/layerCount/depthCurve 直通·`GenOpts.layerCount` 图规模旋钮）→ 海图新 anchor `poi.anchor.flat_gallery`「横岩廊」（蓝洞群第二口·(0.44,0.28) reach 1·[16,30] 窄 span+layerCount 10+钉死 k2.4·N 20-24·威胁换轴「**进来太远**」回程预算·rule of thirds 进 blurb）。**洞型情报**：`chart.ts::describeCaveShape`（往下掉/斜着下/往里钻）与 mapgen **同一 k 来源**（`caveDepthCurveForPlace`/`caveShapeBucket`·图上写的＝潜下去的＝海图诚实轴·说谎钩子留参数位）·SeaChartView 一枚 chart-tag chip。回归：`analyzeMap.meanDepthFrac` + 洞型谱不变量块（三档单调/极端 k 守迷路+#92/缺省=k1 指纹/POI 性格极差）+ 3 份 k 锚点 + `flat_gallery_poi` scenario + chart §5b/§5c（锚点计数 4→5·reach 边界紧 quirk #115）。另起手分账落库上批尾账 `30d2fc6`。quirk **#114/#115**。commit `e9ad187` + 收尾 commit。全量 **28/28** 全绿。**同日续（线上验收反馈四件·quirk #116）**：扫描黑场修复（重挂载拿本帧烤图打底＝测绘记忆不蒸发）+ 猎手红点出墙修复（`edgeRoutePts`/`stalkerRoutePoint` 渲染同源路由+残段截断·smoke R 节）+ Mira dev 货架改 ItemCell 格子（0 元∞·上移不再垫底）+ UpgradePanel 一键升满。详见 CHANGELOG #113。
> **2026-06-10 晚场（补账）**：#111 PanelShell 统一壳/滚动白名单/海图 1:1（代码 06-11 落 `30d2fc6`）+ #112 声呐图实测四修（`8fdbdf3`）/ 猎手原地同拍（`f31b720`）——详记已补进 CHANGELOG #111/#112；连同 #109/#110 一起等作者合并验收（动线见交接 prompt）。
> **2026-06-10 验收支援·UX 四连 + wreck 档案 + 弱变体「更小敌」（交互 session·#110）**：作者边线上验收边下需求、拍「先全做完晚点一起验收」——**(1) dev 测试解锁**：UpgradePanel `?dev` 门后「测试解锁（0 成本）」（`devUnlockUpgrade` 不查不扣·devGrantItem 同口径·quirk #110 dev 后门三条）。**(2) 行前装包格子化**：背包格上限可见（`carryCapacityFor`＝8+extraConsumableSlot·**与 applyCarryItems 截断线同源**·空格虚框）+ 储物柜格点击互转；新通用 `ui/ItemCell.tsx`（图标空占位 data-cat 钩子·cellKey 重挂载＝item-pop 反馈动画）。**(3) Mira 交易系统**：上=她的货格（点击买·售罄禁点·钱不够红字差额不再静默）/ 下=你的储物柜格（可卖标收价点击卖·她不收惰性陈列带原因·全卖保留）·金币/柜格跳动反馈。**(4) wreck 双敌档案**（待办 2a）：蛛蟹 `sound/0.5/size:'large' 钉死/patience 8`＝**浅段守口外**（窄缝避难 18-50m 即可学·首个浅段 large）；沉灯 `light/active/0.45`＋光感 probe tell「不属于你的灯」；**修 active 奇数槽死字段**（waitTurns 例外抬到 WAIT_TURNS·既有数据零变化·quirk #111 承重关系 WAIT≥PROBE_PERIOD）。**(5) 弱变体专属「更小敌」**（待办 2b·作者拍）：梭鱼幼体/盲鳗幼体（hp7/6·threat2·territorial 挨打会撤·感官同亲代·入亲代 JSON 零 registrar）+ `ZoneDef.weakHuntEncounters` 专属池（缺省回落 ambushEncounters）。敌人 7→9·combat baseline 9→11。`playthrough-stalker` 新 §15/§16·smoke K/P 新合同。六 feat commit（e78014b/834b987/5e4dfce/43ba7d9〔作者并行的修缮面板布局〕/eebbc2d/29e5b6d）全部已 push 上线·每 push 前 **28/28** 全绿。遗留：作者**合并验收**（猎手全家桶+decoy+三 UX+更小敌+wreck 档案）→ tunables 手感反馈；①徽章过目；Phase 3 §3.7 留专门 session（作者 pin）。详见 CHANGELOG #110。
