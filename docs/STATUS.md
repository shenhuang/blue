...
> **2026-06-15/16 海图编辑器（?editor dev 工具）+ chart_pois mapId-keyed + home→数据 + contentBox 动态 fit（owner-anchored 续·交互 session·#136·commit `928a4c8`）**：围绕 #135 做**可视化海图编辑器**让作者拖拽调地图。chart_pois 迁 `mapId`-keyed（5 读者 flatten）+ home 坐标移入 `lighthouse_upgrades.json`（**5 座 beacon 全数据**）。新 `src/ui/MapEditor.tsx`（`?editor`·拖 beacon〔owned POI 整簇跟随〕/POI/半径·滚轮 zoom+背景 pan·撤销·导出）+ `vite.config.ts` dev 中间件（`/__save_chart` 保存进项目 + `/__run_regress` 跑回归）+ `smoke-map-editor` SSR 门。修 `vite.config.js` 影子坑（quirk #134·`tsconfig.node` outDir）。§0b 守门改「owner POI 落 owner radius 内」结构不变量（拖动不失效）。`SeaChartView` contentBox 动态 fit 含揭示圈、右/下不钳死 1（容拖远的灯塔/点·世界右/上下无限·左界=海岸细线）。作者用编辑器调定 ch1 地图布局（前哨坐标/半径/偏移落 3 数据文件）。`npm run regress` **35/35 全绿 + build**。新 quirk **#134**。详见 CHANGELOG #136。
## 1. 一句话状态
