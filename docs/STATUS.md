...
> **2026-06-20 dev 测试货架按 category tab 分类（交互 session·#156·commit `94f5aa1`）**：`MiraShopView` dev 货架从平铺网格改为左侧竖排 tab（装备/消耗品/材料/武器改装/剧情/杂项/货币）+ 右侧过滤格子，与物品栏 `LockerView` 完全同款布局。零 engine/data 改动，复用现有 `.locker-*` 样式。无新 quirk。typecheck + 7 静态门绿。## 1. 一句话状态
