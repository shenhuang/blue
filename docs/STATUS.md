...
> **2026-06-12 负伤系统 Wave 1 + scent 第三通道实装（交互 session·#116·同日教学文案过稿）**：照负伤 SPEC §3–§5/§10–§11——`run.injuries`（轻/重两档·同类升档·上限 3 超限对最轻升档·hydrate 单点补 `[]` 不 bump）+ **写入只走 `engine/injuries.ts` 三入口、数值只走 `engine/modifiers.ts::computeModifiers` 唯一折算点**（check-boundaries **规则四**收口触碰面）；消费点全接线（combat 行动消耗〔availability 同口径·ceil〕/敌攻理智 ×/回合 tick/clamp + dive-move 移动氧耗与洋流 ×/全 clamp 点走 `effectiveStaminaMax`）；`EnemyAttack.injuryOnHit`（鲨咬 0.35→bleed 派生·石斑撞 0.4→rib 显式·**仅带字段的攻击掷骰=既有 seed 基线零漂移**）；StatusBar 伤势徽章三件套（点开浮层·条轨 max 同源）。**scent 第三通道（§6.1）**：嗅觉系敌种（鲨/梭鱼起手·待作者过名单）× 流血·重 → advanceStalker 旁路恒锁定（关灯/闭声呐/吸声/迷彩失效·decoy 分支在前=仍 guaranteed）·守口 patience ×1.5（读点折算）·现身线砍半（`scentSpawnReady`）·startCombat unaware→alerted（现役皆 alerted=暂潜伏）。回归：combat scenarios 11→**16**（5 新实跑抄：proc/升档/重档 tick/氧 ×1.4/一场走完轻→重）+ playthrough-stalker **§20** 八断言（含「摸黑/吸声失效但 decoy 仍出路」）+ save hydrate 断言。**29/29 全绿**。quirk **#119**（injuryOnHit RNG 流纪律/injuries.json 顺序语义/fixture 键/规则四 tripwire）。**同日**：作者整体认可 St0 教学文案 → 剧本 SPEC 三处 [待过稿] 清标（#115 遗留①关账）；拍板下一步=**St1 一章 MVP**、med_kit=St1 起手顺带（接 heal+上架·定价提案制）。负伤叙事/徽章文案仍 [待过稿]、数值全起手值。详见 CHANGELOG #116。
