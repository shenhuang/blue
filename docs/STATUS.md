...
>
> **2026-07-22 #329 `.sonar-edge` 死 CSS 清理 + coords2d「阈值收 1‰」备选收口成已拍板决定（同日续·Cowork 交互·Opus·沙箱全量 regress 91/91 绿〔build 环境跳〕·SAVE 不变·新 quirk #272〔另改写 #271① 措辞〕·commit `b80c315`+`13d28d2`+收尾复核一批·push 留 Mac/nightly）**：起手 handoff 定位＝树净 + #328 已完整收尾 + 无在飞 psm ⇒ **无「待收尾的活」**，只清交接备忘两条残渣。①`.sonar-edge`/`.is-chord` 全仓只剩 styles.css 两块定义（游戏内自 #315 起不画连边·dev 潜点预览走 `.dev-cave-edge`）→ 删 + 墓碑注释写死判据「要在游戏内加连边＝先过『玩家该不该白拿拓扑』这一关，不是捡回这段 CSS」。②阈值**维持 1.5‰、不下压 1‰**：分子仅 2 ⇒ 泊松 95% 区间 0.06‰–1.85‰、1.5‰ 本就在噪声带内而 1‰ 在带下；门是确定性的 ⇒ 误红＝良性改动打乱 seed→图 映射把长桥 2→4（λ=2 时 P(≥4)≈14% vs P(≥6)≈1.7%）；要更灵敏＝**加大 `LONG_BRIDGE_SEEDS` 分母、别挪线**。常量一字未动·纯注释 + quirk #271① 措辞同步·**作者要求此后任何地方都别再把它记成待办**。收尾复核另补 **quirk #272**：③`regress.mjs`「补装 binding 即进门」是未实测的推断且**是错的**——rolldown arm64 binding 装上后 `require()` 即 core dump（不是缺包·是跑不起来）⇒ 沙箱 build 门**不是能补的欠账**，注释/消息已改；④`git fetch` 在沙箱可用（只 push 要凭证）⇒ 幻影 ahead 能当场证伪，本次真值 **ahead 29 / behind 1**（真分叉）。详见 CHANGELOG #329。
## 1. 一句话状态
