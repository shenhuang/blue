...
> **2026-06-27 地图洞穴形态系统：竖/横/蛇三布局（Y=真实深度）+ 持久洞也能横 + 29 洞各配形态（交互〔Cowork·Opus〕·#228·机制并入 quirk #196·regress 66/66 绿〔build 沙箱跳过〕·本地 commit 待 nightly 推）**：作者从地图调试器「洞为何全竖」起。根因＝渲染器只有 y∝深度一种铺法（#92），形状一直没人接。① `LayoutStyle`（vertical/horizontal/serpentine·三者 Y 都=真实深度·作者红线）+ 左右手性·删 radial/spiral（违反位置即深度·渲染成蛛网）。② 两条生成路径都盖章 layoutStyle（dev↔游戏不脱节·持久洞原本只在 dev 见形状）。③ 持久洞也能横：深度带内散布 + 连通树按 index 连（脱钩 layer/深度）→ 房间+隧道 2D 横洞。④ 宽洞 POI ×3·`CAVE_GEOM_MARGIN` 接 dev 烤图。⑤ 29 迷宫洞按 fiction 配形（14 竖/10 横〔orientation:horizontal=进来太远威胁〕/5 蛇）·礁穴↔潮缝拉开撞脸。新 SPEC `深海回响_地图渲染补全`。作者看 29 缩略图全家福「很不错」拍 ship。遗留：T2b 深门未做·横向那 10 座玩法改了留作者调数值·push 待 nightly。---
