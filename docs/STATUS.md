...
> **2026-06-20 Otto 声呐清单去重重做：possession-keyed 修旧档锁死（交互 session·#153·接 #150·手术式只提交本 lane 6 文件·与作者 hasCapability/内容 WIP 并发隔离·quirk #104）**：#150 留的重入叠发清单·先用 flag-keyed 退场（`sonar_hinted`）但作者实测暴露**旧档锁死**（`giveItem` #150 才实装·此前 stub 只 setFlag·旧档 flag 已置无实物→发清单首问被永久藏）；改 key 在**实物持有**——新 `notHasItem` 条件 + `hasItem`/`notHasItem` 港口感知（`ownedQty`=profile+run·`hasItem` 原只查 run·港口恒 false·内容零引用故改语义安全）；Otto 首问 `notHasItem(清单)`→发·重访 `hasItem(清单)`→催料新节点 `otto.sonar_remind`·去死 flag；`playthrough-story §4d` 重写 possession-keyed + 反锁死断言。清单 category `story`→「剧情」tab（#139 单一归属·不挪）。隔离验 **42/42 非 build + build 绿**。新 quirk #143。详见 CHANGELOG #153。
> **2026-06-20 热液探深 capstone 产出「下行核心」+ 大深渊情报 hook（交互 session·#152·接 #131·commit `446261d`·guard `wip/vent-capstone`）**：热液柱最深档（裂口·capstone）`grantsItem` 海沟电梯本就消费的 `item.station_module` ⇒ **必经热液**才下得去深渊路（依赖而非钥匙·零改海沟）；新增通用 `grantsItem` 机制（`buildAtLighthouse`/dev 扣 cost 后 `addToInventory`）+ `VENT_INTEL_FLAG` 情报 hook（Phase 3 消费·未动 `d_reveal`）；落成 `check-dive-refs (l)` capstone 产出/消费闭环门 + `playthrough-columns §10`。regress **43/43 全绿**。详见 CHANGELOG #152。
> **2026-06-20 回归 flaky 根因焊死（交互 session·#151·只收 #129 lane·非 build 42/42 + build 绿·commit 待推）**：quirk #129 查清焊死——`playthrough.ts` 是唯一没种子化的 playthrough（教学潜行检定走 `events.ts::performCheck` 裸 `Math.random()`·~7% 失败偏离上浮路径→`应在上浮，实际 dive` 抛）·顶 `Math.random = makeLcg(PLAYTHROUGH_SEED)` 全程定死（实测 8/120≈6.7% → 0/40）·删 `regress.mjs` 的 `retries:2`（确定性测试不靠重试盖回归·绿门可信）。作者并行 depth-columns/给物 WIP 未提交。详见 CHANGELOG #151。
> **2026-06-20 对话发物 `giveItem` 实装 + Otto 声呐清单实物（交互 session·#150·接 #149·commit `72e85c1`）**：`giveItem` DialogEffect 从 TODO stub 实装＝接 `acquireIntoProfile` 物品入袋单点（quirk #143）；新 `item.note.sonar_checklist`（category `story`·非 #149 prompt 误写的 `misc`〔`ItemCategory` 无 `misc`〕）；`otto.sonar_hint` onEnter 发物；`playthrough-story §4d` 端到端断言（走真引擎守接线）。regress **43/43 全绿**。无新 quirk。遗留＝该提示节点可重入叠加清单（门不变·去重留作者拍）。详见 CHANGELOG #150。
## 1. 一句话状态
