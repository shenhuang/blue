...
> **2026-07-23 #335 收口缓坡 `otherSide:'taper'` 实装——单侧墙敞侧 floor 上收成缓坡封边（#334 同日·Cowork 交互·Opus·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·新 quirk #276·code commit `5952398`+docs·push 留 Mac/nightly）**：承 #330 设计点① / #333 遗留（shelf 夹具 taper 曾是「正常 floor 收边」no-op）。作者「墙就先这样·收口缓坡可以做」＝墙面材质留 rock 不碰、只做收口。`openSideDrop` 从 midwater 专用改成**有符号偏移**——两态镜像同一 `openSideMag`（缓入 quad→线性）·`midwater +下沉`（无底临渊）/ `taper −上收`（floor 升过取景 wy＝缓坡封边）·`openSidePast` 去 midwater 门（纯几何两态共用）；bake 结构窗两态都收口（`otherSide==='midwater'` 放宽到 `side!=='both'`·敞侧裸坡）。可见性：`SonarScanPanel` 潜水盒 taper 往上扩 `boxLoY`（`owFloorBottom` 概览下沿不加 taper 分支·升进既有顶部余量·quirk #276）。门 `playthrough-openwater-wall` 加 ⑧（函数级上收单调 + 渲染级翻转 x 随深度移＝升坡非竖直封边·与 ⑦ 镜像）。`OW_WALL_TAPER`(墙 V/U·#330) 与本条 `otherSide:'taper'`(floor 收口) 同名不同物。数值调 author-present·defer §9。详见 CHANGELOG #335。
>
> **2026-07-23 #334 战斗 dev 面板改造成潜点式战斗测试（Cowork 交互·Opus·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·无新 quirk·code commit `ceaca07`+docs·push 留 Mac/nightly）**：作者要「战斗回归像潜点一样·左选敌人·右选装备·『回归』改名『战斗』」。`?editor=combat`「战斗回归 Phase 3」基线编辑器（combatId/adhoc + stats/bonuses/seed/actions[] 表单 + JSON/LS 批处理编排）整块换成潜点式三栏——左 对手手风琴（遭遇/单敌两收起组·`EnemyPortrait` 头像+中文名·属性归中栏「信息中间栏都有」）·中 详情卡·右 逐槽装备+「▶进入战斗」→ 真实 `<CombatView>` 全屏活打（`buildCombatEntryState`·装备现读 `run.equipment` 即刻生效·派生 bonuses 喂上限）。抽 `loadoutPicker.tsx`＝装备选择器单一来源（潜点+战斗共用·DEFAULT_PICKS 再导出保 smoke #317）。`CombatScenarioSerializer` 库留供 CLI（现 UI 孤儿·未删）。nav 回归→战斗（URL key 仍 combat·深链不断）。门联动：combat-panel.css 改 `@import map-panel.css` 拿手风琴·`check-dev-panels` ② 靠留 `enemiesAlive.map` 字面·file-budget 删 CombatDevPanel 条目（1512→625）·smoke ① 改断潜点式骨架（②③ 序列化/引擎 parity 不动）。详见 CHANGELOG #334。
>
> **2026-07-23 #333 临渊侧陆架坡折下降 + 潜水取景盒放开——midwater 敞侧海床下沉替代垂直硬切（Cowork 交互·Opus·沙箱全量 regress 93/93 绿〔build 环境跳〕·SAVE 不变(20)·新 quirk #275·code commit `5d1c7fe`+docs·push 留 Mac/nightly）**：起自作者「右临渊的右边被截断了、不是边缘下降」。开阔水域单侧墙 `otherSide:'midwater'` 敞侧旧＝布尔硬切（`beyondOpenSide→OW_WATER` 的垂直岩面「截断」）→ 改**陆架坡折下降**（新 `openSideDrop`·quad 缓入 + 恒定斜率·越远越深落出取景＝无底·敞侧裸坡不撒结构）。作者第二问「潜水中也要看见断崖往下掉」⇒ `owFloorBottom`（概览）+ `SonarScanPanel` 潜水相机盒都按 `OW_OPENSIDE_REVEAL` 放开敞侧（横向 REVEAL/SLOPE·纵向 REVEAL）。新旋钮 `OW_OPENSIDE_{SLOPE,EASE,REVEAL}`（数值 defer §9）。门 `playthrough-openwater-wall` 加 ⑦（函数级坡折单调 + 渲染级海床顶面 x 随深度外移＝斜坡非竖切）。防埋点/taper/floorless/洞穴不变。详见 CHANGELOG #333。
## 1. 一句话状态
