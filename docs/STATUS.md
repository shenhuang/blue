...
> **2026-07-03 无人值守清 #249 尾：roadmap-stale tz 修 + 9 drift playthrough 归一 harness（Cowork 交互·#252）**：清 #249 遗留 ①③〔方向作者已认可〕。**#4**：`check-roadmap-stale` 两处 `git log --since` 注入模块级 `GIT_ENV`(TZ=UTC)→banner 的 UTC 日期与 `--since` 边界同基准，UTC 16:00–24:00 的提交不再误报过期〔quirk #213 闭环·+10/−2〕。**#3**：9 个 drift playthrough〔ascent/bluecaves/chart/decay/economy/equipment/farm-poi/stalker/tutorial-e2e〕迁 `makeHarness/pt.done()`〔委托 Sonnet subagent·只删样板不动断言·quirk #212 保底〕→ pt harness 迁移收口，`grep -L makeHarness` 现只剩 8 个有意保留的结构档。改文件 10〔1 checker + 9 playthrough·未碰 src/engine〕。全量 `npm run regress` **87/87 全绿**〔含全 38 tsx 行为测·scenarios 422/422〕·build 留 Mac/nightly〔#147〕·不 bump SAVE。遗留：两 banner 现为真过期可选 bump / 100 lore 待过稿 / 3 零提交 worktree 待拍。**树**：main·10 文件 + 三件套待单次提交·ahead 4 待推。
## 1. 一句话状态
