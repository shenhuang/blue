...
> **2026-06-15 海图 POI 显式 owner 归属 + 相对坐标（owner-anchored 重构·交互 session·#135·commit `21eda27`）**：`chart_pois.json` 17 个 POI 从隐式几何归属改为显式 `owner`（所属灯塔 id）+ 相对 owner 声明坐标的偏移；`generateChart::resolveOwnerCoords` 用 `ownerAnchorPos`（静态声明坐标）resolve 成绝对——**坐标解析与点亮判定（`profile.lighthouses`）解耦**·剧情 POI 在 owner beacon 建成前也能定位。`isLit` 改 owner 感知（有 owner→归属·无 owner→原几何+flag-gated）·对当前布局逐点等价旧几何（移除「靠几何重叠点亮」）。story 锚点带 owner 仅作坐标基准+标签·恒显恒可达不被 owner 门控（避免 beacon 解锁死锁）。owner-less 绝对坐标 lane 保留为 `absolute:true` opt-in。守门 `playthrough-chart §0b`（owner 必填或 absolute / 真实 region owner / 坐标 round-trip）。`npm run regress` **34/34 全绿 + build**。新 quirk **#133**。`chart_regions` 区半径微调（作者 WIP·未提交）未纳入本提交·与本重构正交。详见 CHANGELOG #135。
