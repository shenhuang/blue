...
> **2026-06-25 行前装包步可跳过：没东西可带就不进装包步（交互 session〔Cowork·Opus〕·#202·无新 quirk·沙箱 full regress 53/53·待 push）**：作者拍板「下潜时没东西可带就跳过带东西那一步」。`SeaChartView.tsx` 就地分步向导（#140·`none→pack→target`）的「出海」按钮原**无条件**进 pack 步——仓库无可携带消耗品时只渲染「没有可携带的消耗品」+ 一个按钮＝多余一击。修（单点·只动 `SeaChartView.tsx`）：`ChartInfo` 加 `hasPack = carryables.length>0` + `beginDepart()`——「出海」有料→`pack`（旧行为）·无料则跳过：有打捞目标→直接 `target` 步·否则直接下潜；`target` 步返回键按 `hasPack` 派生（有→「←装包」回 pack·无→「←返回」回 none·免空装包面板）。**单一判据管「去哪+返回」不漂移**。引擎零改（`applyCarryItems([])` 早返回·有料/非洞路径逐字节不变）；smoke 不破（直接 prop 渲染 ChartInfo·不走 onClick 决策）。不 bump SAVE·无新 quirk（决策单点自文档化）。遗留：push 待作者本机。
