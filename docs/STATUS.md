...
> **2026-06-27 LLM 试玩 harness（`tools/playtest-llm/`）+ 纯手动任务 `blue-playtest-llm`（交互〔Cowork·Opus〕·#227·新 quirk #195·不 bump SAVE·零改 src·沙箱 typecheck+静态门绿·待 push）**：作者问「playtest schedule 适不适合多 agent 并行」→ 做成「LLM 当真玩家·逐回合驱动真引擎·产 findings」的 harness + ad-hoc 纯手动任务。全调现成引擎 headless API〔`equipItem`/`buildAtLighthouse`/`sellItemToMira`/`selectChoice`/`startDiveFromPoi`…·零引擎改动〕·确定性 replay+港口 checkpoint·逐回合战斗·runaway/loop 守卫·`play.sh`+`PLAYER_AGENT.md`。dry-run-first 抓到 builder 自测掩盖的 2 个 blocker〔港口进潜路径不可见/战斗 loop 误判〕→修→独立 Opus 复跑过门〔surfaced-only 完成 3 潜·逐回合战斗 OK〕。真发现：早期经济=材料墙非金币墙〔建议早期播浅水材料〕·「往深=白拿奖励」误教·晶矿死交互·文笔/结构亮点。任务＝ad-hoc 纯手动〔无 model 参数·Run 时手选 Opus〕·harness 在 regress 门外〔quirk #195〕。push 待 Mac/nightly。---
