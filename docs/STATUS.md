...
> **2026-06-11/12 剧情定调五批 → 剧情 SPEC 落档（交互 design session·#114·纯文档无代码）**：作者五批拍板完整故事框架入新 `docs/spec/深海回响_剧情_SPEC.md`——内核「深海还你你带下去的东西」·一章圆满=主角以为的真结局·**失联=单程下行**·**死亡=断片说**（一二章零泄漏·quirk #117 剧透纪律）·每章大机制（二章幻觉实化/番外温度/三章人变鱼+倒置规则）·前几章同图分区解锁、三章分层向下·解锁高亮规约（数据渲染禁手写·徽章永远诚实）·实装路线图 St0–St7（剧情骑现有轨道·机制按需长·周末引擎只做 [氮醉] 氛围变体）。**先做好一章：下一步=St0（教学关植入半本日志+story flag 脊柱最小版）**。三章「另一个世界」=方向已定调、设计仍留专门 session（d_reveal 不动）。详见 CHANGELOG #114。
