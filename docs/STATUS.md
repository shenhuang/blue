...
> **2026-07-15 #308 大深渊结局 SPEC（Ch4 终局·设计稿·未实装）+ `check-no-human-assertion` regress 门 + `story.ts` 白板对齐（Cowork·Opus 设计 + 本 session 验证收口提交·沙箱全量 85/85 绿·新 quirk #256·commit 待定·push 留 Mac/nightly）**：#300 白板后**新建的第一份终局设计**——把「主角其实不是人类」（你＝古文明一次相对成功的作弊·夺舍消耗人类躯体 + 复活点重新打印·铁律「揭示≠变身」）落成自持 SPEC，并立 `check-no-human-assertion` 门守叙述体永不断言主角人类身份（护 fair-play twist 不穿帮·豁免 label/NPC/journal·破例走 ALLOW·quirk #256）。`story.ts` 顺带清 #300 白板遗留（标休眠脚手架·删 `ch1RecordingFlag`·**保留** `ending.blank`）。SPEC **未实装**·§6 open（夺舍时点 / 记忆侵蚀×run / reconcile / 数值 defer）。全量 85/85 绿。树：main·未提交·push 留 Mac/nightly。
> **2026-07-15 #307 开阔水域「贴底节点」机制——`seabedNodeIds` 单一真相 + `atSeabed` 事件门 + floorless 复用 `midwater`（Cowork 交互·Opus·`npm run regress` 沙箱全量 84/84 绿·新 quirk #255·未提交待作者/nightly）**：作者问「开阔水域最下面节点没贴海床面能贴吗 / 贴底节点能否特殊标记区分剧情」。定论：贴底节点**已贴底**（差 `OW_FLOOR_GAP`·想更近是 defer 数值），真需求＝给贴底节点语义身份门控专属剧情 + 能生成「无贴底节点」纯中层图。新 `engine/seabed.ts`（`terminalNodeIds` 从 ui 下沉·`seabedNodeIds`＝终点 ∧ 有海床档 sand/coral/rock/atoll）；floorless **复用已有 `midwater` tag**（不新造·免撞 #305/quirk #254）；新 `{kind:'atSeabed'}` Condition 门控珊瑚/矿床/爬行生物/尸体/巢穴等**贴底专属**内容·悬空中层不出；渲染层锚点改 `seabedNodeIds` + `OwGeom.floored`（floorless 整窗全水·quirk #255）；验收 `playthrough-seabed.ts`。**内容池事件 JSON 未写**（作者定「暂不写·会监督写」·草案已过·独立 lane·Opus/medium）。全量 84/84 绿。树：main·未提交·push 留作者/nightly。
