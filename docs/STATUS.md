...
- [ ] **敌人库（Enemy Library）· 设计已定·未实装** —— 把敌人从"散文件 + `combat.ts` 手动 import + 无元数据"升级成**可查询 / 可复用 / 目录自动加载**的库，让生成剧情时按 band 取一只合适的已有敌人（先取后造）。两原则：**数据驱动默认**（纯数据敌人＝零引擎改动·只有新机制原语才碰引擎·边界表见 SPEC §3）+ **可维护/可扩展优先**（元数据全可选 additive·不入存档）。三支柱：① `EnemyDef` 加 `bands`/`biomes`/`role`/`threatTier` 元数据（深度×环境×生态位·"红树林热带鱼不进极地"）+ `codex` 背景（喂未来图鉴）→ `pickEnemy`；② def·encounter 解耦走 `enemyRef`（复用 `combatScenario` 现有 ad-hoc 合成）；③ 目录 glob 取代 `combat.ts` 47–65 手动 import。落地时必加 `check-enemy-refs`（引用完整 + 无孤儿 + 有 baseline + schema 合规）并取正式 quirk 号。**远期目标（SPEC §5）**：让独立无记忆 session 能「描述→实装」一只敌人、以及定时无人值守生成敌人——二者均为库落地后的薄层，前置＝支柱三 auto-load（无人值守绝不改引擎）+ 上述 checks（绿门＝唯一可信「完成」信号）+ 入库契约（模板/schema + `--new-enemy` 脚手架）。设计源 `docs/spec/深海回响_敌人库_SPEC.md`（含数据vs引擎边界 §3 + 两工作流 §5 + 实装清单 §7 + 开放问题）。**优先级/排期由作者拍**（2026-06-13 交互 session·先方案不动代码）。
