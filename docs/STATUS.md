...
> **2026-06-25 跨 beacon 多口持久洞 POI 接线（cave roadmap T3b·交互 session〔Cowork·Opus〕·#201·新 quirk #175·commit `7cee379`·沙箱 full regress 53/53·待 push）**：把 T2(#198) 立的持久洞模型接通海图——4 洞登记进 `caves.json`（serpentine_deep/mirror_maze/flooded_gallery 大 + blue_throat 中·口数照 cave_zones_spec §1·entranceDepths 钉口深·占位待调）；`chart_pois.json` 4 主口 anchor 加 `caveEntry(entryNodeId:"node.1")`、新增 4 副口 anchor（owner=对侧 beacon·`node.2`·gated 在 `flag.cave_exit_<zone>`）；**样式拍板**：副口=普通 POI·不连线/不同色·复用现有渲染。**穿越发现 flag（§6.2）T2 并未实装、本棒补**：新 `CaveGenParams.traversalFlag`（caves.json 单一源）+ `ascent.ts::executeAscent` 从**出口门户上浮**置位（揭示对侧副口）。落成机制：`check-cave-bindings` 加穿越链漂移门（副口 cave_exit flag === 洞 traversalFlag·traversalFlag 洞须 ≥2 入口·flag 必被消费）。reach 门不破：副口 gated 不在 fullyRevealedProfile ⇒ §7 锚点计数 23 不变。不 bump SAVE（纯 additive·#99）。**遗留**：与 #200 并发在 main——代码已 commit（仅自己 5 文件）·文档侧（本条+#201 CHANGELOG/#175 QUIRKS）按作者指示**已写不 commit**·留作者把 #200+T3b 一起 commit/push；数值/口深占位待调；T2b 深门最后期。
> **2026-06-25 教学后「重返东礁老沉船」剧情不出现——`captain_revisit` 误归随机池（交互 session〔Cowork·Opus〕·#200·新 quirk #174·待 push）**：真因＝重访节拍 `tutorial.captain_revisit` 被错放进随机事件池（weight 10·与 ~45 个 reef/wreck 竞争·二次下潜实测 ~3% 命中＝基本看不到）·**非完成链坏**（完成链经新 `playthrough-tutorial-e2e` 三路〔上浮/逃跑/船长日志〕真端到端确认已由今日 fix 修好；旧测是手搓 flag 的盲区）。修（照仓库范式·additive）：两个 revisit 变体→`weight:0`（退随机池）+ `poi.anchor.east_reef.storyOpenEvents`（新 `ChartPoi` 字段·`dive-start` 按事件自身 prereq/forbidden/oncePerSave 选合规变体强制开场·= `poi.story`〔锁 4 锚点〕/单事件 `openEventId` 的非锚点·多变体泛化）。**系统审计**：全 39 anchor 有脚本剧情的都走强制·「无强制」全是探索型 POI（随机 ambient 即设计）·扫全库同类误归=0·主线锚点/结局 flag 不靠随机池推进=0（脊柱不被运气卡）。落成门 `check-story-open-events` + `playthrough-story §2d` + `playthrough-tutorial-e2e`。沙箱 `regress --skip build` **53/53**。不 bump SAVE。`storyOpenEvents` 那行已被作者 `7cee379` 顺带提交；余 5+2 文件本 session 提交（不含作者 WIP `styles.css`/`SeaChartView.tsx`）。遗留：重访「下潜开场 vs wreck 深度途中触发」演出取舍待作者拍。
> **2026-06-25 温度系统接线 dive/state（cave roadmap T4 follow-up·交互 session〔Cowork·Opus〕·#199·新 quirk #173·`829611b`·沙箱 full regress 51/51·待 push）**：把 #195 落地的温度纯函数三件套接进 dive/state（SPEC §7 全做）。`Stats.thermalStress`（0–100·**不进 Stat 联合**）；`tickTurns` 每回合 `stepThermalStress`（intensity←`getCaveTemperature(run.zoneId)`·insulation←`loadoutInsulation(equipment)`）+ 过 WARN `thermalStaminaDrain` 扣体力（守「逐回合==一次性」指数不变量·同 stepNitrogen）；`startDive`（zone/POI/band/持久洞唯一汇流点）入潜门控 `entry_blocked` 不让下潜；`loadoutInsulation`=BASELINE(30)+`EquipmentStats.insulation`（温度↔装备唯一桥接·新增 `insulation` EquipmentEffect kind=未来保温服挂点·当前恒 30 兜底）；`SAVE_VERSION 10→11`（删旧档·连带 4 处 playthrough version pin·**quirk #173**）；侧表 zoneId 对齐 `zone.*` 规范 id + zoneId⊆zones.json 门（§7）。**中性洞应力恒 0 ⇒ 现有行为逐字节不变**。遗留：partial「深节点硬门」留 future hook（现靠逐回合软门逼退）；数值/intensity 占位待作者统一调（defer-number-tuning）。
