...
> **2026-07-19 #320 回正改图内浮动小图标（同日续·Cowork·Fable·沙箱全量 90/90 绿·无新 quirk·commit 待定·push 留 Mac/nightly）**：旧头部行内「回正」出现时挤动标题行、图被往下推——移进声呐图右上角绝对定位小图标（⌖·camMoved 才渲染·stopPropagation 不触发平移/取消选中·零回流）；smoke Q2 断言/类名原样。树：main·commit 待定·push 留 Mac/nightly。
