...
> **2026-07-12 #295 残骸区子区改名：`沉船墓园`→`塌架墓园`（脱钩「沉船」意象·Cowork 交互·Sonnet·纯展示字符串+注释+活文档·零逻辑·regress 90/90 全绿·新 quirk #245·未提交待作者）**：作者要子区名不再强绑定「沉船」，讨论后定「只换标签、内容不动」（温带商船/货舱/蛛蟹等叙事保留）。展示名**无单一来源**——`zones.json`(zone.name)/`chart_pois.json`(poi.name)/`lore.json`(22 处 `group`) 三处独立字面量需同步改；另改源码注释/脚本断言消息/scenario `_comment`/5 份活 SPEC；append-only 历史文档（QUIRKS/CHANGELOG/changelog.ts）与沙箱暂存/待删目录按约定不动。验证：typecheck + 全量 regress 90/90 + 4 个直接命中脚本重跑确认展示名换新、断言仍过。**顺带发现**：工作树里混着另一 Cowork session（"Combat status system design"）刚完成的战斗状态系统实装（`engine/status.ts`+`combat.ts` 等，完整且随 regress 一并绿）——问过作者，本次收尾**只提交改名 17 文件**，战斗状态系统留给它自己的 session 收尾。树：main·commit 只含改名 17 文件·ahead·未 push 待 nightly。
