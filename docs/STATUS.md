...
> **2026-07-07 The Warren 女王「身体库存」主线实装（Cowork 交互·Opus·#272·接 #271 SPEC·沙箱 regress 95/95·新 quirk #234·未提交待 nightly）**：§14 女王分散动作（吼叫/信息素/产卵/回血/结盾）收口成一条主线＝耗空身体库存。作者拍 2 决定：**单动作/回合优先级树** + **droneReplenish 退役→繁殖储备节流**。新文件 `combat-warren.ts`（§15 一族从 combat-mechanics 外移守 file-budget·1007→705·combat.ts 1167/1170）：`maybeWarrenQueenAct` 六分支择一——feed(`warrenFeed`·替 `corpseEating` 主动献祭·被吞不掉料)/screen(`warrenScreen`·替静态 `shieldedBy`·`screeningFor`+`checkActionAvailability::queenScreened` 门·开战 `warrenInitScreen` 起初始盾·池空熄火＝跑步机护栏)/lay(`warrenReinforce`+繁殖储备·每产卵扣/每回合缓回·低→间隔长+产卵少)/detonate·hatch·cocoon-boost filler(`roarChance` 退役)。`recordQueenDamage` 挂 applyAttack。新战斗态 `screeningFor`/`recentDamageLog`/`layReserve`/`lastLayTurn` 全不入档·SAVE 未 bump。**独立 code review 抓 §4 bug（→ quirk #234）**：储备节流自纠到非零平衡·单靠数值停不下产卵→死角女王有无限饲料可能杀不死；修法＝`warrenRecoverReserve` 死角(`isHatchery`)零恢复（有限窝耗尽→§4 可杀·长局 evade 实测死角产卵封顶 2 次即停·非死角照常）。两开放叉决议：女王本就**无** `maternalBehavior`(mycelial 的·未动)·feed 只替 corpseEating；茧不当盾/献祭·唯卵例外(§4)。baseline：redesign `warren_hatchery_solo__kill_collapse`(feed 回血耗尽赛+kill·§4 0-survivor terminal)+`reinforce_egg_lifecycle`(lay+breakDestroys+hatch)+新 `warren_queen__feed_sacrifice`(feed+screen 门端到端)·screen(肉墙)/lay(产道)亦被 room1/room2 覆盖·9/9 warren 绿·mycelial_queen 未破。SPEC §15.8 实装状态落档。full regress **95/95 全绿**（沙箱 esbuild-full·prod build 留 nightly·#147）。树：main·未提交待 nightly（#249–#272）·定时任务全停。
> **2026-07-07 月相存档窗 + 女王身体库存 SPEC + 寄生 reframe canon + 尸衣者→水鬼 大改名（Cowork 交互·Opus·#271·接 #270·沙箱 regress 95/95·新 quirk #233·未提交待 nightly）**：四块。**① 月相存档窗**（§9.11·additive 不 bump SAVE·后台 subagent+复核）：`lunar.ts::moonPhasesElapsed`（数相位边界非天差）+ 新 `PlayerProfile.warrenHunt`（离港 bank·仿 caveMaps·`port.ts` 盖章 profile.day / `startDive::resolveWarrenHuntCarry` 接回·≤1 相续、>1 reset）+ baseline `playthrough-warren-savewindow`；清 agent 多加的 dead `RunState.warrenHunt.lastVisitDay`。**② §15 女王「身体库存」主线 SPEC**（作者拍 3 叉·未实装）：feed(替 corpseEating)/screen(替 shieldedBy·⚠跑步机)/lay/detonate/hatch/cocoon-boost 六动作 + 需求优先级树 + 近期伤害计数·命名分吼/信息素·数值占位。**③ §16 寄生 reframe canon**：本体=寄生虫 **髓织虫/Radicula Laceworm**（Sacculina 蓝图 + 脑吸虫神经操纵·嫁接脊椎·科幻许可）控宿主鱼·整巢皆傀儡；**水鬼伞 who's-who**（水鬼/the Living Drowned·恐人/Horror sapiens 罩头足+人类版·恐鱼/Horror piscis=The Warren·双重误分类 potato）；§2b/§2c origin 作废重定向 §16。**④ 尸衣者→水鬼 / corpse_wearer→horror_sapien 大改名**（作者拍替换·2 subagent+复核·95/95 无 bless=行为不变）：稳定 code id `horror_sapien`（不随文案漂）·显示默认 水鬼·英文 the Living Drowned；文件/类型/combat/dev/baseline 全改·`check-terminology` BANNED 改「穿尸/尸衣者→水鬼」·89+ docs swept·SAVE 未 bump。遗留(quirk #233)：冻结深水区 apex 英文 corpse-wearer + event id the_wearer_* + QUIRKS 历史条目仍旧名（内容层·随冻结 B）。full regress **95/95 全绿**（沙箱 esbuild-full·prod build 留 nightly）。树：main·未提交待 nightly（#249–#271）·定时任务全停。## 1. 一句话状态
