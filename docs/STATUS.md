...
> **2026-07-01 通用 NPC 信任系统 Phase 1（机制层·零行为变化·先绿 land）：本 session 与作者多轮设计出「藏宝图+token+特殊商人+通用信任」贸易/关系/寻宝系统〔SPEC 新落 `docs/spec/深海回响_藏宝贸易与信任系统_SPEC.md` v0·系统优先〔关系纵深/玩法循环/长线重玩〕·材料 grind+金币 sink 顺带·阵营前向兼容缝 §3.9 极地火山〕·本批落核心 Phase 1＝通用信任机制。做：profile 加信任表〔Record·加法·不 bump SAVE=12〕+ 新 `engine/trust.ts`〔trustValue/trustTier 派生镜像 ch1Story·gainTrust/loseTrust 单写者镜像 injuries·有符号支持掉信任·阵营 delta 将来单 choke point hook〕+ `npcTrustTier` Condition 原语〔复用 evalCondition DSL·同门控对话 visibleIf + 商店 minTrustTier〕+ 新 `check-npc-trust` 门 + `check-boundaries` 规则七〔信任字段触碰面收口〕。零 NPC 使用⇒零行为变化（Cowork 交互〔Opus〕·#243·新 quirk #203·沙箱 `npm run regress` 79/79 全绿〔--skip playthrough 42 + --only playthrough 37·含 save round-trip·build 留 Mac·#147〕·单 commit〔feat+三件套〕·push 待 nightly）**：过程教训→#203：加 Condition 种类补 3 switch〔collect 返 void typecheck 不报·手动补〕·tripwire 扫注释坑。下一步：Phase 2 特殊商人 MVP〔需作者拍人设/数值〕·数值全 defer·push 待 Mac/nightly。
