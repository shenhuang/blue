...
> **2026-06-25 温度系统：热/冷双极门控（解耦侧表 + 纯函数·ch1 标注·psm 车道 `cave-temp`〔Cowork·Opus〕·#195·新 quirk #171·`4c9e977`·收尾全量 49/49）**：§6 温度门控机制层·与 T2(cave-bmap) 并行走解耦侧表——不碰 dive/state/zones.json。五件套：子-spec + `cave_temperature.json`（按 zoneId 侧表）+ 全纯函数 `engine/temperature.ts` + `playthrough-temperature.ts`（6 组·含数据↔标注一致性门）。接线属 T2 后 follow-up。收尾全量逮到 cave-chart 遗的 main 红 → 修 `smoke-chart-ui` blue_caves stale 断言（新 quirk #171）。
