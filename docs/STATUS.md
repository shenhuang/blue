...
> **2026-06-21 ch1 St2 留白结局 `ending_blank` + 录音第 1 段（交互 session〔Cowork〕·#172·无新 quirk·commit `1991c34`·gate 11/11 绿）**：圆满结局两选项 loot 破损饰品 `item.ch1.steadying_charm_broken`（置 `story.ch1.charm_found`）；之后持饰品重访 vent POI → `dive-start` 据 `chart_pois` 的 `POI.story.revisit*`（通用三字段·`types/chart.ts`）强制开场 `ch1.ending_blank`（门=持饰品 ⟺ fulfilled-first·保证圆满在前、第一次绝不跳过留白；band 深潜早返 `diveIntoBand` 不受扰）。空站+完整图鉴+录音第 1 段 `lore.ch1.recording_1`（导师真声·回收 tells T4「台词全出自日志」的反证·刻意留白不泄失联〔归 St7〕·过 protagonist-voice）。`story.ts` +`ch1RecordingFlag(1)`/+`CH1_CHARM_FOUND_FLAG`→`allStoryFlags`。「够深必出幻觉」scoped（本事件+二章·非全局·无永久佩戴税）+ 饰品**宝石材料**修复（新材料类·一章钻石稀少/外传火山量产/二章群宝）+ 主 SPEC §11 章节制重写 留二章/后续（剧情 SPEC §4.1/§4.4 已记）。沙箱 ff 绕 unlink 坑走 plumbing（read-tree 临时 index→commit-tree→CAS update-ref→truncate-write 同步·同 quirk #148/#170）。
> **2026-06-21 pre_combat 前序叙事子阶段（交互 session〔Cowork〕·#171·无新 quirk·commit `858ab08`·typecheck 5/5·combat-scenarios 33/33 绿）**：高等级遭遇进战前先停一屏叙事，玩家点「迎战」再开打。`DiveSubPhase` 加 `pre_combat`；`CombatEncounterDef` 加 `showIntro`；`enterCombat`（showIntro 门控）+`confirmEncounter`；新 `PreCombatView.tsx`；cave_grouper_boss / mycelial_queen / cocooned_resident 三遭遇标 showIntro:true；猎手伏击直接进战不停顿（不走此门）。
> **2026-06-21 海图 POI 调试器 ChartViewDevPanel（`?editor=chartdev`）+ SSR smoke（交互 session〔Cowork〕·#170·无新 quirk·commit `525e73b`·47/47 绿）**：在 dev 工作台地图组新增 POI 调试面板——可视化所有 chart_pois.json POI 的揭示态（lit/dim/hidden）·reach（effectiveDistance）·深度/洋流/能见度修正标签；左栏 runsCompleted+outpost 控制组·右栏 10 列 sticky 表格；全只读引擎调用；SSR smoke parity 对齐。PSM worktree 已合并进 main（ff-merge 绕坑走 git show+cp+显式 add·同 quirk #148）。
> **2026-06-21 四只新复杂敌人 + 四种 boss AI 机制（交互 session〔Cowork〕·#169·新 quirk #156·commit `581e1ac`·46/46 绿）**：裂球（分裂增殖）/ 清道夫（食尸吸攻击）/ 菌群鱼（女王+工蜂护盾门）/ 茧化居民（三阶段变态）。新机制：`splitBehavior`·`corpseEating`·`shieldedBy`·`metamorphosis`·`phaseArmorOverride`。+4 enemy JSON +8 combat baseline。`droneReplenish` schema 预留 runtime 未接（时序未决）。数值占位·文案[待过稿]。playthrough-corpse 存在预存 flaky（quirk #157）。
## 1. 一句话状态
