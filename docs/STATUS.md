...
61. **深水区 Phase 1 plumbing：可扩展纵向深度轴（band 阶梯 + 蛙跳下潜，2026-06-03）**——「很大的图」的脊柱第一段。SPEC §5 Phase 1 + §12 清单（已勾 plumbing）源真、§10 决策日志记拍板。
    ...
    - **Deferred（明确留后续，见 SPEC §12）**：trench 专属 zone/事件/band 级 tag 池（现借蓝洞、深 cave 事件多 ≤60m 故 trench 事件稀）；范围/分辨节点级 clarity（Phase 0 已 defer 到此、仍未做）；真前哨蛙跳（Phase 2 持久前哨建成后换掉 home stand-in）；band 级 alert 倍率/不饱和成本曲线。**下一步＝Phase 2 跨 run 前哨/能源、或深段内容、或 Phase 1 续。**
