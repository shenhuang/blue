...
> **2026-07-02 藏宝贸易系统 Phase 3 Slice 1a+1b：特殊商人 Silas→Sela 改名+retheme + Corin 藏宝线端到端 + 打捞队 canon 独立档**：把「泛探险家 Silas」收成四人打捞队幸存者 **Sela**（女·收藏家·连 id silas→sela）；小队黑背鸥小队/The Kelp Gulls（前身赤喉海盗团/The Crimson-Throats·理念不合退出）；深潮币→**红喉鹈币**（军舰鸟徽·分赃筹码）。Corin 线端到端：Sela deeper @信任档1 给 `item.treasure_map.corin_survey`（marksPois 揭示固定藏宝点 `poi.anchor.corin_cache`）→ openEventId 开箱得半枚红喉鹈币 `item.keepsake.corin_coin` + 币 → 回港交还 Sela（takeItem + gainTrust + 报酬 + `story.ch1.corin_returned`）。新 DialogEffect `gainTrust`/`takeItem`（走 `trust.ts` 单写口 / `removeFromInventory`）；新 `check-collectibles` 门（keepsake 对玩家无用）；三 `story.ch1.corin_*` flag 进 `allStoryFlags`（支线·不挡主线）；打捞队角色 canon 抽成 `docs/spec/深海回响_打捞队_SPEC.md`（镜科考队 SPEC·藏宝贸易 SPEC §12 只留机制+指针）（Cowork 交互〔Opus〕·#246·新 quirk #206〔沙箱文件 rename＝mv 进 `.git/.sandbox-junk`〕·不 bump SAVE〔纯加法字段/flag/effect·仍 12〕·沙箱 `npm run regress` **81/81 全绿**〔`ESBUILD_BINARY_PATH` 跑 tsx·含扩测 playthrough-trust·build 留 Mac·#147〕·**未提交·commit 待 Mac/nightly**）**：下一步 Slice 2（Nerea 线埋 Vera 伏笔 + 渐深遭遇 + 藏宝图来源分层·§12.10）；Mac 收尾须 `git rm silas.json` + `git add sela.json`（quirk #206）；未做真实浏览器验收（建议 `npm run dev` 走一遍 Corin 循环）。
