...
> **2026-06-14 灯塔/蛙跳重构 step②③④ 完成 + 出海更近删 + DEV 改 ?dev + 信标删 + 暗点可达条件（交互 session·#128·接 #125 step①·多轮线上验收·未提交留作者 push）**：①step② 合并下潜入口（`ChartPoi.bandId`→`diveIntoBand`·**中转站/depot 整删**）+ 深入 POI 由「探深」升级轨置 flag 派生（揭示圈内浮现·复用 #124 flag-gated）；②step③ 点灯塔/哨站=`HomePopup/OutpostPopup`→「设施升级」面板（删 HomeDivePopup/底部按钮/×/从此处下潜）；③step④ 新 `check-dive-refs` 门（任务 31→32）。验收迭代：教学门只认 `?dev`、升级**不扩**扫描（`revealRadius` 钉死）、文案/四字名换行/资格区高亮(一次一个)/「设施升级」改名。后续三删改：**出海更近/距离预耗氧整删**（每潜 turn0 满氧·`reachReduction` 全清·`effectiveDistance` 留纯展示·删「距岸」标签）、**DEV_TOOLS 只认 ?dev**（去 `import.meta.env.DEV`·npm-dev 不再恒开·**反转 quirk #97**）、**信标轨删**（`lightRadiusBonus` 死码连根清·勘测站/探深保留）+ **暗点可达条件** `poiBlockReason`（勘测暗点→点亮更近灯塔/能力门→需要X/天气→潮变）。**regress 32/32 全绿 + build 绿**。新 quirk **#129**（回归并行 flaky·重试即过·根源待查）。**遗留**：flaky 根因单查 / `effectiveDistance` 可清 / 本批未 push。详见 CHANGELOG #128。
