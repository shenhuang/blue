...
> **2026-06-27 收尾 session：术语 lint 机制 + CHANGELOG #219 补号 + roadmap banner 对账（交互〔Cowork·Opus〕·#225·新 quirk #193·不碰游戏逻辑·沙箱 typecheck + check-terminology + 静态门全绿·待 push）**：作者「看看整个项目什么适合收尾」→ 收口 3 条线。①新 `check-terminology` 门（扫 src+scenarios「穿尸」报红·逼回与代码/敌人库对齐的「尸衣者」·把 #224 术语统一收成机制）+ 清 2 个 wreck_graveyard scenario 残留「穿尸」_comment（仅注释·回放零影响）。②补 CHANGELOG #219 空号（lunar 潮窗恒显 `48aa74a`·插回 #218↔#220）。③`cave_roadmap.md` banner 06-25→06-27 对账：subagent 逐条核 git·9/10 项已落 main 翻 ✓〔各带证据 commit〕、唯 T2b 深门 ⬜·roadmap:check 告警 9→3〔余 3 为当日 tutorial 顺带碰·可接受〕。push 待 Mac/nightly。
> **2026-06-27 科考队 SPEC 一致性审查 + 命名立体化 + 路径修订（交互〔Cowork·Opus〕·#224·纯文档·无 quirk·不碰代码/scenario·静态门全绿·待 push）**：作者问「科考队设定结合当前故事是否有不合理处」→ 审查滚成完善。修：术语统一（旧「穿尸者」→「尸衣者」·与代码/敌人库对齐）·区域≠章节命名分离（渊洋科考站/禁岛海域/极地火山/大深渊·去 A/B 裸代号）·会师层升格已拍 canon·主角非失联队·导师二章暗线·尸衣者成因先不交底·Théo 一章末遭遇战。命名立体化（Iskra〔火花〕/Vesper〔黄昏星〕/Voss〔狐狸·早熟天才〕/Alethea〔真理〕·年龄/声音/外貌·Alethea 设女+纯标本编号·去代号用名字）。subagent fresh-eyes 复审 + 路径修订（§6 共同下行+分合交替·大深渊汇合·Alethea 实验挪禁岛师生同地·年龄基准统一＝十年前下潜当时·导师 38·对称表/Vesper 路径/会师非物理会合消歧）。文件＝docs/spec 三件（科考队 SPEC +121·剧情 SPEC·tells 清单）+ memory expedition_team。push 待 Mac/nightly。
> **2026-06-27 战斗「拳脚扭打」改兜底攻击（`fallbackOnly`·交互〔Cowork·Opus〕·#223·新 quirk #192·不 bump SAVE·沙箱全量 65/65〔`--skip build`〕·待 push）**：作者问「拳打脚踢是否多余」。审定 `action.fist` 被起手刀严格压制却常驻菜单＝死按钮 → option 1 改真·兜底（`fallbackOnly:true` + 单判据 `hasUsableWeaponAttack`〔只看持有不看体/氧〕·`listAvailableActions` 末尾门控）。**只动可见菜单·不动可用性**（checkActionAvailability/applyPlayerAction 不读它 ⇒ scenario replay 零影响·quirk #192）。新 `playthrough-fist-fallback` 守四分支。**同日顺手修**预存红 `playthrough.ts`（#222 教学 node 化漏迁的唯一非-node-aware 教学 walker·纯事件链走到节点边界即停）→ 换 node-aware 导航（仿 e2e §D·复用 enterNodeSelection/moveToNode）·沙箱全量回 **65/65**。push 待 Mac/nightly（prod build #147）。---
