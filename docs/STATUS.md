...
> **2026-06-14 氮气系统重做·饱和模型 + 氮醉接线 + 紧急上浮真实化（交互 session·#127·quirk #128·只收氮气一摊·未提交留作者 push）**：作者「教学氮气=0→要更真实」。诊断＝旧 `depth/30` 浅水≈0 且无饱和上限。新 `engine/nitrogen.ts`（潜水期氮气唯一计算点·单写者 #128）：`nitrogenCeiling(d)=100(1−e^(−d/100))` + 半衰期 τ=15 指数逼近（同管吸/排·逐回合==一次性守 stalker）+ 连续氮醉 `0.4(N/100)²(d/50)` 扣理智（**首次接上 SPEC 承重墙**·主 §3 此前从未实装）。接 `tickTurns`；删 `campAtNode` 原深排氮（改 tick 按 ceiling 微涨）；`ascent` N2 迁入。上浮：`isAscentBlocked` 统管含 emergency·删「凿穿洞顶」·`CombatView` 紧急上浮加 `!isAscentBlocked` 门（封闭水域脱离靠 flee）。常量＝SPEC §8 首版旋钮（D0=100/τ=15/k=0.4,p=2）。新 `playthrough-nitrogen` 门 + bluecaves camp 基线重抄。**氮气/上浮全门 + typecheck + 全 node 门 + 27 playthrough 绿**（沙箱补 linux esbuild 跑 tsx）；**build 门未在沙箱跑**（缺 native·留作者/nightly）。**那 2 条红 lighthouse（`playthrough-lighthouse`/`smoke-chart-ui`）＝作者并发未提交的信标轨重构 stale 测试·与氮气零交集**。SPEC＝`docs/spec/深海回响_氮气系统_SPEC.md`（supersedes 主 §3/§390/§401 旧数值）。**遗留**：教学演示拍（主线中期·内容）/减压病持久后果（作者以后做）/旋钮 playtest/单写者 gate 化。详见 CHANGELOG #127。
> **2026-06-14 敌人库收尾·enemyRef 线上闭环 + 图鉴 UI（并行 session `feat/enemy-rest`·#126·**未在 main·待作者合**）**：#121 余项 a/b。**(a)** enemyRef 真用例＝新 `combat.wreck_field_patrol`（party 写 `enemyRef{band,biome}` 不写死 defId·`pickEnemy` 从〔沉灯水母／蛛蟹〕池取）+ 触发事件 `wreck_graveyard.patrol_stir` + combat baseline（seed 100000 取蛛蟹·实跑抄）；引擎修 `combatScenario` 把 `startCombat` 包进 `withSeededRandom`＝pool 取样可复现（defId 零 RNG 消耗 ⇒ 现有 baseline 逐字节不变）；`verify-tutorial` 认 enemyRef 成员。**(b)** codex 充实 9 只（四段氛围·守剧透红线 #117）+ 新 `BestiaryView`（港口「潜水志·图鉴」·只显已遭遇〔`enemy_seen:<id>`·startCombat 单点写·不 bump SAVE〕·氛围+软标签不露数值·只报「已记录 N 种」）。约定全由 gate/注释守＝**无新 quirk**。**regress 31/31 全绿**（事件 238→239·combat baseline 17→18·敌人仍 9）。**route A 跳过**（纯重构·并行隔离下不值·SPEC §8③）。详见 CHANGELOG #126。
> **2026-06-14 前哨衰减/维护系统删除（交互 session·#125·灯塔/蛙跳重构 step ①·未提交·留 closeout）**：作者「衰减没必要了·全部去掉」+「以未来可维护为目标」。删整套前哨衰减/维护：`outpostDecayLevel/effectiveOutpostStage/effectiveRevealRadius/maintainOutpost/OUTPOST_MAINTENANCE_COST` + 衰减常量 + `maintainedRun/storedRun` 计时。**能源保留并解耦**（`outpostEnergy(lh)`/`effectiveOutpostBonuses(lh)` 去 profile 参·capacity=base+hydro·掉线结构性保留）·**有效层塌平**（stage/reveal 恒定不回退）·**中转站改纯库房**（`storedMaterials`·无锈蚀；depot 改「任意手头材料前置库房」＝作者可否·留 ②③ 复议）。`outpostState`→`{stored?;discovered?}`·`advanceOutpost` 不再写它·旧档残留字段无害不 bump SAVE。回归 `playthrough-outpost`（subagent 重写·删衰减/维护/锈蚀段）·`smoke-chart-ui` §N2/N3。**31/31 全绿**·无新 quirk。**起手并发插曲**：树在 auto/weekend、另一写手并发改我地盘（#124 鲸落）→按 quirk #104 停手还原·等合 main 再续（已合）。**遗留＝本重构 ②③④（需作者拍设计）**：点旧灯塔/哨站=设施（去出发/蛙跳）+ 深入 POI 由升级派生（外观同潜点·扫描即浮现）+ 去全局「灯塔设施」按钮；**关键** ②③ 对着新 flag-gated region 原语（#124）重规划·「升级解锁深入 POI」很可能复用它（比 `unlocksDive` 顺）。详见 CHANGELOG #125。
> **2026-06-14 鲸落 flag-gated region 机制核（交互 session·#124·commit `e2d8835`·先架构讨论后实装）**：#120 鲸落遗留的 step ①。作者拍「先讨论这机制是否通用」⇒ 架构（quirk #127）：解锁的**门一律是 flag 存在性**·差异全在「谁置 flag」触发侧（剧情节拍/NPC 对话/捡道具/目击计数）·门保持 flag-AND 不引条件 DSL·计数阈值派生进 story.ts。引擎原语（唯一新增·可跨章复用）：`ChartRegionDef` owner 改可选 + `revealFlag`+`center`（owner-anchored ↔ flag-gated 二选一·`regionConfigErrors` 不变量门）·`isLit` 纳入 flag-gated 区为揭示源（圈内 POI 随 found flag 点亮·delegate 链守 mimic 唯一谎点）·SeaChartView owner-less 区圈渲染。鲸落改 flag-gated（revealFlag=`story.ch1.whalefall_found`+center）·移除占位 `outpost/band.ch1_whalefall`（reveal 与建营地解耦）。story.ts 鲸落 flag 词汇 + `ch1WhaleStory` 派生 + `allStoryFlags()` + 收编海沟 `trench_found`。**焊死**：playthrough-story §5 扩成「全 src/data story.* 字面量 ⊆ allStoryFlags()」·teeth-tested。**31/31 全绿**·quirk #127。**编号 #124 跳开 auto/weekend 的 #122/#123**（main 因本 commit 分叉·折叠须 git merge 别 branch -f）。**遗留＝鲸落 step ②-⑤**（探索潜点+found 门/精简营地/目击 RNG+计数+残骸独立/图鉴+子区·需作者在场·SPEC §10）+ 坐标半径微调。详见 CHANGELOG #124。
> **2026-06-14 敌人库（Enemy Library）实装 + 入库工作流（交互 session·CHANGELOG #121 含同日续·作者「按顺序实装」→「全做」）**：把敌人升级成**可查询/可复用/目录自动加载/可被独立 session 安全产出**的库。落地：支柱三 目录自动加载（`gen-enemy-registry`→`registry.generated`·`combat.ts` 删手动 import·改 JSON 后 `npm run gen:enemies`）+ 支柱一 元数据 `bands/biomes/role/threatTier/codex` 回填 9 只 + `enemyLibrary.pickEnemy`/`resolveEncounterMember`（支柱二 enemyRef·route B 加法·非破坏）+ `--new-enemy` 脚手架 + `check-enemy-refs` **四门入 regress**（registry/引用/无孤儿/baseline）+ 入库 SKILL 草案（`docs/skills/`）。约定全做成 check＝**无新 quirk**。**regress 29→31 全绿**。周末 SKILL 已同步（加敌人走 `gen:enemies`）。**遗留**：route A（encounter 迁出敌人文件·纯重构）、模式 B schedule（作者立·SKILL 草案已备）、codex 充实 + 图鉴 UI。**batch-2 经作者授权破例提交 `auto/weekend`**（quirk #104 例外·随作者 auto/weekend→main 合并进 main）。详见 CHANGELOG #121（含同日续）。
> **2026-06-14 周末内容引擎·#129·midwater×4+vent×4+trench×2 第二扩批**：midwater 7→11 事件（海雪[cosmic]/会亮的水[cosmic]/过路的[uncanny]/无生之水[realistic]）+ vent 7→11 事件（温度的那边[cosmic]/每十七秒[cosmic]/云[uncanny]/白垫[realistic]）+ trench +2（导引线[twilight/realistic]+搁架[midnight/uncanny]）= **+10 事件（228→238）· +20 scenarios（353→373）**。midwater/vent 各从 0 cosmic 升到 2 cosmic；trench 补首个 realistic 锚点。regress **31/31 全绿**。无新 quirk。详见 CHANGELOG #123。
> **2026-06-14 周末内容引擎·#128·midwater+vent 氛围池扩批**：midwater 2→7 事件（假底/温跃层/管水母链/暗流下沉/灯笼鱼）+ vent 2→7 事件（雪人蟹/调节器化学味/管虫林/玄武岩裂口/白色不动生物）= **+10 事件（218→228）· +22 scenarios（331→353）**。灵感：深海散射层假底（DSL·lanternfish swim bladder sonar artifact）+ 温跃层视觉扰动（thermocline schlieren 磨砂玻璃折光）。regress **31/31 全绿**。无新 quirk。详见 CHANGELOG #128。
> **2026-06-14 区域揭示·配置化重写（交互 session·#120·作者多轮在场调）**：作者「大圈重叠成糊」从根修＝**数据驱动可跨章复用区域系统**——删 `BASE_LIGHT_RADIUS=0.72`(直径>全图)、改读 `chart_regions.json`+`engine/regions.ts`(每区 owner/palette/shape/radius)。v4 横向 6 区(家青·海岸半圆/残骸绿/中层蓝大/热液琥珀/海沟深蓝最右/鲸落紫占位)·「教学后只家区」由小半径自然涌现·只渲本图配置区(深脊柱不在 Ch.1 图)。新 `outpost.ch1_trench`(requiresFlag 非锚点门)+band。**per-哨站扫描**(`outpostScanSig` per-灯塔·只扫签名变的灯塔·动画播完才持久化=修「扫描秒消失」)·**发现门控**(`discoveredFlag`·章节前哨非恒显·dev「解锁大区」strip)·SSR 安全 ChartViewport·按钮白框/焦点环+海岸黑线修。回归 chart/lighthouse/smoke 重定基线·**29/29 全绿**·quirk #124-126。三摊相互依赖→合并为一个绿 checkpoint(含界面重构 #123+声呐 WIP·留作者本机 push)。**遗留＝鲸落区剧情(大特性·下个 session·SPEC §10「鲸落区·细化」)**：目击×3→探索潜点→找到鲸落区(flag-gated region·待扩 revealFlag/center)+精简营地(OutpostDef 功能开关变体)+残骸独立目击+图鉴+子区。详见 CHANGELOG #120 / 新 `docs/spec/深海回响_区域揭示配置化_SPEC.md`。
> **2026-06-13 章节哨站批·验收续（交互 session·#118 续·#119）**：接 #118 机制核（`20607fd`）作者线上验收三反馈。①**海沟开灯仍黑**＝诊断非 bug（vent 竖井首跳 14m·超灯/声呐 reach），作者拍**抬声呐 reach `SONAR_DEPTH_REACH` 14→22·MAX 26→30**＝声呐成深水的眼·竖井上半可读·最深仍需摸黑（守北极星）；剧情口径＝前期靠灯看事件·后期靠声呐看完整洞形。②**dev 一键解锁本区**＝`devUnlockChapterRegion`（置 tutorial_complete+锚点 flag+连推点亮·#110 家族）+ OutpostPanel ?dev 按钮。③**区域揭示视觉过 demo 定调**（作者「就是这样·实装更精致」）：家灯塔=海岸线半圆·章节哨站=离岸整圆·解锁点亮圈内可见·海图边界外扩·前期彩圈后期手绘背景图·暗点多数无/少数暗。新增需求两条入 SPEC §10：(A) 升级哨站搜更多暗点（属主实装块）/(B) dev 解锁（已做）。回归 `playthrough-sensors` §12 加 deep/abyss 节点重写·`playthrough-outpost` §14g。**29/29 全绿**。无新 quirk。**遗留＝区域揭示主实装块（下个 session）**：chart.ts 三态化+揭示圈（家半圆/哨站整圆）+边界外扩+首扫仪式(#80)+需求(A)+收编旧「深潜·蛙跳(试验)」band 列表与面板。详见 CHANGELOG #119 / 深水区 SPEC §10。
> **2026-06-13 章节哨站批·机制核（交互 session·#118）**：把 §10「章节哨站」拆成机制核（本批）+ 视觉演出（留作者）。AskUserQuestion 两 pin：方向「全做」（验收/档位制需作者在场）+ 章节蛙跳=**各自跳回本锅点区（新 band）**。落地：**三章节 band**（`band.ch1_wreck/midwater/vent`·order 11/12/13 与深脊柱分段·`depth_bands.json`）+ **三 OutpostDef**（坐标落锚点②③④区·3 阶段料账单 gold 升序·vent 激流·reef①由 home 灯塔覆盖不设哨站）+ **`OutpostDef.requiresAnchor` 双义**（解锁门：对应锚点节拍未到=「暗·不可建」/ 章节网标记：与深脊柱解耦）。引擎：`isChapterOutpost`/`outpostUnlocked`/`chapterOutpostForBand`/`isChapterBand`（lighthouses.ts·读 `ch1AnchorFlag`）；`advanceOutpost`/`canAdvanceOutpost` 锁态拒推进；新 `devAdvanceOutpost`（#110 家族·跳门跳料）；`startDiveFromOutpost` 加 `launchOutpostId` 显式起跳·`deepestOutpostLaunch` 跳过章节前哨（深脊柱逐字节不变）。UI（最小机械层）：深列表 `filter(!isChapterBand)`·OutpostPanel 锁态「暗·待解锁」+点亮出「蛙跳下潜」按钮+?dev 测试推进。回归：`playthrough-outpost` §0 加章节自洽 + 新 **§14** · `smoke-chart-ui` 新 **§S**。**29/29 全绿**·事件/敌人/combat 计数不变。quirk **#122**。**遗留（留作者逐拍）**：首扫仪式动画（#80 sweep）+ 海图可见性三态渲染（亮/暗/无·诚实轴）+ 海图 marker 点选蛙跳完整交互——机制核已就位可直接挂。详见 CHANGELOG #118。
> **2026-06-12 晚 St1 一章 MVP + medkit 上架 + 负伤 §12 二批（交互 session·#117·三 commit `8907275`/`b3efc20`/`5a33364`）**：**(1) medkit 治伤+上架**——`consumable.medkit` 旗标（data-driven 同 decoy.kind）：use_medkit 回体力之外对每处伤按 `heal.medkit` 生效（「全部能治的一起处理」作者拍·整包结算 `applyMedkitHeal` 住 injuries.ts 唯一写者·止血⇒scent 锁定自动解）；Mira 上架 sellPrice 25→买 40·备货 2；combat 17 条 + economy/smoke 基线重抄。**(2) St1 一章 MVP**——四锚点 POI 沿 #69 模板上海图（新 `ChartPoi.story`·任意顺序·vent 需前三齐·**story 坐标=已知点不走灯塔轴/不穿 mimic tell**）+ 新 zone ×2（`open_midwater` layered/`vent_trench` maze 井+廊）+ 事件 208→217（四节拍+观测站圆满〔当时为真·四 tells 低调〕+两氛围池）+ 原创鱼四种 lore 形态；playthrough-story 新 §5 守门·chart 锚点 5→9·scenarios 309→**331**。**(3) 负伤 §12 六条全过**——**12.1 作者推翻两档封顶⇒档位制重设计立项**（每伤种自有档数+转移路径·留专门 session）；12.2 scent 名单=鲨/梭鱼/盲鳗+幼体随亲代（数据已落）；12.3 panicked 联动暂不叠；12.4 瘫痪爬到点亮前哨=算获救；**12.5 全部文案改「事后挑错制」**（quirk #120·不再前置过稿）；12.6 失真配比留二章。作者同日剧情 SPEC 七/八/九批（章节五槽/会师层灵力·AI/三章双路流程）随 docs 入库。regress **29/29 全绿**。quirk **#120/#121**。**遗留：作者合并验收顺延（动线清单已交·#116+本批 5 commit 待 push）**。详见 CHANGELOG #117。
...
- [x] **敌人库（Enemy Library）· 基础+解耦已实装（2026-06-14）·少量余项** —— 把敌人从"散文件 + `combat.ts` 手动 import + 无元数据"升级成**可查询 / 可复用 / 目录自动加载 / 可被独立 session 安全产出**的库。两原则：**数据驱动默认**（纯数据敌人＝零引擎改动·只有新机制原语才碰引擎·边界表 SPEC §3）+ **可维护/可扩展优先**（元数据全可选 additive·不入存档）。**已落地**：支柱三 目录自动加载（`scripts/gen-enemy-registry.mjs` → `src/data/enemies/registry.generated.ts`·双运行时安全·改 JSON 后 `npm run gen:enemies`；combat.ts 删 7 手动 import）+ 支柱一 元数据 `bands/biomes/role/threatTier/codex` 回填 9 只 + `engine/enemyLibrary.ts::pickEnemy`/`matchEnemies`/`enemyThreatTier` + `scripts/check-enemy-refs.mjs` **四门入 regress**（registry 不过期 / 引用完整 / 无孤儿 / 有 baseline）+ `combat-runner --list-enemies --band/--biome/--role/--threat-tier` 过滤 + 回归 `playthrough-enemy-library`。**regress 29→31 全绿**。约定全做成 check＝**无新 quirk**。**已续（同日「全做」）**：支柱二 `enemyRef`（route B 加法·`resolveEncounterMember`/`canResolveMember`·startCombat 接入·check 加 enemyRef 可解析门）+ `--new-enemy` 脚手架 + §5 入库 SKILL 草案（`docs/skills/`）。**待续**：① route A（`combatEncounters` 迁出敌人文件·纯重构·非必须）；② 模式 B 的 schedule（作者立·SKILL 草案已备）；③ codex 充实 + 图鉴 UI。设计/进度源 `docs/spec/深海回响_敌人库_SPEC.md`（§3 边界 / §5 工作流 / §7 清单 / §8 开放问题）+ CHANGELOG #121（含同日续）。（设计 2026-06-13·实装 2026-06-14·「按顺序实装」→「全做」）
