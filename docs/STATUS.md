...
> **2026-06-27 战斗「拳脚扭打」改兜底攻击（`fallbackOnly`·交互〔Cowork·Opus〕·#223·新 quirk #192·不 bump SAVE·沙箱 my-lane 全绿〔`--skip build` 64/65〕·待 push）**：作者问「拳打脚踢是否多余」。审定 `action.fist` 被起手刀严格压制却常驻菜单＝死按钮 → option 1 改真·兜底（`fallbackOnly:true` + 单判据 `hasUsableWeaponAttack`〔只看持有不看体/氧〕·`listAvailableActions` 末尾门控）。**只动可见菜单·不动可用性**（checkActionAvailability/applyPlayerAction 不读它 ⇒ scenario replay 零影响·quirk #192）。新 `playthrough-fist-fallback` 守四分支〔持刀隐藏/赤手露出/持枪无弹露出/带弹复隐〕。**⚠ 预存红（非本次·已证实 baseline HEAD 同样红）**：`playthrough.ts`（legacy 教学事件链 walk）报「应在上浮，实际 dive」＝#222 node 化漏迁这条非-node-aware walker（descent→grouper/wreck→deeper 成节点边界）·作者拍「留作者修」（升级 node-aware 或退役·**会卡 nightly 发布门**）。push 待 Mac/nightly。
