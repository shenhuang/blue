...
> **2026-06-21 自动试玩 sim 工具 + findings + 月度 schedule（交互 session〔Cowork〕·#173·无新 quirk·typecheck 绿·纯 docs+tools/·不碰 src）**：真引擎驱动「理性玩家」机器人 ~2200 潜 → `tools/playtest-sim/`（`run.sh` 随时跑·自适应 esbuild）+ `docs/playtest-findings.md`（P0/P1/P2 backlog）+ schedule `blue-playtest-sim`（月度兜底 + 手动）。要点：避战软锁(P0-1)·刷子瓶颈=遭遇稀非每杀少(P1-1)·shark_tooth 只教学掉一次(P1-2)·深区零产出 + stalker 借浅怪(P1-3/4)·开阔区氧过剩(P2-1)·**海沟 station 终局不可达〔`trench_found` 无人置位·疑 St1/St2 留白〕(P1-8)**·spare_tank 未定义(P2-4)。findings 基于旧 HEAD·main 已推进需重跑刷新。
...
> **2026-06-21 海图 POI 调试器 ChartViewDevPanel（`?editor=chartdev`）+ SSR smoke（交互 session〔Cowork〕·#170·无新 quirk·commit `525e73b`·47/47 绿）**：在 dev 工作台地图组新增 POI 调试面板——可视化所有 chart_pois.json POI 的揭示态（lit/dim/hidden）·reach（effectiveDistance）·深度/洋流/能见度修正标签；左栏 runsCompleted+outpost 控制组·右栏 10 列 sticky 表格；全只读引擎调用；SSR smoke parity 对齐。PSM worktree 已合并进 main（ff-merge 绕坑走 git show+cp+显式 add·同 quirk #148）。## 1. 一句话状态
