...
> **2026-07-17 #311 猩红暴君 boss 机制层 + openwater 持久化机制层——两 SPEC 并行推到底（Cowork·Opus 编排 + 6 subagent 无重叠车道·regress 沙箱 87/87 绿〔build 环境跳〕·新 quirk #257/#258/#259·commit 待定·push 留 Mac/nightly）**：handoff 待办 #1（猩红暴君·`enemy.scarlet_tyrant` 新 + `combat-scarlet.ts` 吃活同伴夺词条/运行时去重/波次 1→3→4→5/第五波剧情杀 + baseline·头足非古文明·词条全靠吃 5 封顶）+ #2（openwater 持久化·`caveMaps→diveMaps` 泛化 + `getPersistentTarget`/`startDiveIntoPersistent` 分叉收口〔cave 逐值等价·openwater 复用 `generateDiveMap` 冻结〕+ `ItemDef.deposit` 守门·SAVE 16→17·quirk #257）并行落**机制层**。**路线 P（作者拍板 boss 主线接入）**：boss anchor 退回纯占位〔去 story 块·带 story 块＝主线 beat 触发整套可达契约·#300 空主线必 fail·warren 先例〕·story-pin/发现门/章尾/打赢 flag/hunter 生产接通随 #300 主线重建 defer（quirk #258）。整合修 3：kinslayer 缺 `scarletFeed`〔互吃预演哑火〕/ Lane A 漏 4 处硬编 `SAVE_VERSION` 断言（quirk #259）/ mainline 门。**验证**：typecheck + `bless:combat` 无漂移（48/48）+ 全量 regress **87/87** + 对抗复审 8 点**无必修**〔flag：boss `biome:midwater` 随机隔离实靠「无 midwater enemyRef」；wave5 仅首攻触发〕。build 沙箱环境跳（缺 rolldown linux binary·nightly 跑）。边界：机制层全落·boss 主线接入随 #300 & openwater 世界投放 Lane C 各自 defer。树：main·commit 待定·push 留 Mac/nightly。
> **2026-07-16 #310 开阔水域 SPEC 回填「贴底节点」机制已实装 + 修 §8 Phase 1 过期陈述（Cowork·Opus·docs-only·沙箱全量 85/85 绿·无新 quirk·commit 待定·push 留 Mac/nightly）**：`npm run handoff` 待办 #2（轻·可 Sonnet/low）——#307 只改代码没回改 SPEC，本条对齐 `docs/spec/深海回响_开阔水域_SPEC.md`：§4 新增 `4a. 贴底节点机制`（`seabedNodeIds` 单一真相 / `atSeabed` 门 / `midwater`=floorless / `playthrough-seabed` 验收 / 内容池空池待作者监督）+ zoneTag 集补 `atoll`（#305·quirk #254）+ §3 内联锚点标记 + **§8 Phase 1 修过期陈述**（原「未提交」实为 `b469431`/#302 早入 main·补 `8a. 已落地进度`回填 #302/#304/#305/#307 git 真值 + 未落项）。docs-only 单文件 +24/−4·四文档门 + 全量 regress 85/85 绿。树：main·commit 待定·push 留 Mac/nightly。
> **2026-07-15 #309 交接文件 `NEXT_SESSION_PROMPT.md`→`docs/HANDOFF.md` 改名 + `npm run handoff` 末尾内联它（Cowork·Opus·沙箱全量 85/85 绿·commit 待定·push 留 Mac/nightly）**：作者要「下 session 说『读/跑 handoff』拿到一切」。命令名不动（quirk #96 深引），改人写方向文件名 + 让 `handoff.mjs` 末尾内联 `docs/HANDOFF.md`（`existsSync` guard·>120 行截断）⇒ 一条 `npm run handoff`＝git 定位＋人写方向。tracked 提交改 `.gitignore`/QUIRKS #96/STATUS/`handoff.mjs`/`check-quirk-refs` SKIP_REL/`regress.mjs`·`zones.ts` 注释/开阔水域 SPEC（`CLAUDE.md` 同改·但 gitignored 只落本机）；历史旧名（CHANGELOG/worktree）不碰。**作者侧**：`blue-closeout` skill 第 7 步文件名要在 Settings>Capabilities 改成 `docs/HANDOFF.md`（我改不了 skill 缓存）。树：main·未提交·push 留 Mac/nightly。
## 1. 一句话状态
