...
> **2026-06-21 shark_tooth 正常可获取（P1-2）+ 材料刷点范式 openEventPool 轮替（交互 session〔Cowork〕·#161·新 quirk #150）**：常规 reef 加鲨鱼入口（`reef.reef_shark` 入 `[reef]` 池 + `combat.reef_shark_solo/_pair` 复用现 def 不造数值）⇒ 鲨鱼牙不再只教学掉一次；新字段 `ChartPoi.openEventPool` ＝数据驱动「专门刷点」（入潜按 `runsCompleted` 轮替取 ≥3 个专属 beat 之一·"能刷但别反复同一段"），首例 `poi.anchor.reef_shark_shoals`（3 beat）。约定落成机制：`check-farm-pois.mjs`（挂 anchor/≥3 beat/引用可解析/beat 专属/与 openEventId 互斥·接 regress）+ `playthrough-farm-poi.ts` 钉轮替。沙箱 typecheck+6 静态门+事件 287/287+farm-poi/chart/story 全绿；唯一红＝作者并行未提交 boss WIP（零重叠·不在本 commit）。
> **2026-06-21 记忆 consolidate 落空→失联 `[[wikilink]]` 操作流程提升成 QUIRK #147/#148（交互 session〔Cowork〕·#158·无代码改动）**：auto-memory 层（`MEMORY.md`+topic 文件）本 Cowork 沙箱不可达（#157 同日刚编辑过·在作者机器某处·没挂进沙箱）；审计 6 个失联 `[[wikilink]]` vs git——`deep-game-vision`→深水区 SPEC·`weekend_content_log`→CHANGELOG·常驻警告全在 QUIRKS·无唯一副本丢失；唯二无汇总 QUIRK 的操作流程 `[[blue-regress-sandbox]]`(23 refs)/`[[sandbox-git-commit]]`(10) 落成 **#147**（沙箱 esbuild 双跑 regress）+**#148**（沙箱 git/.lock 操作汇总）。作者拍：不重建平行 non-git memory 层·靠 CLAUDE.md+`npm run handoff`。docs-only·纯-node 门绿·作者 boss WIP（`enemies.ts`）未提交不在本 commit。
> **2026-06-21 起手定位瘦身（交互 session·#157）**：`npm run handoff` 输出 56KB→8.7KB——CHANGELOG 末尾段改「只打最近 12 条加粗标题」（正文留 `docs/archive/CHANGELOG.md`·`recentChangelogTitles`）+ MEMORY.md 索引四条巨墙压成单行 hook + 归档已完成 `weapon_system_impl_prompt.md`。handoff.mjs 不在 build/test 路径·源码零改·无新 quirk。
