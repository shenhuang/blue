...
> **2026-06-21 海图 POI 调试器 ChartViewDevPanel（`?editor=chartdev`）+ SSR smoke（交互 session〔Cowork〕·#170·无新 quirk·commit `525e73b`·47/47 绿）**：在 dev 工作台地图组新增 POI 调试面板——可视化所有 chart_pois.json POI 的揭示态（lit/dim/hidden）·reach（effectiveDistance）·深度/洋流/能见度修正标签；左栏 runsCompleted+outpost 控制组·右栏 10 列 sticky 表格；全只读引擎调用（`poiRevealState`/`effectiveDistance`/`describeModifier`/`ownerAnchorPos`）·owner-anchored 坐标自动 resolve。SSR smoke：①渲染不抛错 ②parity 对齐（reef lit·reach=2·修正标签·前哨 hidden）。PSM worktree 已合并进 main（ff-merge 绕坑走 git show+cp+显式 add·同 quirk #148）。
...
> **2026-06-21 战斗 dev 面板：实战预览（真实 CombatView）+ 新战斗字段 + 可视化 + SSR smoke（交互 session〔Cowork〕·#168·无新 quirk）**：`?editor=combat` 面板加「实战」模式——抽出 `enterCombat` 单一进战路径（批处理 `runCombatScenario` 逐字节不变·combat-scenarios 26/26），新 `buildCombatEntryState` 只造到开战相位、喂真实 `<CombatView>` 反应式打（活 RNG）；phase 构造留引擎守 check-boundaries 规则二·`CombatDevPanel` 引用 `CombatView`＝dev→game（#152 允许向）。右栏模式条批处理↔实战翻成 `[⟲ 重开][退出实战]`·换遭遇/切模式/改 party 自动退实战。表单补 `bonuses`（#164·quirk #151）/`wornSkin`（#162）/`injuries`（§10）。新 `smoke-combat-panel` 接 regress。typecheck 0·46/46 绿·build 留 Mac/nightly。
## 1. 一句话状态
