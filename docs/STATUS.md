...
> **2026-06-30 升级引用门 `check-upgrade-refs` + 删死 `unlockShopItem` 通路：起手 4 路 Explore 扫 backlog→核完确认交接「全 defer」基本成立〔agent 翻出多为 stale·F4/economy #238-239、B-2 #241、声呐 SDF、猎手 P1、playthrough flake #129、lunar MVP 已 landed〕·唯一真·非-defer 独立项＝P2-4〔`item.spare_tank` 未定义〕。起底双重失效：item 未定义〔`check-data-schema` 不跨文件核引用·静默漏过〕+ `unlockShopItem` 机制死〔itemId 只 add 进 `bonuses.unlockedShopItems` Set·全仓无人读·Mira 走 `port.ts::SHOP_STOCK_*` 表〕。作者拍 R〔删死·非接线做实〕：删 effect/描述/联合成员/派生字段/UI case/playthrough log·新 `check-upgrade-refs` 门把「升级 itemId 引用悬空」整类钉成 regress 红〔补 data-schema 缺口·先跑即抓 spare_tank〕（Cowork 交互〔Opus〕·#242·新 quirk #202·不 bump SAVE·沙箱 `npm run regress` 78/78 全绿〔--skip playthrough 41 + --only playthrough 37·tsx 跑·build 留 Mac·#147〕·fix commit `cdac718` + 本条收尾·只显式 add 8 文件·不碰同树 untracked `docs/infra/art-pipeline-bootstrap.md`〕）**：教训→quirk #202：残留扫漏 `scripts/`〔playthrough 不在 typecheck 范围·删派生字段全仓扫·全量 regress 兜〕。遗留：push 待 Mac/nightly·`art-pipeline-bootstrap.md` untracked〔另一 session·未 add·留作者处置〕·P2-2 潜中回氧件/St2 剧情/F5 tier〔#233①〕/τ=4〔#233②〕仍 defer。
> **2026-06-29 B-2 三前哨「通电点亮」末阶连接句过稿落定：主线柱迁移〔#234〕引入「建前哨→该区主线坐标灰〔看得到去不了〕转亮〔下得去〕」连接节拍·三处末阶 narrative 占位由作者过稿挑「变体 A」×3 落定〔残骸=日志第二条坐标／中层=第三条／热液=最后一条·去 `[占位·待过稿]`〕（Cowork 交互〔Opus〕·#241·无新 quirk〔语调早由 `check-protagonist-voice` 机制化〕·不 bump SAVE〔纯文案串〕·沙箱 `npm run regress` 77/77 全绿〔tsx 跑·build 留 Mac·#147〕·content commit `b80d37d` + 本条收尾·只显式 add `lighthouse_upgrades.json`）**：执行交接 todo #1〔#234 遗留〕。改动：去抒情〔删「那一刻」/「第一次」〕·点出灰/亮＝看得到→去得了·删中层与第二阶段重复的「有了标记」·修热液拗口句·坐标统一「日志第 N 条坐标」+「海图」转亮。过 `check-protagonist-voice`〔2064 段〕+ `check-terminology` + `check-data-schema`。遗留：#234② 高清大图 `ART[id]` 现空〔defer〕·F5 tier 标号〔#233①〕/τ=4 手感〔#233②〕仍 defer·push 待 Mac/nightly·build 留 Mac。
> **2026-06-29 统一道具图标系统 land（作者「收获/物品图标」WIP 收口）：`ItemIcon` 分层解析占位 glyph〔`GLYPH[id]` 63 专属→槽/role/类目兜底·新道具自动有图·`ART[id]` 预留高清大图位〕+ 潜点「可能收获」无剧透材料 chip〔`poiHarvestMaterials` 单 filter `category==='material'`〕+ 四 UI 接入点〔CorpseView/EquipmentDoll/ItemCell/SeaChartView〕+ 新 `check-material-icons` 门〔Rule A GLYPH key 真道具/Rule B 4 role 配色齐〕（Cowork 交互〔Opus〕·#240·无新 quirk·不 bump SAVE·沙箱 `npm run regress` 77/77 全绿〔tsx 跑·build 留 Mac·#147〕·feat commit `334177a` + 本条收尾·**只显式 add 11 文件·非 -A**）**：作者跨 #236–#239 同树在途 WIP·本 session 验证完整无缺口后收口 land。收口顺手改 `regress.mjs:118` stale 注释〔F4 已 #239 提硬门〕。flake 注：首跑 76/77〔combat-scenarios `fissure_sphere__split_trigger`·quirk #22 并发全局 Math.random〕→隔离 3/3 + 全量重跑 77/77 全绿·与 icon 无因果。遗留：高清大图 `ART[id]` 现空〔挂载点就位〕·B-2 文案〔#234〕·push 待 Mac/nightly·build 留 Mac·F5 tier 标号〔#239①〕/τ=4 手感〔#233②〕仍 defer。
> **2026-06-29 F4 稀疏软警告→硬门：`auditReachability` 里 F4 `warnings.push`→`violations.push`〔#238 遗留①·#201 预告的一行提升〕+ `toMermaid` 加 `badRegions`〔F4 区域级违规·无 itemId/where→原 emit 不渲染·补红 `✗F4`〕+ `check-economy-reachability` 文档/print 把 F4 移硬门段〔warnings 现 F5-only〕+ 新 `scripts/__tests__/economy-dag.test.mjs`〔check-tooling 自动发现·2 跨区门不红/3 跨区门一处 violation·断言落 .violations 非 .warnings·+ 真实数据金丝雀〕（Cowork 交互〔Opus〕·#239·无新 quirk〔更新 quirk #201 F4 软→硬〕·不 bump SAVE·Mac `npm run regress` 78/78 全绿〔含 build+46 tsx〕·沙箱 31/31·commit `8911ccb`〔feat〕+ 本条收尾·**只显式 add 我的 5 文件·不裹同树图标 WIP**）**：起手核实纠 stale——交接称「trench 现 3·提硬门 main 会红」实测 reef0/wreck0/vent1/midwater2/trench2 无一 >2〔密度 pass 已在 #237 落地·跨区门早 ≤2〕→提硬门当下即绿且锁住稀疏。E2E 负证：真实 dag 注 trench 第 3 条跨区门〔lantern_gland〕→F4 violation·check 退 1〔red ✓〕。遗留：F5 两处 tier 标号〔bluecave_geode/wreck_bronze·defer〕·同树「收获/物品图标」WIP 作者在途〔含其 dirty `scripts/regress.mjs` line118 注释仍写软警告〕·push 待 Mac/nightly·B-2 文案〔#234〕/τ=4 手感〔#233②〕仍 defer。
