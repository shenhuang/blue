...
> **2026-06-12 剧情路线图 St0「剧情脊柱」落地（交互 session·#115）**：`engine/story.ts` 最小版——章节/节拍**从 profile.flags 派生**（`story.ch1.*` 口径只在本模块生成·教学钩+四锚点节拍位+圆满/留白结局位+`chapterUnlocked` 占位判定·#66/#69 additive 套路不 bump SAVE_VERSION）；教学关植入**「半本日志」开场钩**：新 `tutorial.prologue`（两选项都种 `story.ch1.hook`）开场**种**导师/日志/不信失联，`tutorial.ending_log` 重写「两本日志」结尾**收**（船长段原文保留改归属陌生船长·与导师日志互证·四个坐标圈上海图=海图解锁同一动作）——一钩两扣不重复埋；**canon 对齐父亲→导师全库 7 处**（lore id 改 `lore.ch1.captains_page`·清单见教学关剧本 SPEC §8）。新 `playthrough-story`（派生/港口无 run 路径/round-trip/canon+字面量守门）+scenarios 307→309+verify-tutorial 从 zones.json 再生起始事件。regress 28→**29 任务全绿**。quirk **#118**（story flag 单一来源+canon 守门）。**教学新文案全 [待过稿]**（quirk #117·待作者过稿）；下一步=**St1 一章 MVP**（四锚点 POI+锚点链+圆满 tells——需作者在场）。详见 CHANGELOG #115。
