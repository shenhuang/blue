...
61. **深水区 Phase 1 plumbing：可扩展纵向深度轴（band 阶梯 + 蛙跳下潜，2026-06-03）**——「很大的图」的脊柱第一段。SPEC §5 Phase 1 + §12 清单（已勾 plumbing）源真、§10 决策日志记拍板。
    ...
    - **Deferred（明确留后续，见 SPEC §12）**：trench 专属 zone/事件/band 级 tag 池（现借蓝洞、深 cave 事件多 ≤60m 故 trench 事件稀）；~~范围/分辨节点级 clarity~~（**已实装 #62**）；真前哨蛙跳（Phase 2 持久前哨建成后换掉 home stand-in）；band 级 alert 倍率/不饱和成本曲线。**下一步＝Phase 2 跨 run 前哨/能源、或深段内容、或 Phase 1 续（band 级 alert 曲线）。**
...
62. **深水区 Phase 1 续·节点级 clarity：深度分档 + 范围/分辨升级轨（2026-06-04，填 #60/§12 明确 deferred 的"范围/分辨"钩子）**——把 0a 定的 run 级二元 clarity 升成节点级。SPEC §12「范围/分辨」（本次勾）+ §10 决策日志（2026-06-04 条）源真。
    ...
    - **Deferred（续 SPEC §12）**：band 级 alert 倍率 / 越深越狠不饱和成本曲线（现 `ALERT_DEPTH_FULL` 60 饱和）；trench 专属内容（Phase 1 占位）；真前哨蛙跳（Phase 2）。**下一步＝Phase 2 跨 run 前哨/能源、或深段内容、或 band 级 alert 曲线。别预先触发 d_reveal（#42）、别加第三只常规敌人。**
