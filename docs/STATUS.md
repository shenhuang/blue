...
> **2026-06-14 探深「深度柱」重构·废老蛙跳/深脊柱 → 每座灯塔一根数据驱动深度柱（交互 session·#130·quirk #131·未提交留作者 push）**：旧「一脊柱·一级一个 `flag.probe.*` 点·前哨蛙跳」→ **每座灯塔一根向下深度柱**（建到第 K 级探深 → 档 1…K lit / K+1 dim〔暗点〕/ 更深 hidden·一级露一档·勘测暗圈的垂直版·北极星「灯塔=信息基建」）。**单一数据源** `data/depth_columns.json`（5 柱·家2/残骸3/中层4/热液4/海沟6·**新灯塔只加一条 column 即有柱**）；`engine/columns.ts`（纯叶子）派生 band（`columnBands`→bands.ts 合并 getBand）+ probe 升级轨（`columnProbeTracks`→lighthouses.ts 合并·onlyLighthouse·canBuildAt/面板**零改**消费）+ 海图深入 POI（`buildColumnPois`→chart.ts 注入·`depthTierRevealState` 档位制·教学门+宿主门）。下潜走 `startDiveFromPoi(bandId)`→`diveIntoBand`·column POI 并入**宿主前哨在线补给**（能源保留·#128）。**删** startDiveFromOutpost/deepestOutpostLaunch/isChapterBand/chapterOutpostForBand/OutpostDef.bandId/深脊柱 4 前哨+4 建造事件/旧 5 probe 轨+flag.probe.*/深脊柱浅段+4 章节 band/11 poi.deep.*/4 深脊柱区；**留** abyssal/hadal/subhadal/nameless 作 Phase 3「另一个世界」脚手架（海沟柱故意止于 ~108m·d_reveal 别擅自动）。`check-dive-refs` 重写（柱配置 + advanceOutpostId 在册防回流）+ 新 `playthrough-columns`（端到端档位揭示）。**SAVE 4→5**。`npm run regress` **33/33 全绿 + build 绿**。新 quirk **#131**。**数值/级数收尾已在 SPEC §10 定案**（同日设计讨论·家2/残骸3/中层**6**/热液4/海沟**4**·海沟第4级=科考站电梯 capstone·删预留 band·**待下个 session 实现**·代码仍是 #130 占位）。SPEC＝`docs/spec/深海回响_探深深度柱_SPEC.md`。详见 CHANGELOG #130（含同日续）。
