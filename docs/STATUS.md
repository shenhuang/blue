...
> **2026-06-19 Otto 装备升级系统 段1（交互 session·#144·接 #139/#140·物品栏与装备 SPEC §4·commits `1569d2e`/`f27d5f3`/`9b7f7a2`/`faa4ebc`/`cf60398`）**：作者拍装备架构后落地段1。9 槽纸娃娃〔潜水衣/气瓶/潜水灯/声呐/武器主·副/饰品×3·**去护甲**·气瓶库=beacon、打捞行会=Mira 非装备〕；B→C 合并走**分段 A**（实力单一来源=穿戴件）。**段1 地基**：`EquipmentSlot` 加 sonar/charm2/charm3（留 tool/ranged/charm 历史 key）；新 `UpgradeStep`+`EquipmentMeta.upgradeSteps`；`engine/equipment.ts`（纯叶子·单点）`getEquipmentStats` **只累加 upgradeSteps 增量**〔不碰 base·starter=0·基线零扰动〕+ `canUpgrade`/`upgradeEquipment` 账单·`getRunBonuses` 并入装备氧/体；气瓶试点（占位数值）·**无 SAVE bump**。**UI**：`EquipmentDoll` 纸娃娃＝**Otto 改装**（新 `PortServiceMode 'upgrade'`·Otto NpcCard 入口）；物品栏「装备」tab 改**平铺**所有装备（已装备 variant `'equipped'` 加框）；`ItemCell` 改**正方形**（icon 铺满·name 底部居中条·equipped 框在边缘·纯 CSS·商店/战利品/装包全跟变）；港口出口 `物品栏`+`海图` 改 sticky 常驻底部（海图置底·NPC 卡片随页滚）。守门新 `playthrough-equipment`。regress **40 非 build + prod build 全绿**。新 quirk **#140**。**遗留＝段2**（需作者在场）：三传感器线（声呐/灯/规避）迁进件 + 退役 upgrades.json + SAVE 6→7 + 数值对账；下潜「查看装备」接 HUD；饰品 2/3 解锁；数值占位待调。详见 CHANGELOG #144。
> **2026-06-19 鲸落「物品即解锁」两层（交互 session·#143·接 #137/#141/#142·commit `8f456c3`）**：作者要「作弊买鲸落手记道具＝完整解锁鲸落区」。两层加法（没撕 #117/#137）：①`story.marksPois`⇒reveal——持有标记某 POI 的文献即揭示该坐标（`engine/items.ts::poisKnownFromItems`+`chart.ts::documentKnowsPoi`·`poiRevealState` 绕发现门·仍受能力/天气门·`isPoiLit`/`isPoiExplainedByLighthouse` 认它不误穿 mimic）；②`story.setsFlag`⇒里程碑——获得道具时把 flag 并入 `profile.flags`（单点 `state.ts::acquireIntoProfile`·`port.ts` 三入袋路径含 `devGrantItem` 作弊全改走它）开鲸落区圈/营地/握手。新 `item.whalefall_log`（setsFlag whalefall_found + marksPois 3 生态点）·`whalefall.found` loot 发它（取消该文件旧「全 loot-free」）·导师日志（已 marksPois 四锚点）自动成真解锁源。守门：marksPois⊆authored anchor（chart §0b）·setsFlag 被 story §5b 自动守·行为焊 whalefall §3/§4/§7。regress **39 非 build + prod build 全绿**·无 SAVE bump·新 quirk **#139**。详见 CHANGELOG #143。
> **2026-06-19 导师半本日志（文献坐标迁移+displayCoord）+ 港口事件发物 + 结算页换行格子（交互 session·#142·接 #141·commit `8328148`）**：#141 遗留「坐标移日志道具」+ 作者新结算需求。新 `item.mentor_logbook`（剧情道具·四锚点 marksPois 从 `old_chart` 迁入·正文 `lore.ch1.mentor_logbook`·旧海图回退＝描述+「摊开海图」）；`tutorial.ending_log`+`ending_safe` 两结尾发放（`Outcome.loreEntry` string|string[] 一拍两条·`ending_safe` 不解锁船长 lore）。`applyOutcome` 接通**港口拍发物**（无 run→`profile.inventory`）。`MarkedPoiInfo.displayCoord`+新 `formatChartCoord`（×100·1 位小数·单一来源·从解析后绝对 mapX/mapY 派生）；`LockerView` lead-in「最后是 N 个坐标」+ **坐标整行可点直接跳海图**（删独立按钮·旧海图按钮改「摊开海图」）。`ResolutionView` 带回改 **ItemCell 格子换行**陈列（`outcome.loot`·不按来源分组·作者「全都一起显示」；曾实装 `lootLog` 分组归因·作者拍不要后整套删净）。守门：`smoke-chart-ui §T` 重写 + `playthrough-story §2/§2b` 港口发物门。`regress --skip build` **38/38** + prod build 绿。无新 quirk。**遗留**：教学叙事复审（Task B·需作者读）+ P3 Otto 升级（需作者在场·SAVE 6→7）+ 文案待过稿；待作者过目：日志两结尾都发、结算带回含携带件。详见 CHANGELOG #142。
> **2026-06-19 物品栏「剧情」tab + 武器近战/远程双槽 + 公会浮标→旧海图(教学叙事重写) + 文献正文/坐标功能（交互 session·#141·接 #139/#140·commit `9a4f981`）**：验 #140 绿（持久装备全 dive 路径 thread·hydrate 单点）。物品栏「日志」tab→**剧情**（陈列 `category:'story'` 道具：文献〔有 `unlocksLoreEntry`·点开显见闻正文·注册 `lore.ch1.captains_page`＝船长日志〕+ 信物指南针）；装备件（含未装备备件）归**装备 tab**（新「未装备」只读区·非「其它」）；「其它」改单格栅。武器拆 **tool=近战(潜水刀)** + 新 **ranged=远程**（additive·合并 hydrate `{...starter,...equipment}`·不 bump SAVE·6 处 loadout 字面量补 ranged）。**公会浮标→旧海图**（item id `marker_buoy`→`old_chart`·flag `buoy_obtained`→`chart_obtained`·教学叙事重写：沉船捞旧海图·删「还给公会」·待作者复审文案）。**文献正文/坐标功能（可复用·藏宝图同轨）**：道具 `story.opensChart`/`marksPois` + 引擎 `itemOpensChart`/`itemMarkedPois`/`resolveMarkedPois` + `LockerView` 详情显正文 + 海图坐标列表（可达点→「在海图上查看」→`onOpenChartAt`→`SeaChartView.focusPoiId` 跳选）；`check-dive-refs`(k) 守 marksPois 引用。`node scripts/regress.mjs --skip build --concurrency 8` **38/38 全绿**·build 留 nightly。无新 quirk。**遗留（下个 session·作者拍）**：坐标从旧海图移到**「日志」道具**（导师半本日志·实装为起始道具·文案+四坐标）·旧海图回退＝描述+摊图·坐标显示 `XX.X,Y.YY` 从 `mapX/mapY` **派生**·点坐标跳海图(已实装复用)。详见 CHANGELOG #141。
## 1. 一句话状态
