...
> **2026-06-20 NPC 进度对话续作：Mira 材料提示/卖灯退场 + Otto 声呐提示（交互 session·#147·接 `90d82e9` Aldo）**：照 Aldo 范式补 Mira + Otto。Mira 新增三条进度对话（卖灯推销·退场门 `notHasFlag(flag.owns_light)`；礁石/残骸材料提示·onEnter 一次性 setFlag 退场）；Otto 从硬编码 NpcCard 升级成完整 NPC（`otto.json` 新建·声呐提示对话·`notHasFlag(flag.owns_sonar)` 退场门·`dialog.ts`/`PortView.tsx` 接线）。`craftEquipment` 补 `itemSetsFlags` 路径（打造件与购买件等价置 flag）；`item.light.hand_torch`/`item.sonar.handheld` 各加 `story.setsFlag`。`playthrough-story §4c/§4d` 守 Mira/Otto 门控自洽。regress **42/42 + build 全绿**。无 SAVE bump·无新 QUIRKS 编号。注：工作树有作者并行的装备 base/换装 WIP（EquipmentDoll/LockerView/combat.ts 等·含 QUIRKS #142 草稿）·本 session 只提交 7 文件+docs。详见 CHANGELOG #147。
