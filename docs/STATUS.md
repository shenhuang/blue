...
> **2026-06-18 SPEC §9 五问拍板 + 持久装备字段 + 海图出发三步流（交互 session·#140·接 #139·commit `0a6eee7`）**：拍板 5 个 SPEC 开放问题：灯具/声呐均是装备（点槽换同类·升级移交 Otto P3）；charm 暂空（P3）；日志 tab＝图鉴+航海志（`kind='journal'`·按 group 分册）+见闻志（`kind='lore'`）；任务 tab 本期不放。引擎侧：`LoreEntryDef.kind` 字段（`lore`/`journal`）；`PlayerProfile.equipment?` 持久字段（hydrate `?? createStarterLoadout()`·无 SAVE bump）；`createNewRun`/`hydrateGameState`/`dive-start`/`dialog` 传装备。UI 侧：`LockerView` 读持久装备 + 日志 tab 按 kind 分区（航海志按 group 分册）；删 `SLOT_LINE`/`UpgradePanel`（归 Otto P3）。`SeaChartView` 新增 `departStep` 三步流（none→pack→target）；`ChartInfo` 具名导出（smoke 直渲任意步）；`smoke-chart-ui` P1/F 重写。`npm run regress --skip build` **38/38 全绿**·prod build 留 nightly。无新 quirk。**遗留**：P3 Otto 升级系统（`ownedEquipment`/`OttoUpgradePanel`/`upgradeSteps`·SAVE 6→7）+ target 步精选逻辑 + 换装栏 + charm。详见 CHANGELOG #140。
## 1. 一句话状态
