...
> **2026-06-09 声呐有机洞穴渲染 ④ port（交互 session·作者逐图验收 v3·feat `82b4368`）**：#100 把声呐图重做成 canvas 洞穴后，④「canvas 画对」待作者肉眼——本 session headless 离屏渲染（真 mapgen+layout+**真导出 `caveSdf`**·PPM→PNG）给作者看图迭代。作者要「真实侧剖洞穴：随机房间 + 蜿蜒隧道 + 节点只是洞里 POI·可多 POI 同室·可贴边·能关联剧情更好」→ **`SonarScanPanel` 洞穴 SDF 从「节点圆 blob+边直胶囊」重做成有机洞穴**（节点图退**隐藏骨架**·连通/深度/gameplay 不变·纯渲染）：域扭曲 `caveWarp` + smin 平滑并集（房间熔大洞＝多 POI 同室）+ 弯折路由隧道 + 房间散瓣 + 死路壁龛（`buildCaveGeometry`·确定性按 id 派生·守 #100）+ POI 偏心语义落点（`poiOffset`：出口/气偏顶·休整偏底·事件贴壁）+ `voidTrack(−warp)` 标记跟随扭曲洞（不浮岩里·关键坑）。`caveSdf/buildCaveGeometry/poiOffset/voidTrack/caveWarp` 纯导出·已用真导出离屏比对验收的 v3 still（MAD 0.7/255）。不 bump SAVE_VERSION·engine↛ui 不破·SSR smoke E4-E8 不变（偏移纯视觉·点击仍同一 move）。`npm run regress` **27/27** 全绿（14.1s·含 prod build）。新 quirk #102。**canvas 画对仍待作者线上 `?dev` 肉眼**（sweep/点击/对齐·SSR 测不到）。详见 CHANGELOG #104 + SPEC §6④/§9 + QUIRKS #102 + [[sonar-render-redo]]。
> **2026-06-07 周末内容引擎自动 pass · 深渊 `[abyssal]` 变体密度『前所未见之物』（无人值守·#103）**：续同日 #98/#99/#101/#102 四档 established-zone 浅/中段变体密度后，按本任务「越深越优先、越深越离奇」**轮换到深度 band**——五个深 band（trench/abyssal/hadal/subhadal/nameless）皆 9-11 事件饱和，唯 **abyssal（108-140m）母题本就欢迎『真实深海地貌/群落拧进 底在退/拽/招』**（已含鲸落 the_fall/海雪 the_settling/xenophyophore 屋 the_houses），且深渊＝生态最稠密层（teeming plain）＝最该塞满前所未见之物（越深越离奇）。**9 新事件 append 进 `abyssal.json`**（零 registrar）·tone **4C/5U**·全 `[abyssal]` 单 tag（#19·band.tags 隔离不跨蓝洞）·**loot-free**（深水伏笔层 #44/#54）·**无新敌人**（守 2/zone·生物全作 fauna 非 combat）·**不触发 d_reveal** #42·**永不交底** #54（read 成功给平淡科学解释、give-in/read 失败落不交底）·3 选项＝give-in／理智 check 读穿／关灯绕开认准上诚实逃生口。灵感＝WebSearch 四口新井（brine pool「绝望热浴缸」/ Atolla 窃贼警报水母 / Apolemia 管水母最长动物 + Macropinna 桶眼鱼透明头 / 冷沥青火山）。C·oncePerRun：`brine_pool`〈盐卤湖·岸上躺着的不是睡是泡〉/`the_alarm`〈窃贼警报·把你的位置播给底下更大的〉/`the_tar`〈冷沥青火山·留住每样歇脚的东西〉；C：`needs_nothing`〈氧最少层什么都不要的·你还要气＝你还活着〉；U：`the_tubeworms`〈红口管虫林齐齐朝坡下斜〉/`the_herd`〈海参成群下坡·队里有你大小的空当〉/`the_long_line`〈管水母假导引绳·会朝你缩·区别 the_guideline 真绳〉/`the_upturned_face`〈桶眼鱼·把眼睛从朝上转到朝下〉/`the_glass`〈玻璃海绵笼·羡慕出不来的安稳〉。新 `lore.abyssal.*` 9 条·the_alarm 逃生口交叉勾连 `lore.deep_water.cold_light`·abyssal 事件 11→20（深渊＝最稠密 band）·事件 198→207·event baseline 293→307（+14：6 give-in/4 check-success/3 check-fail·#43 实跑抄）。`npm run regress` **27/27** 全绿（6.8s·含 prod build）。**未 commit**（无人值守·留审阅）。无新 quirk（沿用 #19/#42/#44/#54）。详见 CHANGELOG #103 + [[weekend-content-log]]。
> **2026-06-07 周末内容引擎自动 pass · 旧灯塔礁中段 `[reef]` 变体密度（无人值守·#102）**：续同日 #98 reef 浅 / #99 墓园浅 / #101 蓝洞浅——三个 zone 的 12-25m 浅池当日补完后，按 #94 Tier-3「变体密度·最常被看见的池」**轮换深度 shallow→mid**（次常见段）。recon 矩阵（`event-runner --list` × zone×深度×tone 实数）确认 mid 三池里 **reef 26-44m 最薄最偏 tone（6R/4U/2C·realistic-heavy）**、且 `[reef]` 只在旧灯塔礁不跨 zone（#19）＝最低风险池。**10 新事件 append 进 `reef.json`**（零 registrar）·tone **2R/6U/2C**（专补 realistic-heavy·把 reef mid 拉向 8R/10U/4C）·全 `[reef]`·loot 天然 #44（coral/lobster·U/C 伏笔 loot-free）·**无新敌人**（守 2/zone·生物全作 fauna 非 combat）·**不触发 d_reveal** #42·U/C 守**永不交底** #54·stamina-check 只锁 success #49。灵感＝WebSearch 四口新井（真实海洋「猎奇」扭进冷 uncanny：Cymothoa 噬舌虫 / 博比特虫沙伏 / 锯齿矛䲁拟态清洁鱼 / 棘龙虾单行迁徙）。R：`lobster_train`/`barrel_sponges`〈桶海绵 deep-time〉；U：`borrowed_tongue`〈噬舌虫·小黑眼睛朝上看你〉/`false_cleaner`〈拟态·『连被咬的也照旧排队』·不碰 mimic apex〉/`the_cocoon`〈鹦嘴鱼黏液睡袋〉/`the_watcher`〈虾蛄柄眼+空蚀拳〉/`the_sweepers`〈珊瑚夜战〉/`shadow_above`〈巨影掠礁全僵〉；C 伏笔：`the_strike`〈博比特虫·oncePerRun〉/`blue_hour`〈介形虫黄昏齐拍发光·承 the_low_note·oncePerRun〉。新 `lore.reef.*`/`lore.reef_deep.*` 8 条·reef 事件 32→42·reef mid 池 12→22·事件 188→198·event baseline 276→293。`npm run regress` **27/27** 全绿（6.8s·含 prod build）。**未 commit**（无人值守·留审阅）。无新 quirk（沿用 #19/#43/#44/#49/#54）。详见 CHANGELOG #102 + [[weekend-content-log]]。
> **2026-06-07 周末内容引擎自动 pass · 蓝洞群浅段 `[cave]` 变体密度（无人值守·#101）**：续同日 #98（reef 浅段）/#99（墓园浅段）按 #94 Tier-3「转变体密度·最常被看见的池」轮换**换 zone**——reef→墓园→蓝洞群。recon 确认近七 pass 填满最深四 band+nameless+established 40-60m/≤44m+reef/墓园浅段后，**唯一仍薄的『每潜必经』池＝蓝洞 12-25m 浅段**（4R/4U/1C·entrance_light 过曝 quirk #20）。**10 新事件 append 进 `blue_caves.json`**（零 registrar）·tone **4R/4U/2C**·全 `[cave]`（#19·12-25m 与灯塔 cave-tag 45m+ 不重叠故仅蓝洞）·loot 天然守 #44（lobster/coral·U/C 伏笔 loot-free）·**无新敌人**（守 2/zone）·**不触发 d_reveal**（#42）·U/C 守**永不交底**（#54）·stamina-check 只锁 success（#49）。灵感＝WebSearch 四口新井（cenote 树根 / 盲毒 remipede 靠嗅觉追踪 / relict 钟乳石＝淹掉的干洞 / halocline 假水面）。R：`root_tangle`/`percolation`/`shell_drift`/`surge_passage`；U 伏笔：`the_feelers`〈嗅觉追你的盲虫〉/`the_mirror`〈盐线假出口〉/`the_draft`〈洞口呼凉气〉/`fresh_silt`〈朝里去的新印·先来者〉；C 伏笔：`deeper_blue`〈竖井暖蓝诱下·oncePerRun〉/`the_same_passage`〈被绕回原地·oncePerRun〉。新 `lore.bluecaves.*` 6 条·蓝洞事件 27→37·事件 178→188·event baseline 261→276。`npm run regress` **27/27** 全绿（含 prod build）。**未 commit**（无人值守·留审阅）。详见 CHANGELOG #101 + [[weekend-content-log]]。
> **2026-06-07 声呐渲染重做 ②③④ 实装（交互 session·#98 系列·接 `59ee10d` 洞穴一致性）**：按声呐渲染重做 SPEC §6 引擎优先→渲染最后、每步独立全绿分提——**② 猎手 mid-edge + 追击**（`5eedc11`·位置 `nodeId`→`{nodeId,edgeTo?,edgeProg?}`·`STALKER_HSPEED`0.8 速度阀〈<1 可甩·不变量 1−HSPEED≤CONTACT_DIST0.5 一跳必贴〉·对穿〈传 fromNodeId〉/贴近接触·扫描门控快照红点·停下迎战 `standAndFight` 先手 ambushing）·**③ 声呐开/关窗口**（`ccef8b0`·`sonarOn`/`sonarNext` 持续开关·scan-on-open〈到站自动扫〉·暴露按状态·下回合预承诺+本回合反悔·**仅 sonarUnlocked 才落字段＝未解锁逐字节守 stealth**）·**④ canvas 洞穴渲染**（`3ce44e3`·`SonarScanPanel` SVG schematic→canvas 有机剖面〈导出 `caveSdf`：SDF+值噪声·半分辨率〉+ 雷达扫描〈rAF·波前门控〉+ 节点显隐〈只相邻可去 choices 画可点标记·点击=move·防剧透〉+ mid-edge 红点插值〈不要 X〉+ 旧图保留·`deriveMapLayout` 小 pxPerMeter+byLayer·CSS head 注入避污染 SSR）。每步 `npm run regress` **27/27** 全绿。**canvas 画对待作者 dev-server 肉眼**（绿≠画对·quirk #91/#93·调参旋钮见 SPEC §9 末）。新 quirk #100（洞穴一致性）/#101（渲染重做约定）。声呐渲染重做 SPEC 引擎/逻辑面完成、§9 打勾。详见 SPEC §6/§9 + CHANGELOG #100 + QUIRKS #100/#101 + [[deep-game-vision]]。
> **2026-06-07 周末内容引擎自动 pass · 沉船墓园浅段 `[wreck]` 变体密度（无人值守·#99）**：续 #98（同日 reef 浅段变体密度）按 #94 Tier-3「转变体密度·最常被看见的池」轮换**换 zone**——reef→墓园。recon 确认**墓园 18-25m 浅段是当前最薄的『每潜必经』池**（严格浅段仅 ~2R/2U/1C），#98 刚补过 reef、深 band/40-60m/≤44m 已由 #75/#77/#81/#82/#85/#88 填满。**10 新事件 append 进 `wreck_graveyard.json`**（零 registrar 改动）·tone **5R/4U/1C**·全 `[wreck]`（#19）·灵感＝WebSearch 四口全新井（避开 #85 的 Thistlegorm/Titanic/幽灵网 + corpse-wearer/敲击线）：船钟认尸船的老规矩 / 康吉鳗占沉船当窝 / 煤舱铲子摆着没人收 / Antikythera 过精巧的齿轮 / 古陶罐压在钢船下 / 全船钟停在同一分钟 / 沉船长成礁 / 鱼学会等潜水员 / 太齐整的住舱 / 有人扫到一半的甲板。R：`ship_bell`/`the_lodger`〈康吉鳗 wreck-as-habitat〉/`coal_bunker`/`the_reef_now`〈[18,24] 隔离故用天然 coral/lobster·守 #47〉/`amphora_hold`〈深时 grounded〉；U 新鲜错位（≠corpse-wearer）：`the_quarters`〈太齐整·oncePerRun〉/`the_gear_lump`〈数不停的齿轮·撬走=废铜 grounded／拨动=uncanny 拆两路〉/`the_fish_that_wait`〈学会的期待〉/`the_same_minute`〈水停钟+时间冻结〉；C 伏笔：`swept`〈有东西在打理沉船『会回来』·oncePerRun·环境无人影·subtle 深水欺骗最浅伏笔·不触发 d_reveal〉。守则全同既有：loot 按 #44/#47（realistic→wreck 人造 brass/canned/net·the_reef_now 隔离用天然·uncanny/cosmic 伏笔 loot-free）·**无新敌人**（守 2/zone）·**不触发 d_reveal**（#42）·uncanny/cosmic 守**叙述永不交底**（#54·success 给平淡解释/fail 落不交底）·stamina-check 只锁 success（#49）。新 `lore.wreck_graveyard.*` 7 条·墓园池 24→34·事件 168→178·event baseline 244→261。`npm run regress` **27/27** 全绿（含 prod build）。**未 commit**（无人值守·留审阅）。详见 CHANGELOG #99 + [[weekend-content-log]]。
> **2026-06-07 周末内容引擎自动 pass · reef 浅段 `[shallow,reef]` 变体密度（无人值守·#98）**：连六 pass（#75/#77/#81/#82/#85/#88）填满最深四 band + established 40-60m + ≤44m + 新开 nameless 后，按 #94 Tier-3「转变体密度」+ #88 点名「reef≤25m 伏笔层轮换」，轮换回**最薄也最常见的段**——旧灯塔礁 12m `[shallow,reef]` 浅池（仅 6 事件·每次出海必经）。**10 新事件 append 进 `reef.json`**（零 registrar 改动）·tone **4R/4U/2C**·全 `[shallow,reef]`（只灯塔礁吃 shallow·天然隔离 #19）·loot 按 #44/#47（realistic→天然 coral/lobster + 灯塔渔人造 brass/net·uncanny/cosmic 伏笔 loot-free）·**无新敌人**（守 2/zone）·**不触发 d_reveal**（#42）·uncanny/cosmic 守**叙述永不交底**（#54）·stamina-check 只锁 success（#49）。灵感＝WebSearch 三口新井（花园鳗同步缩洞 / 倒立水母 Cassiopea / 乌贼 passing-cloud 催眠）。R：`fish_trap`/`mooring_block`/`staghorn`/`seagrass_flat`；U 伏笔：`garden_eels`/`upside_down_jellies`/`cuttle_passing`/`star_sand`；C 伏笔：`your_shadow`〈影子不跟你·伏笔 nameless.the_outline〉/`the_low_note`〈低频配着你〉。新 `lore.reef_shallows.*` 6 条·12m 浅池 6→11·事件 158→168·event baseline 231→244。`npm run regress` **27/27** 全绿（含 prod build）。**未 commit**（无人值守·留审阅）。详见 CHANGELOG #98 + [[weekend-content-log]]。
> **2026-06-06 基建治理 session · 把解耦/定位约定做成机制（非散文）+ 两 Tier-2 提案（作者已定元层任务·#94）**：原则「想守住的解耦落成机制而非散文」已入 `CLAUDE.md` 顶部 + 起手约定。**T1 实装**：① `engine ↛ ui` 边界 → `scripts/check-boundaries.mjs`（扫 engine 命中 react/ui import 即报退 1）注册进 regress·全量任务 **26→27**·0 违例＝绿（quirk #95）；② 交接定位 → `scripts/handoff.mjs` + `npm run handoff`（纯只读再生 git log/status + nightly 头 + STATUS 顶 + CHANGELOG 尾·根治「夜间漂移」手抄·quirk #96）。提交 `bbd08d3`（feat）+ docs。**T2 仅方案**（`docs/infra/`）：`dive.ts` 纯搬移拆分（barrel 保 API·defer 到并发隔离后）+ 三写手并发隔离（周末引擎独立 `auto/weekend` 分支·触两定时任务 SKILL·待作者确认）。**T3 标记**：内容天花板——ZoneTag 用尽 / nameless 叙事地板·建议周末引擎转**变体密度 / 更少更大批**、增长轴交给交互 session 新机制。不碰游戏功能/SPEC/SAVE_VERSION/定时任务 SKILL。`npm run regress` **27/27** 全绿。详见 CHANGELOG #94 + `docs/infra/*` + QUIRKS #95/#96。
> **2026-06-06 声呐图垂直化＝真实深度 ·「位置即深度」系统不变量（作者已定定向任务·#92）**：声呐图 / MapDevPanel 纵轴此前无深度含义（`deriveMapLayout` 把 depth 横排 x∝layer、y 只是同层 id 堆叠序）→ 改成 **`y∝node.depth`（上＝浅 / 下＝深·真实米数·固定 px/米·取 > blip 直径防纵向重叠）**，x 改同深度分散（按 depth 分箱居中·无方向语义），单一来源 `ui/mapLayout.ts` 共用 SonarScanPanel + MapDevPanel。**P1 渲染**——SonarScanPanel viewBox 转 portrait·`focusWedgePath` 重映（deeper↓ / back↑ / lateral←→·SVG y 朝下）·量程环/楔形/接触半径改 `min(VIEW_W,VIEW_H)`·残图小地图竖盒·`.sonar-scan` 限高 300px；MapDevPanel 零改自动跟随（全由 layout 驱动）。**P2 扇区耦合**——`engine/sonar.ts::nodeSector` 从 `layer` 差分改 **`node.depth` 差分**（新 `SECTOR_DEPTH_EPS`1.5 容差带）：否则「朝深」楔形指下、定向 ping 却扩 layer-deeper（迷路图里可能 depth 更浅、渲染在上方）＝自相矛盾；`revealSonarScanDirectional`/`stalkerSector`/`seenStalkerSector`/`pingAimsAtSoundStalker` 全随之按真实深度。`playthrough-sonar` §13 fixture 解耦 layer↔depth（layer 故意相反）真测 depth-based（layer 实现会三条全错）。**P3 系统不变量**——审计确认 mapgen 主下行 depth 早已单调-from-start（层状 `round(d0+step·L)` 逐层非减+同层相等·迷路起点钉 d0=图顶最浅、最深钉 d1）→ 只加 `playthrough-mapgen-scenarios`「位置即深度」垂直性不变量（迷路 60+层状 30 seed）兜·**不改生成·不破快照**（区别 #90 全 additive）。**视觉验证**（绿 ≠ 画对·quirk #91 教训）＝SSR→SVG 坐标核对：上浅下深 monotonic·楔形朝下·min 间距 ≥ blip 直径无重叠·回边 chord 可读。新 quirk #93（位置即深度系统不变量）。未发布不 bump SAVE_VERSION（布局纯渲染派生·EPS 常量）。`npm run regress` 26/26 + prod build·提交 `1829d14`（feat）+ 本 docs commit。详见 CHANGELOG #92 + 深水区 SPEC §13/§10 + QUIRKS #93。
