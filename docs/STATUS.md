...
> **2026-06-19 装备段2：声呐迁 Otto 打造件 + 退役三传感器线 + 死字段清净 + 手电 Mira 购买 + E/C/D 收口（交互 session·#145·接 #144·feat `926f185`）**：作者拍**新装备模型**——声呐＝唯一逐级件（Otto 用材料**打造** Lv.1→升 Lv.5·`item.sonar.handheld` craftCost+upgradeSteps=旧 `sonar_rig` 数值逐项对账），手电＝Mira 购买固定件不升级，灯/电池/规避升级退役删（可日后做灯/服档位件加回）。**退役** `upgrades.json` 三传感器线·`getRunBonuses` 声呐项改读装备件 `eq`·`sonarUnlocked`＝`hasSonarEquipped` 派生·事件门 `hasEquipment{slot:'sonar'}`·**SAVE 6→7**。**2 删**：三线退役死字段整链清（`UpgradeEffect`/`UpgradeBonuses`/`getUpgradeBonuses`/`UpgradePanel`）·`getRunBonuses` 灯/规避喂 0·保留 `deriveSensorTuning` 旋钮 + `RunStartBonuses` 形状（re-add 接口·`powerMaxBonus` 还被 dive-start 用）。**1 做**：手电上 `SHOP_STOCK_EQUIPMENT` 货架（Mira 可购·40 金·shop UI 显示）。**E**：`EQUIPMENT_SLOTS` 单一来源 + 穷尽性断言（加槽漏改＝typecheck 红）+ 新 `smoke-equipment-ui`。**C**：`EquipmentDoll`(readOnly) 接 `DiveHeader` 第三面板「装备」。**D**：`unlockedAccessorySlots` 单点占位（恒 1）。对账门 `playthrough-upgrades §8`。regress **41 非 build + prod build 全绿**。新 quirk **#141**。**遗留**：灯/服档位件（加回灯/规避升级·别重建线）、仓库→槽换装流程、饰品 2/3 解锁来源、其它槽升级内容 + 数值手感。详见 CHANGELOG #145。
