...
> **2026-06-21 战斗 dev 面板：实战预览（真实 CombatView）+ 新战斗字段 + 可视化 + SSR smoke（交互 session〔Cowork〕·#168·无新 quirk）**：`?editor=combat` 面板加「实战」模式——抽出 `enterCombat` 单一进战路径（批处理 `runCombatScenario` 逐字节不变·combat-scenarios 26/26），新 `buildCombatEntryState` 只造到开战相位、喂真实 `<CombatView>` 反应式打（活 RNG）；phase 构造留引擎守 check-boundaries 规则二·`CombatDevPanel` 引用 `CombatView`＝dev→game（#152 允许向）。右栏模式条批处理↔实战翻成 `[⟲ 重开][退出实战]`·换遭遇/切模式/改 party 自动退实战。表单补 `bonuses`（#164 boss 体力解卡·quirk #151）/`wornSkin`（#162 皮囊→loot 变体）/`injuries`（负伤 §10）·`buildEquipmentOverride` 改由 `EQUIPMENT_SLOTS` 派生（防 5 槽子集漂）。`EnemySnapshot` 加 phaseIndex/phaseCount/reachable·批处理+实战标 boss 阶段/战场压力/链鳗按序（CombatView 锁最前节+「被前节挡住」）。新 `smoke-combat-panel`（SSR+round-trip+engine parity）接 regress。typecheck 0·`--skip build` 46/46 绿·build 留 Mac/nightly·8 文件·无新 quirk。
> **2026-06-21 一章 tells 定稿清单 + St1 收尾盘点（交互 session〔Cowork〕·#167·无新 quirk·纯 docs）**：核出起手两 stale 项——「另一个世界」/d_reveal = 剧情 St7（叙事三/四章）capstone·非独立 build·作者拍低优先（ch4）；St1 四锚点早已实装全绿（`ch1.json` 四锚点+圆满结局+POI+12 scenario+`playthrough-story`）。改做 St1 收尾＝补 roadmap 唯一欠项「tells 清单定稿」：新 `docs/spec/深海回响_一章tells清单.md` 把圆满结局回读 tells（四条 canonical + 7 散落伏笔含 vent 单程下行 seed）锁成单一来源·只编目+指向·**d_reveal 冻结·St7 归作者逐拍**；§4.1 加指针。纯 docs·零行为变化。下一拍 St2（留白结局 `ending.blank` 无触发器 + 录音第 1 段·回收 tell T4）。
> **2026-06-21 素材经济可视化工作台：materialStats 引擎叶子 + EconomyDevPanel 热力图（交互 session〔Cowork〕·#166·新 quirk #155）**：镜像剧情分布先例（`eventStats`↔`StatsDevPanel`）——`engine/materialStats.ts`（纯叶子·`computeMaterialStats()` 把旧 `material-audit.mjs` 解析搬进 engine 单一一份：敌 loot + 事件 outcome.loot + 柱 capstone grantsItem ＝来源·任意层级 `materials[]`〔含 `tier.cost.materials`〕＝需求·导出每素材源数/总需求/状态 + 素材×zone 矩阵 + bottleneck/deadstock/idle）+ `ui/dev/EconomyDevPanel.tsx`（KPI + 素材清单〔瓶颈红/死货红/死料灰〕+ 热力图·按 tier/zone/状态筛·挂 `?editor=economy`）。CLI `material-audit.ts` 薄壳收口同一聚合（`--json` 逐字节同旧·解析单一真相）·`smoke-economy-panel` SSR+parity 门接 regress（`css-stub-loader` 解 tsx 不认 .css）。deadstock 留 advisory（不加硬门）。typecheck/check-boundaries/5 smoke 绿·build+playthrough 留 Mac/nightly。**（同日续 v2）** 按作者复盘重做成**三 tab 共用一张 素材×大区 等宽热力图**（来源 ΣEV / 消耗 / 净值=源−耗·未点行格式一致只换内容）+ 点行明细（来源方式〔敌人/挖矿/事件/深度柱〕·概率·消耗场景·状态页左右对比框）·大区＝忠实标签·消耗按设施所在区·装备归港口·全加法 parity 不动。
## 1. 一句话状态
