...
> **2026-06-27 地图调试器洞穴四边裁切修复（`CAVE_GEOM_MARGIN` 单一来源·交互〔Cowork·Opus〕·#226·新 quirk #194·不 bump SAVE·沙箱 typecheck 绿·待 push）**：作者报「调试器洞穴左右像被切、地图都纵向」。澄清纵向＝设计〔quirk #92 y∝深度·横轴无方向语义〕；四边裁切＝真瑕疵——`MapDevPanel` 烤洞穴 rect 用节点包围盒〔padX/padY 仅 30/24〕，有机洞穴〔房间/散瓣/域扭曲·≤~75px〕鼓出被画布裁。修：`SonarScanPanel` 导出 `CAVE_GEOM_MARGIN`〔按几何旋钮派生=75·单一来源〕·`MapDevPanel` 用 `caveRect`〔节点框四边各扩 margin〕统一 bake/canvas/容器/viewBox。游戏内移动取景窗不中招。push 待 Mac/nightly。---
