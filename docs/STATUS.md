...
> **2026-07-03 对话选项面板收窄：新/已聊/同功能三档选 3 + 换话题 + 跨局已聊记忆（Cowork 交互·#255）**：作者反馈后期 NPC 对话选项同屏冒出过多，设计三档优先级（未触发>已聊>同功能冗余·同功能凑不够格才补位，够了整档摘出不是排队等轮到）+ 非空档数>1 时保底留 1 个轮换位 +"换个话题"按钮。`engine/dialog.ts` 新 `DIALOG_DISPLAY_CAP=3`/`selectDisplayChoices()`；`PlayerProfile.seenChoices?: Set<string>` 跨局持久（纯加字段不 bump SAVE，走既有 Set round-trip）。中途加过常驻关闭键，作者反馈"位置错位"直接撤回——连带把已顺手标 filler 的通用退出选项（`leave` 等）撤销标记，否则同功能整档被挤出候选池时那些节点会真的关不掉对话（面板没有常驻关闭键，退出全靠选项本身）。收尾追加 `check-dialog-filler-exit.mjs` 把这条红线机制化（只判 `choices.length>3` 的节点，避免误伤 `aldo.coords_left` 这类短节点）。测试期间自查出一个死按钮 bug（同功能挤出后总数恰好=上限时误报 needsRotate）已修；顺带纠正一条 13 天前的过期记忆（Otto 其实已有完整对话树）。全量 `npm run regress`（新增 1 门后共 90 任务）**89/89 全绿**·build 留 Mac/nightly（#147）·不 bump SAVE·新 quirk #216。**树**：main·12 文件（10 改+2 新）+ 三件套待单次提交·ahead 6 待推。
> **2026-07-03 海图 POI 触摸命中改"容器拦截+世界坐标最近距离分发" + 消两处坐标完全重合（Cowork 交互·#254）**：作者问「地图上的点，触摸范围是否会造成遮挡？」，排查出热液前哨下两对 anchor/roaming **坐标完全相同**（`serpentine_deep_vent`↔`vent_dead_chimneys`、`flooded_gallery_vent`↔`vent_cold_seep`，两边解锁条件互不排斥会同天共存，原生 DOM 层叠命中下其中一个 100% 摸不到）。三件事：①挪开两对撞车坐标（幅度 ~0.04）；②`ChartViewport.tsx` 新增 `hitTargets`/`TAP_HIT_RADIUS_PX`，三类标记按钮改 `pointer-events:none`，鼠标/触摸命中改走"抬指针→世界坐标→半径内最近一个"分发（键盘 Tab/Enter 走各自 onClick 不受影响）；`SeaChartView.tsx` 选中逻辑抽三个具名函数供键盘/指针两路共用；③新 `check-chart-poi-coords.mjs` 焊坐标撞车回归门（resolve 绝对坐标两两查重合·纯函数已用真实数据+合成用例验证）。手工代入数值重推了一遍屏幕↔世界坐标换算公式核对无误（SSR smoke 测不到真实指针交互，自动化盖不到这块）。沙箱装 `@esbuild/linux-arm64@0.28.1` 后全量 `npm run regress` **88/88 全绿**·build 留 Mac/nightly（#147）·不 bump SAVE·新 quirk #215。**树**：main·6 文件（5 改+1 新）+ 三件套待单次提交·ahead 5 待推。
> **2026-07-03 Sela 引荐门与深度脱钩 → Mira 打捞委托单点门控（Cowork 交互·作者多轮拍·#253）**：剧情侧讨论顺势敲定——Mira＝销赃中间人（非前海盗成员·留缝）；Sela 引荐（`sela_tip`→`intel.mira.sela`）**不再门深度 `anchor.midwater`**，改由 Mira 一次性打捞委托门控，且门在**任务完成 flag `story.ch1.mira_salvage_done` 而非 `npcTrustTier`**（引荐＝二元 story 事实·flag 门让「做完⟹见得到 Sela」结构性、免数值调参掀翻；Mira 分档信任等她有分档内容再 additive 接）。单一旋钮＝任务位置：深度只写在 `poi.anchor.mira_salvage` 的 offer 门·挪任务则 Sela 可达点自动跟随。实装 6 文件（story.ts 两 flag+allStoryFlags / mira.json 委托对话+regate sela_tip / chart_pois POI / midwater.json `mira.salvage` 事件〔loot 红喉鹈币=fence tell+bootstrap〕 / playthrough-trust §8/§9 焊耦合 / SPEC §12.14）。runtime 抓到 typecheck 漏的 bug（mira.json `dialogs` 是顶层键非 `npc.dialogs`·`as unknown` cast 盖过·tsx 实跑才现）。沙箱装 esbuild 后全量 `npm run regress` **87/87 全绿**·build 留 Mac/nightly（#147）·不 bump SAVE·无新 quirk。**树**：main·6 文件 + 三件套待单次提交·ahead 5 待推。
## 1. 一句话状态
