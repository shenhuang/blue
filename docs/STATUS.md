...
> **2026-06-26 工作效率复盘 + 三条「从 git 真值核对漂移状态」机制（交互 session〔Cowork·Opus〕·#206·新 quirk #177/#178·沙箱 regress 13/13 子集·41 tsx 留 Mac·待 push）**：复盘近 21 个历史 session 找效率摩擦（3 并行 subagent 读 transcript·保主上下文净）→ 落 3 条机制（全 advisory·复用既有 helper·无新红门）：(1) `regress.mjs` 沙箱自动跑 esbuild-free 子集（typecheck+静态门 13/13·41 tsx 行为测留 Mac/nightly·收尾门不再吃 41 红再人工忽略·**沙箱子集绿≠可发布**·quirk #177）；(2) `npm run roadmap:check`（`check-roadmap-stale.mjs`）对账 roadmap「当前状态」banner vs git·实测揪出 cave 全 9 车道已 landed-since-banner（quirk #178）；(3) `psm status` git 真值核对 stale-landed flag + `handoff` 印运行环境边界 + roadmap 漂移自检（quirk #178）。改 `regress.mjs`/`handoff.mjs`/`psm.mjs`/`package.json` + 新 `check-roadmap-stale.mjs`。遗留：push 待作者本机；效率复盘 #2（并发共享树碰撞·worktree-per-session）是工作习惯决策·留作者拍。
