...
> **2026-07-02 全仓体检 + 全部修复（6 维度审计→对抗验证→两 phase 并行修复·#248）**：审计确认 23 条问题全修。**引擎**：combat flee「全灭未结算窗口」崩溃修（入口先全灭结算+防御 break）；分裂/补虫 `Date.now()` instanceId 撞号→`CombatState.spawnSeq`（撞号曾掩盖分裂体出生即再分裂级联·一并修）；flee 判定子串→`pendingFleeSuccess` typed flag（#107 过时）；#244 零掉率落成 `fledInstanceIds` 机制（victoryModifier 是引擎不读的死数据）。**自动化**：psm land(Mac) temp ff 后 `--since main` 恒零 diff 假绿洞→`preFfSha` 基准；psm gc 四类扩展（已收走 poi-roaming）+判定纯函数 14 测；affected 入口补 `smoke-*.mjs`。**新门 7 个（regress 81→88）**：`typecheck-scripts`（scripts 两万行入 typecheck·存量 38 错清零）/`check-quirk-refs`/`check-lore-refs`〔ratchet〕/`check-weekend-base`/`check-file-budget`〔ratchet〕/`smoke-event-stats`/check-terminology silas 禁词。**结构**：port.ts 六货架表抽 `src/data/shop.json`（红线③变实+写侧校验+Sela 相位窗机制强连）；combat.ts 1650→1074〔`combat-mechanics.ts`〕·mapgen.ts 1471→105 门面+5 子模块（行为逐字节等价）；`scenarioShared.ts`/`pt.ts` harness/`sonarGeometry.ts` 抽取。**文档**：QUIRKS #208 裁撤号段墓碑+五处死引用考据修复；主线柱迁移/战斗系统 SPEC banner 翻面；README 收窄成指针；handoff 新报 spec banner advisory〔7 过期+11 无头〕。清理 `_debug_silas*`+gitignore 防线、3 僵尸 worktree 注册项、sandbox-junk（Cowork 交互〔Fable·ultracode〕·#248·新 quirk #208–#211·不 bump SAVE·Mac `npm run regress` **88/88 全绿**〔含 build〕）**：遗留＝作者拍三件（裂球平衡·victoryModifier 裁留·三棵零提交 worktree 废弃与否）；机械件（7+11 份 SPEC banner、32 playthrough 迁 pt、lore 基线批补）留后续 session。
> **2026-07-02 藏宝贸易 Phase 3 Slice 2 的 Ch1 部分：Corin 遗物 retcon 币→徽章 + 渐深遭遇（Sela 热液区深层再遇）+ Mira 通用图来源分层·Nerea 线移二章**：① Corin 收藏品 `item.keepsake.corin_coin`（半枚红喉鹈币）→ `item.keepsake.corin_badge`（红喉鹈徽章·赤喉海盗团徽记）·**徽章两段揭示**（交还 Corin 徽章 Ch1 收口 → 交还 Nerea 徽章 Ch2 交底赤喉海盗团）；② 渐深遭遇：热液区 3 个 `roam.sela_meet_deep_*`（gated `story.ch1.anchor.vent`+`flag.sela.met`+intel+相位窗·置 `flag.sela.met_deep` 供二章消费·文案透她「只会更深」）；③ Mira 便宜卖通用图 `item.treasure_map.salvage_generic`（新 `SHOP_STOCK_CHARTS` 表·价与 sellPrice 解耦·`oneTimeFlag` 一次性·揭示浅礁固定点 `poi.anchor.salvage_generic`）。作者拍 **Nerea 整条线移第二章**（＝生物派主区·二章零代码故延后·Ch1 埋 `flag.sela.met_deep` 钩）。`playthrough-trust §6/§7` 扩测·`smoke-economy-panel` brass srcCount 27→28·`emit-economy-graph --write` 再生（Cowork 交互〔Opus〕·#247·新 quirk #207〔smoke srcCount 硬编码坑 + `SHOP_STOCK_CHARTS` 价/sellPrice 解耦+oneTimeFlag〕·不 bump SAVE〔纯加法·仍 12〕·沙箱 `npm run regress` **81/81 全绿**〔含 tsx·build 留 Mac/nightly·#147〕·**未提交·commit 待 Mac/nightly**）**：下一步＝Nerea 二章线（待二章落地）；未做 Mac 真实浏览器验收（建议 `npm run dev` 走 Corin 徽章循环 + 热液深遇 Sela + 买 Mira 图）。
## 1. 一句话状态
