...
> **2026-07-05 感知重做 4 待决点全拍板 + ① 氧气幻觉致命 + san 双层阈值（Cowork 交互·Opus·#261·不 bump SAVE）**：#259 感知重做留的 4 个待决点（SPEC §7/§10）本 session 逐条拍 + 实装 ①。**① 氧气致命**：`combat.ts` 氧气≤0 判死移出 `!inHallucination`（幻觉内外都判·体力/san 仍软化不判）——全封＝进幻觉无敌泄张力；无脚本死靠应急上浮（step6 排死亡判定 step8 前·无条件零成本）兜。**san 分层**（作者「50 看得到、20 才致命」）：`clarity.ts` 拆致命层 `HALLUCINATION_SANITY` 50→**20**（幻觉战+氧气致命）/ 可见层新常量 `HALLUCINATION_VISIBLE_SANITY`=**50**（改选项类幻觉·本不致命·本就存在零新做）·原散 6 处字面量 50 收口单一真相（quirk #227）。`playthrough-sensors` 加 14d（幻觉氧尽仍淹死）守门。**② mimic `_doc`**：接受（纯注释）。**③ 黑=整潜级**：作者 override→要 per-node 黑 + 砍黑/不黑两档（murky 取消）+ mapgen 分布 + 剧情自洽·方向 b·**留专门 session**（blast radius 已调研·Opus/high 单车道·机制层先落 / 内容层按区·剩余细节 dive-start log / check-no-murky 门 / 各区密度待敲）。**④ `the_leftover_echo`**：保留·性质待定。**验证**：full regress **92/92 全绿**（沙箱 Linux esbuild@0.28.1·仅 prod build 缺 rollup 留 nightly）。**（2026-07-05 同日续·Opus）** 揭示提示文案「靠 X」→「持有 X」：改 `EventView.tsx` `.reveal-tag` **唯一渲染点** `靠 {revealBy}`→`持有 {revealBy}`（前缀唯一字面·`{revealBy}` 由 `revealAttribution` 数据驱动派生真名·**所有道具自动跟走·非逐项改**·quirk #224 文案微调·细节见 CHANGELOG #261 续段）；同步 smoke/注释/SPEC §2.1b/quirk #224 正文·真·"rely on" 义「靠」不动·affected+static gate 42/42 绿。**树**：main·#261 本体 8 文件 +（同日续）6 文件〔EventView/events/styles/regress/smoke + SPEC〕+ 三件套·两次单提交·push 留 nightly·ahead 待推（#249–#261·续段并入不占新号）·定时任务全停。
> **2026-07-05 修下潜/海图 POI 面板「下潜/出海」按钮溢出框外（`.chart-info` 高度被钉死·Cowork 交互·Sonnet·#260·不 bump SAVE）**：作者报「下潜界面的下潜按钮在框外」。根因＝`.chart-info`（出海/装包/锁定目标三步的框）在**竖向** flex 容器 `.chart-2d` 里用了 `flex: 1 1 240px`（早期左右布局遗留）——竖排下 `flex-basis:240px` 把框高钉死 240px，且显式 `min-height:180px` 关掉了 flex 子项默认 auto(min-content) 兜底 → 内容超 240px 框不撑开、`可能收获`+`出海/下潜`按钮溢到框底边外；窄屏（blurb 折两行）触发、宽屏内容恰好装得下故只手机现。修＝`flex: 1 1 240px`→`flex: 0 0 auto`（按内容撑高·`min-height` 只当地板·加注释防回退·quirk #226）。用 #257 截图 harness 实证：框 240→284px、`出海`/`下潜` 两步 × 手机/PC 按钮几何 INSIDE。**验证**：full regress **92/92 全绿**（含 check-boundaries；build 门沙箱缺 rollup-linux-arm64-gnu 跳过·#147 留 nightly）·CSS-only 不 bump SAVE。**树**：main·1 文件（`src/styles.css`）+ 三件套单次提交·sandbox 无凭证 push 留 nightly·定时任务全停。
> **2026-07-04 感知系统重做：北极星换轴——灯/声呐/深度不再欺骗、欺骗只剩单一「低理智」轴（→选项+怪物）·车道 2–6 全实装（Cowork·Opus/high 拆引擎 + Sonnet/medium 内容·#259·SPEC `docs/spec/深海回响_感知重做_SPEC.md`）**：旧「越深越欺骗」机制成本高、实测不好玩（声呐从不进决定·欺骗只是可忽略预览文字）→ 换新北极星「三件感知各司其职且诚实·欺骗收敛成一根低 san 轴·san 回满即消」。**车道 2（拆引擎 ~−480 行）**：删整套声呐欺骗（`sonarReturn`/`spoofsSonar`/`evadesSonar`/`nodeSonarView`/`sonarPhantoms`/`applySonarDeception`/`sonarDeception`/`effectiveFalseEchoSanity` + `lampPreview` 低 san 分支）·`clarityForNode` 三档塌成**二态灯门** `clarity.ts::lampGateLocked`（**反转**：黑=灯的用武之地·门判据「灯开+有电」·旧 `lampEffective` 已删·quirk #220）·`NodeChoice.locked` 新增·新门 `check-no-sonar-deception` 入 regress（#219）。**车道 3a**：`NodeSelectView` 把 `locked` 渲染成可见但 disabled +「需要灯」+ `.lock-tag`（黑=整潜级·#221）。**车道 4**：删声呐双回合状态机→单发「扫一记」ping·`sonarScanRange` 驱动 BFS 视觉前瞻（与猎手听觉统一·#222）·杀空转升级·Lv.3 重指 `sonarScanRangeBonus`。**车道 5-1**：低 san 幻觉怪（§7① 形态 a）`maybeHallucinationEncounter`·软化结算·**幻觉战封死全部死亡窗=无脚本死**（#223）。**车道 5-2**：`revealAttribution`「持有 X」揭示提示·零逐项写（#224）。**车道 6**：`trench.json`/`midwater.json`/`lore.json` 声呐骗人内容改写成「诚实回波 + 你自己低 san 硬读」·冻结 corpse-wearer 伏笔；**更正：`abyssal`/`hadal`/`subhadal.json` 不存在**（早被深度柱取代·别再追·#225）。B（mimic/apex）冻结。四份旧 SPEC 加 tombstone 指本 SPEC（深水区欺骗侧/声呐与房间 S2/声呐渲染 §4/剧情 §3.7 双轴→单轴）·诚实侧全保留。**验证**：full regress **92/92 全绿**（含 check-no-sonar-deception 新门）。**待作者过 4 点**（氧气幻觉中是否致命/`mimic.json _doc` 改一行/黑=整潜级/`the_leftover_echo` 冻结是否该改·SPEC §7/§10）·数值占位待统一调。**树**：main·车道 2–6 + 三件套未提交（待作者 Mac 复核 + vite build + commit）·定时任务全停。
## 1. 一句话状态
