...
> **2026-06-25 多口持久洞 方案 B（cave roadmap T2·`cave-bmap`〔Cowork·Opus〕·#198·新 quirk #172·`d4bde53`·Mac land 52/52·沙箱 full regress 50/50）**：洞从「每潜重生的临时图」升级成「一张冻结进存档的真实空间」。先写子-spec 逐节走查定案（独立 `caves.json` + `cave.<名>` 命名 + 砍 schemaVersion + **深度模型 B 自采坐标·非 hop 距离**）。四增量：A 存档形状（`PersistentCave`/`caveMaps`/`caveEntry` + `SAVE_VERSION 9→10`·序列化复用 __map/__set 零新代码）· B `generatePersistentCaveMap`（多入口 + 核心钉 d1 + k 密度剖面·复用迷路骨架·**单口旧 maze byte-identical**）· C 绑定续存（`startDiveIntoCave` load-or-generate + 尸体/采尽加载 overlay + 写回 explored/harvest **by caveId**）· D 渲染契约（`persistentExploredForRun`/`cavePortalsForChart` + 声呐图已探片预亮·additive）。新 `playthrough-cave.ts` 11 块不变量（含横向不污染深度）。**休眠态**：无 POI 带 `caveEntry`（caves.json 1 占位洞）⇒ 现有游戏零行为改变·待 **T3b** 接 chart_pois.json。深门循环洞=**T2b**（依赖本棒·最后期）。
> **2026-06-25 POI roaming 续批 9 模板收尾（交互 session〔Cowork·Sonnet〕·#197·`d56203b`·playthrough-chart ✓·typecheck ✓）**：8 条前序已落地；修正 `roam.lighthouse_trench`：owner `lighthouse.home`→`lighthouse.ch1_trench_outpost`·zone `old_lighthouse_reef`→`vent_trench`·更名「暗流带」·modifier 改 current:strong+visibility:dark·mapX/mapY 缩至 trench radius(0.107) 内（offMag 0.078）。无新 quirk。
## 1. 一句话状态
