...
> **状态：已完成·关闭（2026-06-10 作者线上 `?dev` 验收通过——含手机 `?dev&panel=map` 全图翻看·「现在也很好」·CHANGELOG #107 续②）。** §6 ①②③④ 全落地（#100 ②③④ → #104 ④ 有机洞穴 port v3 → MapDevPanel 验收工具 → 线上验收）。**本文件转只读参考**：调参旋钮在 `SonarScanPanel` 顶部（§9 末）；有机洞穴约定与坑见 QUIRKS #100/#101/#102（标记必 `voidTrack(−warp)`·别画 `layout.pos`）。残余 gameplay 向（猎手 Phase 2+）在猎手 SPEC §7，与本 SPEC 无关。
>
> 原状态（留档）：v1 草案（2026-06-07，作者逐拍 demo 敲定，预期工作 #98 起）。把下潜内声呐图从「示意 node graph（圆点 + 连线）」重做成**有机洞穴垂直剖面 + 雷达式扫描**，并把猎手追击从「节点快照」升成「可处在通道中段 + 扫描门控显示」。本 SPEC 是作者经多版交互 demo 拍板后的源真。建法承袭项目「SPEC → 分阶段实装 → 每阶段全绿自审」。
>
...
>
> **⚠ §4 声呐开/关窗口已被感知重做取代（tombstone·2026-07-04·#259）**：§4「声呐窗口规则（开/关·何时扫）」的**双态开/关状态机**（本回合开+预约下回合·`sonarOn`/`sonarNext`/scan-on-open）→ 已改为「**ping 才扫**」单动作（一次性·付电+alert·无跨回合持久 sonar 状态·见 `深海回响_感知重做_SPEC.md` §2.2 + quirk #222）；`sonarScanRange` 现驱动 BFS 视觉前瞻。**渲染（§2 雷达扫描 / §3 一记 ping 扩散 / §5 猎手追击 / §6 有机洞穴）全部保留**·只换「何时触发扫描」这一层。
