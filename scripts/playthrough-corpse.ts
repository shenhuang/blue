...
// 启动一次到旧灯塔礁的下潜
state = { ...state, run: createNewRun({ zoneId: 'zone.vertical_test' }) };
state = startDive(state, 'zone.vertical_test');
...
for (let attempt = 0; attempt < 10; attempt++) {
  let s: GameState = { ...state, run: createNewRun({ zoneId: 'zone.vertical_test' }) };
...
for (let i = 0; i < 10; i++) {
  let s: GameState = { ...state, run: createNewRun({ zoneId: 'zone.vertical_test' }) };
  s = startDive(s, 'zone.vertical_test', { targetCorpseId: targetId });
  const has = s.run?.map
...
let sm: GameState = {
  ...
  run: createNewRun({ zoneId: 'zone.vertical_test' }),
};
sm = startDive(sm, 'zone.vertical_test', { targetCorpseId: 'death-maze-test' });
...
let sBad: GameState = { ...state, run: createNewRun({ zoneId: 'zone.vertical_test' }) };
sBad = startDive(sBad, 'zone.vertical_test', { targetCorpseId: 'death-does-not-exist' });
...
hsState = startDive(hsState, 'zone.vertical_test');
