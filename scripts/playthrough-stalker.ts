...
function chainMap(depth: number): DiveMap {
  ...
  return { zoneId: 'zone.vertical_test', generatedAt: 0, startNodeId: 'n0', nodes };
}
...
L('\n========== 7. spawnNodeFor 现身点 ==========');
{
  ...
  // 小图（够不到 hops）→ 退回最远可达
  const small: DiveMap = {
    zoneId: 'zone.vertical_test', generatedAt: 0, startNodeId: 'a',
    nodes: {
...
{
  const tSound = createNewRun({ zoneId: 'zone.vertical_test', bonuses: { soundAbsorbBonus: 0.5 } }).sensorTuning!;
  const tCamo = createNewRun({ zoneId: 'zone.vertical_test', bonuses: { camoBonus: 0.5 } }).sensorTuning!;
  const tBoth = createNewRun({ zoneId: 'zone.vertical_test', bonuses: { soundAbsorbBonus: 0.5, camoBonus: 0.5 } }).sensorTuning!;
...
L('\n========== 9. mid-edge 追击（§5·中段推进/贴近/对穿/中段快照/迎战）==========');
{
  ...
  assert(engaged.phase.kind === 'combat' && engaged.phase.combat.preemptive === true, '9e: 迎战给玩家首回合免费行动（preemptive）');
...
// 洞穴内容整删（2026-07-12）：原两个数据守门样本 zone（蓝洞群/reef_pocket）已随 27 条真实洞穴 zone 删除。
// zone.vertical_test（QA 夹具·见 zones.json _qa_fixture_doc）技术性保留了 ambushEncounters/weakHunts 字段，
// 专为不让本节（moveToNode 真实读表接线）随内容一起静默失效——第二个「不同 zone」对照样本已无处可repoint，
// 该半条断言随之收窄为单 zone 覆盖（覆盖收窄，非机制改变）。
{
  assert(
    getZone('zone.vertical_test')?.weakHuntEncounters?.[0] === 'combat.blind_eel_juv_solo',
    '16: zone.vertical_test（QA 夹具）weakHuntEncounters＝盲鳗幼体',
  );

  // 接线（weakStalkerStep via moveToNode·同 §14d 路数）：浅水中奖 → 弱猎手的遭遇是幼体不是成年体
  const shallow16 = (runId: string): GameState => {
    const base = huntState({ alert: 0, depth: 18, huntEnabled: false });
    return { ...base, run: { ...base.run!, runId } };
  };
  const JUV_POOL = getZone('zone.vertical_test')!.weakHuntEncounters!;
  let hit16 = '';
  ...
  L(`  zone.vertical_test 幼体池守门 / moveToNode 接幼体遭遇（${hit16}）/ 硬性不被档案推翻 / 注册指向幼体 ✓`);
}
...
L('\n========== 18. 目的地 kind 不冻结猎手（穿过 hunter 修复·06-11）==========');
{
  ...
  const mapR: DiveMap = {
    zoneId: 'zone.vertical_test', generatedAt: 0, startNodeId: 'n0',
    nodes: {
