// 通用 NPC 信任系统 Phase 2 回归：特殊商人 Silas（藏宝贸易与信任系统 SPEC §6·#244）。
//   1. 交头点：3 个候选 roaming 模板（roam.silas_meet_new/waxing/full）——情报（intelFlag）前 hidden，
...
//   4. 信任跨档：累计花费到 npc.silas 自带 thresholds([10,30,60]) 才解锁高档货架（tier1/tier2 条目）。
//
...
// —— 1. 交头点揭示：情报前 hidden；情报后随相位窗浮现，同 day 只一个 ——
{
  ...
  for (const day of [dayNew, dayWaxing, dayFull]) {
    ...
    const found = chart.pois.filter((p) => p.templateId?.startsWith('roam.silas_meet_'));
    assert(found.length === 0, `day=${day}（未给情报）：3 个候选点都不应出现在海图上（现 ${found.length}）`);
  }
  L('  未给 intel.mira.silas：交头点全程不上图（即便相位对得上）✓');
  ...
  for (const [day, expectTemplate] of [
    [dayNew, 'roam.silas_meet_new'],
    [dayWaxing, 'roam.silas_meet_waxing'],
    [dayFull, 'roam.silas_meet_full'],
  ] as const) {
    const profile: PlayerProfile = { ...baseProfile(day), flags: new Set([...baseProfile(day).flags, 'intel.mira.silas']) };
    const chart = generateChart({ profile });
    const shown = chart.pois.filter((p) => p.templateId?.startsWith('roam.silas_meet_'));
  ...
  L('  给了 intel.mira.silas：每个相位恰好一个候选点 lit（可去），另两个 dim（可规划）——"同一时间至多一个可去" ✓');

  // waning 相位：三个候选窗全不命中——即便有情报，也全 dim、没有一个 lit（他这一相真不在）。
  {
    const profile: PlayerProfile = { ...baseProfile(dayWaning), flags: new Set([...baseProfile(dayWaning).flags, 'intel.mira.silas']) };
    const chart = generateChart({ profile });
    const shown = chart.pois.filter((p) => p.templateId?.startsWith('roam.silas_meet_'));
    assert(
...
// —— 2. 港口在场门：met flag + 当前相位在窗内 ——
{
  ...
  assert(!isSpecialMerchantInPort(notMet), '未见过（无 flag.silas.met）→ 不在场');

  const metButWaning = { ...baseProfile(24), flags: new Set([...baseProfile(24).flags, 'flag.silas.met']) };
  assert(!isSpecialMerchantInPort(metButWaning), '见过但当前是 waning（他不在窗内）→ 不在场');

  const metAndNew = { ...baseProfile(3), flags: new Set([...baseProfile(3).flags, 'flag.silas.met']) };
