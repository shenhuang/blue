...
state = withProfile(
  [
    ...
    { itemId: 'item.cave_octopus_beak', qty: 9 }, // dk lv3(3) + dk lv4(2) + sonar lv1(2) + sonar lv3(2)
    { itemId: 'item.eel_skin', qty: 8 }, // sonar lv1(2) + sonar lv2(3) + sonar lv3(3)
  ],
  1000,
);
...
for (const id of ['upgrade.dive_kit.lv1', 'upgrade.dive_kit.lv2', 'upgrade.dive_kit.lv3', 'upgrade.dive_kit.lv4', 'upgrade.sonar.lv1', 'upgrade.sonar.lv2', 'upgrade.sonar.lv3']) {
...
log.push('  dive_kit lv1-4 + sonar lv1-3 → bonuses 聚合 → getRunBonuses 透传 → createNewRun 烤进 run ✓');
