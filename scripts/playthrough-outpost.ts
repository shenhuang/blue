...
//   9. 多前哨链：trench_deep（水流前哨）服务 abyssal 蛙跳、不服务更浅 band、多前哨选最深起跳
//  10. 多前哨链续（方向 D）：hadal_deep（超渊静水前哨）服务 subhadal（渊外 >180m）蛙跳、是最深起跳点
//
...
// ============================================================
// 12. 材料中转/寄存（Phase 2b 续，方向 D）：存/取/容量（衰减损耗删除后纯库房·不锈蚀）/建造保留寄存
// ============================================================
L('\n========== 12. 材料中转/寄存 ==========');
// 起手：lit reef_deep（静水）+ 建中转站 lv1（cap 6）。给足岸上料。
...
assert(storedUnits(storedMaterials(sD.profile, OUTPOST)) === 3, '12: 存后寄存用量 3');
...
assert(storedUnits(storedMaterials(sD.profile, OUTPOST)) === 3, '12: 超容量存料 no-op');
sD = depositToDepot(sD, OUTPOST, 'item.eel_skin', 3); // 填满到 6
assert(storedUnits(storedMaterials(sD.profile, OUTPOST)) === 6, '12: 填满到容量 6');
...
assert(storedUnits(storedMaterials(sD.profile, OUTPOST)) === 5, '12: 取回 1→用量 5');
assert(brassAt(sD) === brassBefore - 3 + 1, '12: 取回的料进岸上仓库');
assert(!canWithdraw(sD.profile, OUTPOST, 'item.brass_fitting', 99).ok, '12: 取超量→不可');
// 纯库房·不锈蚀（衰减损耗删除后）：raw stored == storedMaterials·二者皆 5
assert(storedUnits(sD.profile.outpostState![OUTPOST].stored) === 5, '12: raw stored 与有效寄存一致（不锈蚀·无衰减损耗）');
// 建造保留既有寄存（防御性：advanceOutpost 写 outpostState 不丢 stored）
let sAdv = stateWith(
  [
    { itemId: 'item.coral_shard', qty: 4 },
    { itemId: 'item.brass_fitting', qty: 5 },
...
sAdv = {
  ...
  profile: {
    ...sAdv.profile,
    outpostState: { [OUTPOST]: { stored: [{ itemId: 'item.brass_fitting', qty: 2 }] } },
  },
};
...
console.log(
  '\n✓ 深水前哨（Phase 2a 建造/蛙跳 + Phase 2b 能源/寄存/多前哨链 + abyssal 脊柱 + 章节哨站批解锁门/dev/蛙跳）回归通过',
);
