...
const pt = makeHarness('深水前哨（章节哨站建造/补给设施/解锁门/发现门 + 点亮 promote 灯塔带深度柱·#131 探深重构后）回归');
const { L } = pt;
const assert: PtAssert = pt.assert;

// 主线锚一座**章节前哨**（残骸前哨）——#131 后唯一活着的前哨形态（深脊柱前哨已删）。
// **主线柱迁移**：建造门从「本区锚点（requiresAnchor: 'wreck'）」翻成「**上一 beat** flag」——
// 残骸前哨 requiresFlag = story.ch1.anchor.reef（reef beat 做完才能建残骸前哨·解死锁·链式 build-gate）。
const OUTPOST = 'outpost.ch1_wreck';
const RESULT_LH = 'lighthouse.ch1_wreck_outpost';
// 残骸前哨的「可建门」flag = 上一 beat（reef）完成 flag（翻转后·单一来源 lighthouse_upgrades.json requiresFlag）。
...
// 主线柱迁移：残骸前哨建造门 = 上一 beat（reef）完成 flag（翻转后·不再是 requiresAnchor: 'wreck'）。
assert(def!.requiresFlag === GATE_FLAG, `0: 残骸前哨 requiresFlag=${GATE_FLAG}（上一 beat reef·翻转后）`);
assert(def!.requiresAnchor === undefined, '0: 残骸前哨不再用 requiresAnchor（已翻成 requiresFlag·主线柱迁移）');
...
// 全部留存前哨（主线柱迁移后：四座章节前哨 wreck/midwater/vent **翻成 requiresFlag=上一 beat flag**·
...
const CHAIN_GATE: Record<string, string> = {
  'outpost.ch1_wreck': ch1AnchorFlag('reef'),
  'outpost.ch1_midwater': ch1AnchorFlag('wreck'),
...
for (const o of allOutposts) {
  ...
  // 三座主线区前哨（wreck/midwater/vent）的 requiresFlag = **上一 beat** flag（reef←·wreck←·midwater←·防死锁）。
  if (CHAIN_GATE[o.id]) {
...
assert(!allOutposts.some((o) => o.requiresFlag === ch1AnchorFlag('reef') && o.id !== 'outpost.ch1_wreck'), '0: reef beat 免费入口·仅作为残骸前哨的解锁前置·不另设 reef 哨站');
...
// ============================================================
L('\n========== 1. 三阶段推进 + promote ==========');
// 备齐残骸前哨全程料（s1 scrap×2/40·coral→scrap 经济 2026-06-28 · s2 crab×2+brass×2/70 · s3 lantern×2/110）→ scrap2 crab2 brass2 lantern2, gold 300
let s = withGate(
  stateWith(
    [
...
assert(round!.version === 13, '4: SAVE_VERSION 13（round-trip 后不变·感知门 diveModifier.gate bump）');
L('  stage flag round-trip / SAVE_VERSION 13 不变 ✓');

// ============================================================
// 5. 补给设施：建成即全额生效（能源容量门控已删·2026-06-21）
// ============================================================
L('\n========== 5. 补给设施建成即全额生效 ==========');
// 残骸前哨点亮后建充电桩(+20 电池) + 制氧机(+10 氧)：两者都该全额计入（不再有「同时在线几个」的能源门）。
let sE = withGate(
  stateWith(
    [
...
L(`  残骸前哨：充电 +${bonE.rechargeBonus} / 制氧 +${bonE.oxygenSupply}（两者都在·无能源门）✓`);
...
// 备齐残骸前哨全程料（s1 scrap×2/40·coral→scrap 经济 2026-06-28 · s2 crab×2+brass×2/70 · s3 lantern×2/110）
