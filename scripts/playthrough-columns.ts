...
L('\n========== 7. 柱潜点下潜落 run ==========');
{
  ...
  assert(after.run!.diveModifier?.gate?.sense === band.gate?.sense, '7: run.diveModifier.gate 落 band（sense 一致）');
}
L('  从 lit 档 startDiveFromPoi → run 落 zone/turn0/alert/hunt/gate ✓');
