 ...
 * 又能在长桥率明显恶化时变红。
 *
 * **2026-07-22 复议后拍板：就留 1.5‰、不下压到 1‰——这条别再当待办捡起。** 理由是分子只有 2：泊松下 2/3912 的
 * 95% 区间约 0.06‰–1.85‰，1.5‰ 本来就落在「main 现状的采样噪声带」之内、1‰ 已压到带下面。而本门是**确定性**的
 * （同 seed 同图），所以「误红」不是 CI 意义的随机 flake，而是**任何打乱 seed→图 映射的良性改动**（改域宽/撒点
 * 密度/点数预算）把长桥数从 2 抬到 4 就判回归——λ=2 时 P(≥4)≈14%、P(≥6)≈1.7%，差一个量级。门失信一次比漏一次贵。
 * 真要更灵敏，**杠杆是加大分母**（抬 LONG_BRIDGE_SEEDS 让 λ 变大、相对区间收窄），不是把线往噪声里挪。
 * 代价（已知且接受）：作者曾试 vertical 横纵比 0.5→0.7 把越界推到 ~1.2‰（因别的原因否决）——那个档次的回退
 * 当前门逮不住。**这是门参数、非玩法手感**（[[defer-number-tuning]] 数值统调不含它·它不是手感）。 */
