...
// 月相经济（Phase 3 MVP·SPEC §8）：用上弦相（day=9·lunarPriceMultiplier=1.0·中性）确保单价断言不受月相浮动影响。
// Mira 价格随月相浮动、但中性相位 ≡ 基线——这是该功能的预期行为，不是 bug。
state = { ...state, profile: { ...state.profile, day: 9 } }; // 上弦小潮·价格乘子 1.0
...
L('\n========== Mira 回购：买入扣金 + 进仓库 ==========');
state = createInitialGameState();
// 月相经济（Phase 3 MVP·SPEC §8）：上弦相 day=9·乘子 1.0·买价 = 基线（miraBuyPriceFor）。
state = { ...state, profile: { ...state.profile, bankedGold: 1000, day: 9 } };
...
state = createInitialGameState();
// 月相经济（Phase 3 MVP·SPEC §8）：上弦相 day=9·乘子 1.0·买价 = 基线（后续 Phase 6 buyFromMira 用）。
state = { ...state, profile: { ...state.profile, bankedGold: 100000, inventory: [], day: 9 } };
