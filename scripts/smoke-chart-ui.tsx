...
// 解锁声呐、未扫过（空记忆）→ 面板在。
// 白板收口：「未扫过＝空态(全黑)」只对开放水域成立（洞穴里你脚下那间恒可见·#1·SonarScanPanel L621）——
// 开放水域 zone 已全删，空态无从由存活 zone 复现，故 sonar-scan-empty 断言随之移除；只留「面板在」。
...
const n1Revealed = devRevealOutpost(stateWith(['flag.tutorial_complete', 'flag.warren_discovered'], []), 'outpost.ch1_wreck');
...
// （原「开阔水域空记忆仍是空态」对照块随开放水域 zone 全删移除·白板收口——空态只对 open-water 成立·无存活 zone 可复现。）
...
L('  洞穴空记忆非空态(脚下那间可见) · SSR 无交互态残留 · 敞口通道伸向未扫端并收窄 ✓');
