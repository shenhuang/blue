...
function withLogbook(s: GameState): GameState {
  return { ...s, profile: { ...s.profile, inventory: [...s.profile.inventory, { itemId: MENTOR_LOGBOOK_ITEM_ID, qty: 1 }] } };
}
...
//    剧情锚点恒显；蓝洞群已迁 home（§7·cave-chart）→ 随家区出现；沉船墓园属未解锁残骸区→不揭示；
//    蛙跳入口收编进 home 灯塔 popup。
// ============================================
L('========== A. 教学后 · 无升级（持导师日志·主线柱迁移）==========');
// 主线柱迁移（2026-06-28 内容自洽回归）：教学尾拿导师日志（inventory 有 item.mentor_logbook·marksPois 带四坐标）→
// 四条主线 beat 坐标早揭示。reef host=home 恒在 → lit（「漆号珊瑚丛」恒显于家区·免费入口）；wreck/midwater/vent host 前哨未建 → dim。
const A = withLogbook(stateWith(['flag.tutorial_complete'], []));
...
// 主线 beat①（reef·host=home 恒在·持日志 lit）＝「漆号珊瑚丛」恒显于家区（免费入口）。
assert(htmlA.includes('漆号珊瑚丛'), 'A: 主线 beat① reef「漆号珊瑚丛」持日志后恒显于家区（host=home·lit·免费入口·主线柱迁移）');
...
L('  渲染成功：家区 POI（含迁 home 的蓝洞群）+ 剧情锚点恒显 + home 灯塔节点 ✓');
...
const lockedChoice: NodeChoice[] = [
  { nodeId: 'n1', depth: 25, zoneTag: 'wreck', preview: '太暗，看不清——需要灯', clarity: 'none', locked: true },
];
...
const htmlN2 = renderToStaticMarkup(
  <OutpostPopup outpostId="outpost.ch1_wreck" state={N2} onStateChange={noop} />,
);
...
const htmlSLock = renderToStaticMarkup(
  <OutpostPopup outpostId="outpost.ch1_wreck" state={sLockState} onStateChange={noop} />,
);
...
// 主线柱迁移：残骸前哨「可建门」翻成上一 beat（reef）flag（story.ch1.anchor.reef）——不再是本区 anchor.wreck。
...
const htmlSLit = renderToStaticMarkup(
  <OutpostPopup outpostId="outpost.ch1_wreck" state={sLitChapWithAnchor} onStateChange={noop} />,
);
...
// 导师日志详情（2026-06-28 内容自洽回归）：正文（lore 已读）+ **陈列四条文献坐标**（marksPois 恢复·#117 续）——
// 「最后是 4 个坐标：」。TLock 只 home 灯塔 → reef（host=home）lit＝可点行（locker-coord-row clickable·跳海图）；
// wreck/midwater/vent host 前哨未建 → dim＝纯陈列行（看得到去不了·与教学「圈下四个坐标」自洽）。
...
assert(htmlTLogbook.includes('4 个坐标'), 'T: 导师日志详情陈列四条文献坐标（marksPois 恢复·日志携带四坐标·#117 续）');
assert(htmlTLogbook.includes('locker-coord-row clickable'), 'T: 导师日志详情有可点坐标行（reef host=home → lit·可跳海图选中）');
assert(!htmlTLogbook.includes('摊开海图'), 'T: 导师日志非海图信物·详情无「摊开海图」');
assert(!htmlTLogbook.includes('surveyed'), 'T: 未完成任何 beat → 日志坐标无「划掉」（surveyed 仅 beat 完成才有）');
// 已勘划掉态（2026-06-28·作者「去过的坐标拿到关键结果后在日志里划掉」）：reef beat 完成（anchor.reef 置位）→
// 该坐标在日志列表「划掉」（surveyed·删除线）·且仍可点重访（clickable·host=home 恒 lit）。
const TLockSurveyed = {
  ...TLock,
  profile: { ...TLock.profile, flags: new Set([...TLock.profile.flags, 'story.ch1.anchor.reef']) },
};
const htmlTLogbookSurveyed = renderToStaticMarkup(
  <LockerView
    state={TLockSurveyed}
    onStateChange={noop}
    onClose={noop}
...
assert(
  htmlTLogbookSurveyed.includes('locker-coord-row surveyed clickable'),
  'T: reef beat 完成 → 日志该坐标「划掉」（surveyed）且仍可点重访（clickable）',
);
...
// 引擎（2026-06-28 内容自洽回归）：导师日志 marksPois 恢复＝四条柱派生 story 潜点坐标（poi.dive.<短名>.story）；
// 旧海图 marksPois 仍空（坐标已迁导师日志#142·old_chart 只 opensChart）。reveal 走文献坐标·reach build-gated（engine 侧由 playthrough-chart/columns 守）。
assert(itemMarkedPois('item.old_chart').length === 0, 'T: 旧海图 marksPois 仍空（坐标迁导师日志#142·old_chart 只 opensChart）');
assert(itemMarkedPois('item.mentor_logbook').length === 4, 'T: 导师日志 marksPois 恢复＝四条柱 story 潜点坐标（日志携带四坐标·#117 续·内容自洽回归）');
L('  剧情/其它/装备归类 · 文献正文可读 · 导师日志陈列四文献坐标（reef 可点·余 dim）· 旧海图回退摊图 ✓');
