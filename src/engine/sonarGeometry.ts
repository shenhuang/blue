...
// ─── 临渊侧陆架坡折（#333·开阔水域 SPEC §6.4「otherSide:'midwater' 敞口侧无底·陆架断崖式」）────────────
// 单侧墙 + otherSide='midwater' 的敞侧：旧实装是一刀布尔硬切（beyondOpenSide→OW_WATER·海床右缘竖切成
// 一堵垂直岩面·作者反馈「被截断·不是边缘下降」）。改成**陆架坡折**：过坡折起点（最外节点 x ± OW_WALL_MARGIN）
// 后海床线性下沉（缓入·无尖角）·越远越深 ⇒ 落出取景＝临渊无底（openWaterRender::openSideDrop）。
// 与 floor/wall/cave 各自独立旋钮（调坡折绝不动底/墙·延续 §2.4）。起步值·手感一律 defer §9（[[defer-number-tuning]]）。
export const OW_OPENSIDE_SLOPE = 1.2; // 坡折下沉率（世界 y 每世界 x·越大越陡·>0 才是下降·0 会退回竖切观感）
export const OW_OPENSIDE_EASE = 18; // 坡折肩部缓入距离（世界·斜率在此距离内 0→满·避免坡折起点长出尖角·同 §5.3「圆钝非尖脊」）
export const OW_OPENSIDE_REVEAL = 90; // 临渊坡折可见余量（世界）：dev 概览 owFloorBottom 下沿 + 潜水取景盒敞侧外扩（横向 REVEAL/SLOPE·纵向 REVEAL）共用·让「断崖往下掉」在游戏内也看得见
