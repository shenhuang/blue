...
// band 引用 zone 提供内容、用自己的绝对 depthRange 覆盖 zone.depthRange（经 startDiveFromOutpost → mapgen）。
...
/** 全部 band，按 order 升序（order 越大越深）。 */
const BANDS: DepthBand[] = [...file.bands].sort((a, b) => a.order - b.order);
 ...
 * 深度不走 depthOffset——band 用绝对 depthRange 覆盖 zone（见 mapgen GenOpts.depthRange / startDiveFromOutpost）。
