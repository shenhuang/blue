 ...
 */
function maybeInterceptJuvenile(
  state: GameState,
 ...
 */
function maybeConsumeJuvenile(state: GameState): GameState {
  if (state.phase.kind !== 'combat') return state;
  const combat = state.phase.combat;
  let s = state;
  ...
  for (const e of combat.enemies) {
    ...
    const juvenileDef = ENEMY_DEFS.get(juvenile.defId);
 ...
 */
function applyMaternalEnrageIfAlone(state: GameState): GameState {
  if (state.phase.kind !== 'combat') return state;
  const combat = state.phase.combat;
  let s = state;
