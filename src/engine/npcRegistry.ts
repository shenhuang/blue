...
// `[aldoData, miraData, ottoData]` 的 import 列表，加 Sela 时两处都要记得改一个明显的重复维护点
// （trust.ts 那份漏改过一次·debug 才发现「攒够信任但 tier 不涨」——thresholds 表没扫到 sela.json）。
...
import selaData from '@/data/npcs/sela.json';
...
export const NPC_FILES: NpcFile[] = [
  ...
  selaData as unknown as NpcFile,
];
