...
const SHOP_STOCK_CONSUMABLES: Record<string, number> = {
  ...
  // 深潮币（藏宝贸易与信任系统 SPEC §4）：Mira 少量高价卖·特殊商人 Silas 货架花它（§6.3）。
...
// ============================================================
// 特殊商人 Silas（藏宝贸易与信任系统 SPEC §6·Phase 2 MVP）——
// 探险家镜像·货架双门控（深潮币 `item.deep_token` + 信任档 `npcTrustTier`）。
...
export const SPECIAL_MERCHANT_NPC_ID = 'npc.silas';
export const SPECIAL_MERCHANT_SHOP_ID = 'shop.silas';

/**
 * Silas「在港」的月相窗（须与 chart_pois.json 里 3 个 roam.silas_meet_* 模板各自的 lunarWindow
...
/**
 * Silas 是否「在港」可对话/交易（PortView 据此显/藏他的 NPC 卡片）：见过一次（flag.silas.met·
...
export function isSpecialMerchantInPort(profile: PlayerProfile): boolean {
  ...
  return (
    profile.flags.has('flag.silas.met') && SPECIAL_MERCHANT_ACTIVE_PHASES.includes(lunarPhase(day))
  );
}

/** 深潮币 item id（唯一花费货币·§4）。 */
const TOKEN_ITEM_ID = 'item.deep_token';

/**
 * Silas 货架（§6.3·数值 defer-number-tuning）：itemId → 花费 token 数 / 所需信任档 / 每次回港补货上限。
...
/** 该 itemId 是否在 Silas 货架上（不判信任档——UI/校验分开问，见 listSpecialMerchantShelf）。 */
...
/** 列出 Silas 全货架（含未达信任档的条目——UI 据 `locked` 显示"未达 X 信任"，同金币不足提示口径）。 */
...
/**
 * 从 Silas 买 `itemId` `qty` 件：token 够 + 信任档够 + 备货够就买多少（min 三者）；否则原样返回（no-op）。
 * 交易额涨信任（§3.5①）：净花费 token 数直接进 gainTrust（Silas 自带 thresholds [10,30,60]·npcs/silas.json）。
 */
